...
<#
.SYNOPSIS
    Generates language-neutral AppHost API reference JSON files by running
    `aspire sdk dump --format json` for each hosting integration with ATS capabilities.
...
.DESCRIPTION
    ...
    3. Outputs one semantic package document to src/frontend/src/data/apphost-modules/
...
.PARAMETER OutputDir
    ...
    Defaults to <repo-root>/src/frontend/src/data/apphost-modules.

.PARAMETER SupportOutput
    Path to the aggregated language-support JSON file. Defaults to
    ASPIRE_API_LANGUAGE_SUPPORT_FILE or
    <repo-root>/src/frontend/src/data/apphost-language-support.json.

.PARAMETER PackageFilter
    Optional wildcard filter to process only specific packages (e.g. "*Redis*").

.PARAMETER BaseModulePath
    Optional previously generated Aspire.Hosting semantic module used to
    deduplicate integration packages when core is not part of this invocation.

.PARAMETER SkipBuild
    Skips building AtsJsonGenerator before generation. Use only when the tool
    has already been built, such as parallel resumable chunk generation.
...
.PARAMETER DumpCliVersion
    Optional Aspire CLI version recorded in dump provenance.

.PARAMETER DumpGeneratedAt
    Optional workflow-supplied generation timestamp recorded in dump provenance.
...
.EXAMPLE
    ...
    ./generate-apphost-api-json.ps1 -AspireRepoPath D:\GitHub\aspire

    # Using Name@Version syntax with a local CLI build
    ./generate-apphost-api-json.ps1 -NuGetPackageVersion "Aspire.Hosting@13.1.2","Aspire.Hosting.Redis@13.1.2" `
        -AspireCliProject D:\GitHub\aspire\src\Aspire.Cli\Aspire.Cli.csproj

    # Filter to specific packages
    ./generate-apphost-api-json.ps1 -AspireRepoPath D:\GitHub\aspire -PackageFilter "*Redis*"
#>
...
# Opt-in resilience: when ASPIRE_APPHOST_API_CARRY_FORWARD=1, a package whose ATS dump
...
$CarryForwardOnDumpFailure = ($env:ASPIRE_APPHOST_API_CARRY_FORWARD -eq '1')

$FinalOutputDir = if ($OutputDir) {
    [System.IO.Path]::GetFullPath($OutputDir)
}
else {
    Join-Path $RepoRoot "src\frontend\src\data\apphost-modules"
}
$FinalSupportOutput = if ($SupportOutput) {
    [System.IO.Path]::GetFullPath($SupportOutput)
}
elseif (-not [string]::IsNullOrWhiteSpace($env:ASPIRE_API_LANGUAGE_SUPPORT_FILE)) {
    [System.IO.Path]::GetFullPath($env:ASPIRE_API_LANGUAGE_SUPPORT_FILE)
}
else {
    Join-Path $RepoRoot "src\frontend\src\data\apphost-language-support.json"
}
...
$StagedSupportOutput = "$OutputDir.support.json"
New-Item -ItemType Directory -Path $OutputDir -Force | Out-Null

$TempDir = Join-Path $OutputDir ".tmp-dumps"
New-Item -ItemType Directory -Path $TempDir -Force | Out-Null

function Remove-StaleAppHostModuleFiles {
    param(
        [string]$PackageName,
...
function Get-AppHostModuleApiItemCount {
    param([psobject]$ModuleJson)

    return Get-JsonArrayPropertyCount -Object $ModuleJson -PropertyName "items"
}

function Remove-EmptyAppHostModuleFile {
    param(
        [string]$PackageName,
        [string]$OutputFile
    )
    ...
    if (-not (Test-Path $OutputFile)) {
        throw "Cannot inspect missing AppHost module file at $OutputFile"
    }

    $json = Get-Content $OutputFile -Raw | ConvertFrom-Json
    if ((Get-AppHostModuleApiItemCount -ModuleJson $json) -gt 0) {
        return $false
    }

    Remove-Item $OutputFile -Force
    Write-Host "  Omitted empty AppHost module for $PackageName" -ForegroundColor DarkYellow
    return $true
}
...
function Test-IsAppHostSdkPackage {
    [CmdletBinding()]
    ...
    return @(
        Get-ChildItem -Path $packageJsonDir -Filter '*.json' -File |
            ForEach-Object {
                ...
                if ((Test-IsAppHostSdkPackage -PackageName $json.package.name) -and
...
if (-not $AspireRepoPath -and -not $hasExplicitNuGetPackageVersion) {
    ...
    if ($hostingPackages.Count -eq 0) {
        Write-Error "No AppHost SDK package JSON files found in src/frontend/src/data/pkgs"
        return
    }
    ...
    if ($duplicatePackages.Count -gt 0) {
        ...
        throw "Multiple generated C# package versions prevent deterministic AppHost API generation:`n  $($details -join "`n  ")"
    }
    ...
    if ($NuGetPackageVersion.Count -eq 0) {
        Write-Error "No versioned AppHost SDK package JSON files found in $packageJsonDir"
        return
    }

    Write-Host "  Found $($NuGetPackageVersion.Count) AppHost SDK packages to process" -ForegroundColor DarkGray
}
...
function Invoke-AspireCli {
    ...
    $previousRestoreIgnoreFailedSources = $env:RestoreIgnoreFailedSources
    $previousRestoreDisableParallel = $env:RestoreDisableParallel
    if (-not $AspireRepoPath) {
        $env:RestoreSources = $NuGetOrgServiceIndex
    ...
    if ($AspireCliProject) {
        ...
        $env:RestoreIgnoreFailedSources = $previousRestoreIgnoreFailedSources
        $env:RestoreDisableParallel = $previousRestoreDisableParallel
    }
...
if ($AspireRepoPath) {
    ...
    # Only hosting packages expose ATS capabilities for generated AppHost SDKs.
...
# Only Aspire.Hosting.* packages expose ATS capabilities for generated AppHost SDKs.
# Client/component packages (e.g. Aspire.Azure.Data.Tables) are not applicable.
if ($NuGetPackageVersion -and $NuGetPackageVersion.Count -gt 0) {
    foreach ($spec in $NuGetPackageVersion) {
        ...
        if (-not (Test-IsAppHostSdkPackage -PackageName $pkgName)) {
...
if (-not $AspireRepoPath) {
    ...
    if ($officialFeed.IsRelease) {
        Write-Host "Release branch detected ($($officialFeed.BranchName)). AppHost module generation will resolve official Aspire packages from $($officialFeed.DisplayName)." -ForegroundColor Cyan
    }
}
...
if (-not $SkipBuild) {
    ...
    if ($LASTEXITCODE -ne 0) {
        ...
        Remove-Item -Path $StagedSupportOutput -Force -ErrorAction SilentlyContinue
        exit 1
    }
}
else {
    Write-Host "Using prebuilt AtsJsonGenerator..." -ForegroundColor DarkGray
}
...
$skippedPackageNames = [System.Collections.Generic.HashSet[string]]::new(
    [StringComparer]::OrdinalIgnoreCase)
$coreOutputFile = if ($BaseModulePath) {
    $resolvedBaseModulePath = [System.IO.Path]::GetFullPath($BaseModulePath)
    if (-not (Test-Path $resolvedBaseModulePath -PathType Leaf)) {
        throw "Base AppHost module not found: $resolvedBaseModulePath"
    }
    $resolvedBaseModulePath
}
else {
    $null
}
...
foreach ($pkg in $corePackages) {
    ...
    try {
        ...
        if (-not [string]::IsNullOrWhiteSpace($DumpCliVersion)) {
            $transformArgs += @("--dump-cli-version", $DumpCliVersion)
        }
        if (-not [string]::IsNullOrWhiteSpace($DumpGeneratedAt)) {
            $transformArgs += @("--dump-generated-at", $DumpGeneratedAt)
        }
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            if ($version) {
                Remove-StaleAppHostModuleFiles -PackageName $name -CurrentOutputFile $outputFile -OutputDirectory $OutputDir
            }
        } else {
...
# Verify core was generated or supplied (needed as base for dedup)
...
foreach ($pkg in $integrationPackages | Sort-Object { $_.Name }) {
    ...
    try {
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            if (Remove-EmptyAppHostModuleFile -PackageName $name -OutputFile $outputFile) {
                $skipped++
...
Write-Host "Aggregating AppHost language support..." -ForegroundColor Cyan
$supportArgs = @(
    "run", "--project", $ToolProject, "--no-build", "--",
    "support",
    "--input-dir", $OutputDir,
    "--output", $StagedSupportOutput
)
if (-not $isFullReconciliation -and (Test-Path $FinalOutputDir)) {
    $supportArgs += @("--baseline-dir", $FinalOutputDir)
    $packagesToReplace = @($Packages | Where-Object {
        -not $failedPackageNames.Contains($_.Name)
    } | ForEach-Object {
        $_.Name
    } | Sort-Object -Unique)
    if ($packagesToReplace.Count -gt 0) {
        $supportArgs += "--replace-package"
        $supportArgs += $packagesToReplace
    }
}

& dotnet @supportArgs
if ($LASTEXITCODE -ne 0 -or -not (Test-Path $StagedSupportOutput)) {
    Write-Host "Failed to aggregate AppHost language support" -ForegroundColor Red
    Remove-Item -Path $OutputDir -Recurse -Force -ErrorAction SilentlyContinue
    Remove-Item -Path $StagedSupportOutput -Force -ErrorAction SilentlyContinue
    exit 1
}
...
$supportDirectory = [System.IO.Path]::GetDirectoryName($FinalSupportOutput)
if (-not [string]::IsNullOrWhiteSpace($supportDirectory)) {
    New-Item -ItemType Directory -Path $supportDirectory -Force | Out-Null
}
Copy-Item -Path $StagedSupportOutput -Destination $FinalSupportOutput -Force
Write-Host "Updated language support: $FinalSupportOutput" -ForegroundColor Green
