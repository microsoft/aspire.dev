...
public sealed class YouTubeClient : IYouTubeClient
{
    ...
    {
        var c = _httpFactory.CreateClient(HttpClientName);
    ...
    {
        var apiKey = _options.CurrentValue.YouTube.ApiKey;
    ...
    {
        var c = _httpFactory.CreateClient(PubSubHttpClientName);
        ...
        if (!response.IsSuccessStatusCode)
        {
            ...
            _logger.LogWarning("YouTube WebSub subscribe failed: {Status} {Body}", response.StatusCode, body);
            response.EnsureSuccessStatusCode();
        }
    }
}
