...
test('same-deployment navigation and history preserve the running document', async ({ page }) => {
  ...
  await expect.poll(() => page.evaluate(() => Reflect.get(window, '__deploymentSession'))).toBe(true);
  await page.goBack();
...
for (const identity of ['changed', 'missing'] as const) {
  test(`${identity} deployment uses native navigation before incoming scripts execute`, async ({ page }) => {
    const errors: string[] = [];
    ...
    await expect.poll(() => page.evaluate(() => Reflect.get(window, '__deploymentSession'))).toBeUndefined();
...
test('native Document PiP survives same-build navigation and closes on a deployment reload', async ({
  ...
  await expect.poll(() => page.evaluate(() => Boolean(Reflect.get(window, '__nativePipTest')))).toBe(true);
  await page.locator('header a[href="/docs/"]:visible').click();
  await expect(page).toHaveURL(/\/docs\/$/);
  ...
  await expect(page).toHaveURL(url => url.pathname === '/');
  await closed;
  await expect.poll(() => page.evaluate(() => Boolean(Reflect.get(window, '__nativePipTest')))).toBe(false);
