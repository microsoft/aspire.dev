...
test('featured samples filter real metadata and retain language when browsing all samples', async ({ page }) => {
  ...
  const select = samples.getByRole('combobox', { name: 'Sample language' });
  ...
  await expect(samples.getByRole('status')).toHaveText('6 featured samples');
  ...
  await expect(page).toHaveURL(/\/hub\/browse\/\?type=sample&language=typescript$/, { timeout: 30_000 });
  ...
  expect(entries.every((entry) => entry.type === 'sample' && entry.language.includes('typescript'))).toBe(true);
});
...
test('featured sample deep links, reload, history, and reset preserve user intent', async ({ page }) => {
  ...
  await expect(page).toHaveURL(/keep=1&sample-language=typescript#samples$/);
...
test('featured sample empty state recovers and unspecified samples remain visible only for all languages', async ({ page }) => {
  ...
  await expect(samples.getByRole('heading', { name: 'No featured samples for Python' })).toBeVisible();
  await expect(samples.getByRole('status')).toHaveText('0 of 6 featured samples: Python');
  await samples.getByRole('button', { name: 'Clear language filter' }).click();
...
test('featured filter reflows and passes focused accessibility checks in both themes', async ({ page }) => {
  ...
  const samples = page.locator('featured-samples');
  await samples.getByRole('combobox', { name: 'Sample language' }).selectOption('typescript');
