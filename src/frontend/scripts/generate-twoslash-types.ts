...
let classBaseCount = 0;
if (existsSync(PKGS_DIR)) {
  const pkgFiles = readdirSync(PKGS_DIR)
    .filter((f) => f.endsWith('.json'))
    .sort();
  for (const f of pkgFiles) {
    const pkg = JSON.parse(readFileSync(resolve(PKGS_DIR, f), 'utf8')) as PkgJson;
    const identity = packageIdentity(pkg.package);
    let classBaseByFullName = classBasesByPackage.get(identity);
    if (!classBaseByFullName) {
      classBaseByFullName = new Map<string, string>();
      classBasesByPackage.set(identity, classBaseByFullName);
    }
    for (const t of pkg.types ?? []) {
      ...
      if (existing && existing !== t.baseType) {
        throw new Error(
          `Conflicting base types for ${t.fullName} in ${identity}: ${existing} and ${t.baseType}`
        );
      }
      if (!existing) {
  ...
  console.log(
    `   + ${classBaseCount} class-inheritance links from ${classBasesByPackage.size} pkgs/ package(s)`
  );
}
...
const handlePackageByName = new Map<string, string>();
...
for (const mod of modules) {
  ...
  for (const h of mod.handleTypes ?? []) {
    if (!handleByName.has(h.name)) {
      ...
      handlePackageByName.set(h.name, packageIdentity(mod.package));
      handleTypes.push(h);
    }
  }
...
function getBaseTypeHierarchy(h: HandleType): string[] {
  ...
  const packageKey = handlePackageByName.get(h.name);
  const classBaseByFullName = packageKey ? classBasesByPackage.get(packageKey) : undefined;
