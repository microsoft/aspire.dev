...
`certificates.nssDbPaths` takes precedence over the upstream
`DOTNET_DEV_CERTS_NSSDB_PATHS` environment variable. When the Aspire setting
has no value, the CLI falls back to `DOTNET_DEV_CERTS_NSSDB_PATHS` and then to
normal NSS database discovery — the upstream environment variable's behavior
is unchanged. Run [`aspire certs trust`](../commands/aspire-certs-trust/) to
apply the configured trust settings.
