...
Since Aspire 13.6, the hook runs the CLI's native `aspire agent telemetry --hook` executable directly rather than through a shell wrapper, reducing the overhead added to every agent tool call. GitHub Copilot CLI and Claude Code hook registrations still match every tool call, so coverage is unchanged.
