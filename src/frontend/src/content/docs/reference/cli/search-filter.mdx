...
Use the `timestamp` field qualifier to narrow structured logs, traces, and spans to a specific date/time range. The qualifier accepts ISO 8601 date/time strings together with a comparison operator:
...
aspire otel logs --search "timestamp:>=2024-06-01T12:00:00Z severity:error"
```
