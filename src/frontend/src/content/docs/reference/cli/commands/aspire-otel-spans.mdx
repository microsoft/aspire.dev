...
- **`--search <search>`**
  ...
  Supported fields for spans: `resource`, `name`, `span-id` (`spanid`), `trace-id` (`traceid`), `status`, `kind`, `duration`.
...
- View spans with a specific name:

  ```bash title="Aspire CLI"
  aspire otel spans --search "GET /api/orders"
  ```
