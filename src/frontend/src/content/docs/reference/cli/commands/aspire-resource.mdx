...
### Log file paths on failure

When a resource command fails, the CLI prints the paths to the log files so you can inspect detailed diagnostics:
...
- **See logs at** — the log file for the CLI process that ran the command.
- **See AppHost logs at** — the log file for the AppHost process the CLI connected to (only printed when the AppHost log path is known).

These log files contain detailed output that is not normally displayed at the console. Sharing them is the recommended first step when reporting issues with resource commands.
...
- Use `--` to avoid conflict with an Aspire CLI option named `--apphost`:
  ...
  ```bash title="Aspire CLI"
  aspire resource my-resource custom-cmd --apphost './apphost.ts' -- --apphost myvalue
  ```
