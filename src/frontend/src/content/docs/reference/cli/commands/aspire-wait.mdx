...
The command connects to a running AppHost via the backchannel and streams resource state changes in real-time. It validates that the specified resource exists before entering the wait loop, so typos in resource names are caught immediately rather than causing a silent timeout. If the resource enters the `FailedToStart` state, the wait fails immediately with exit code 18, even when the target status is `down` — a resource that failed to start is never treated as a successful "down" outcome.
...
| 18   | Resource entered the `FailedToStart` state, regardless of the target status    |
