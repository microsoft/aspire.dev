...
`aspire ps` provides an AppHost-level summary only. To inspect per-resource details or continuously stream resource state changes, use [`aspire describe`](/reference/cli/commands/aspire-describe/).
