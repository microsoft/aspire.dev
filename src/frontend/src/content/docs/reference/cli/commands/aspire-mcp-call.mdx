...
The `aspire mcp call` command calls an <abbr title="Model Context Protocol" data-tooltip-placement="top">MCP</abbr> tool exposed by a resource in a running AppHost. Use `aspire mcp tools` first if you need to discover the available resources and tool names.
...
- **`<resource>`**
  ...
  The name of the resource that exposes the <abbr title="Model Context Protocol" data-tooltip-placement="top">MCP</abbr> tool.

- **`<tool>`**

  The name of the <abbr title="Model Context Protocol" data-tooltip-placement="top">MCP</abbr> tool to call.
