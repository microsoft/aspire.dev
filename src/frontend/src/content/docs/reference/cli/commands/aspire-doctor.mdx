...
<Aside type="note">
  ...
  on Unix, `certutil` process trees so timed-out checks don't leave child
  processes running.
</Aside>
...
When the aggregate two-minute timeout for all environment checks is reached, an
`environment-checks` entry is added with the same
`checkType`/`timeoutSeconds` metadata shape, identifying the check that was
running when the aggregate deadline expired, and any remaining checks are
skipped.
