...
- **`--search <search>`**
  ...
  Search and filter telemetry fields such as log messages, attribute values, names, source, and IDs. Supports full-text search and field filters. For more details on supported search syntax, see the [CLI search documentation](https://aka.ms/aspire/cli-search).
...
- Search logs for a specific term:
  ...
  ```bash title="Aspire CLI"
  aspire otel logs --search "database timeout"
  ```
