...
- **`--search <search>`**
  ...
  Full-text search across log text fields, such as log messages, attribute values, scope, IDs, and timestamps. Supports plain free-text fragments and structured `field:value` qualifiers. For more information, see [Search and filter](/reference/cli/search-filter/).
