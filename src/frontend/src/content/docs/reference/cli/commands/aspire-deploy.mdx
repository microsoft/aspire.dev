...
The `aspire deploy` command first invokes the [`aspire publish`](../aspire-publish/) command. After which, Aspire executes all deployment pipeline steps registered in the app model.
...
- Executes all publish pipeline steps registered in the app model.
- Executes all deploy pipeline steps registered in the app model.
