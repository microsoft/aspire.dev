...
<Aside type="note">
  The `aspire update` command requires an interactive terminal. The `--non-interactive` flag is not currently supported for this command. In CI/CD pipelines or other non-interactive environments, update Aspire packages manually using `aspire add {package}` or equivalent tooling.
</Aside>
