...
A repository can pin the Aspire CLI independently of the executable you're currently running, using an `Aspire.Cli` entry in `.config/dotnet-tools.json` or `dotnet-tools.json`, or a `@microsoft/aspire-cli` entry in `package.json`. `aspire update` updates those repository-local pins alongside AppHost integrations.
