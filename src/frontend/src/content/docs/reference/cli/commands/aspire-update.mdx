...
- **`--channel`**
  ...
  When you switch to an explicit channel (any value other than the implicit stable default), `aspire update` also rewrites the top-level `channel` key in `aspire.config.json` so that subsequent `aspire add` and `aspire update` runs resolve packages against the channel you just switched to.
