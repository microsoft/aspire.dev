...
- **`--channel`**
  ...
  `--channel staging` is only synthesized on CLI builds whose baked identity is `stable` or `staging`. On `daily`, `local`, and `pr-<N>` CLI builds, there's no reliable feed to resolve a genuine staging package set from, so `aspire update --channel staging` fails with `FailedToUpgradeProject` and an error naming the running identity instead of silently resolving `daily` packages under the `staging` label. To opt back in on those builds, either:

  - Set the `overrideStagingFeed` configuration entry to an explicit staging feed URL, for example `aspire config set -g overrideStagingFeed <feed-url>`, or
  - Enable the `StagingChannelEnabled` feature flag for dev/test scenarios.

  When staging channel resolution succeeds, the resolved feed URL, quality, and pinned version are logged so you can confirm what `--channel staging` actually selected.
