...
To stop the running AppHost and exit, press <Kbd windows="Ctrl+C" mac="Command+C" linux="Ctrl+C" /> (or send `SIGTERM` on Linux/macOS). The CLI requests a graceful shutdown of the AppHost and its resources.

If graceful shutdown is taking too long and you need to exit immediately, press <Kbd windows="Ctrl+C" mac="Command+C" linux="Ctrl+C" /> a second time to terminate the process immediately.

## Hot Reload and watch behavior

By default, `aspire run` starts the distributed application once. It doesn't watch the AppHost or resource source files. After you change AppHost code, stop the running AppHost with <Kbd windows="Ctrl+C" mac="Command+C" linux="Ctrl+C" />, and then run `aspire run` again. For individual resource changes, keep the AppHost running and restart or rebuild the resource from the Aspire CLI or dashboard when needed.
