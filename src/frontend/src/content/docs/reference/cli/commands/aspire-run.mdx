...
If graceful shutdown is taking too long and you need to exit immediately, press <kbd>Ctrl</kbd>+<kbd>C</kbd> a second time to force-terminate the process.

:::note
Pressing <kbd>Ctrl</kbd>+<kbd>C</kbd> during AppHost startup (before all resources are running) cancels the startup immediately and exits.
:::
