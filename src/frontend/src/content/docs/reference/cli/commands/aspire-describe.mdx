...
- **`-f, --follow`**
  ...
  Continuously stream resource state changes. In table mode, each update prints a line showing the resource name, state, health, and endpoints. In JSON mode, each update emits a single JSON object per line (NDJSON format).
...
- **`--format <Json|Table>`**
  ...
  In snapshot mode (without `--follow`), JSON output wraps resources in a `{ "resources": [...] }` object. In follow mode, JSON output uses NDJSON (one JSON object per line) for streaming.
...
- Stream resource changes as NDJSON for scripting:
