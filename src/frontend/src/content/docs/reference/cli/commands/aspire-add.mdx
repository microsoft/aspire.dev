...
- **`integration`**
  ...
  The value must exactly match either the friendly name or the full package ID (for example, `redis` or `Aspire.Hosting.Redis`). Partial or misspelled names are handled differently depending on the mode:
