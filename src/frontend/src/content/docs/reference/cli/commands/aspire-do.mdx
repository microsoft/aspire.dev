...
- `aspire do [step] --list-steps` — a quick, compact listing of every pipeline step with its direct dependencies and tags. Available on `aspire deploy`, `aspire publish`, `aspire destroy`, and `aspire do`. Useful for a fast "what would happen?" view before running a command. Unlike normal `aspire do` execution, the `step` argument is optional with `--list-steps`; omit it to list every step in the pipeline.
...
Running `aspire do [step] --list-steps` produces a numbered list of every step in the pipeline, what each one depends on, and any tags it carries. Pass a `step` to scope the listing to that step and its dependencies, or omit it to list every step in the pipeline:
...
Add `--format json` to get the same listing as machine-readable JSON instead of the tree view, for example to pipe into `jq`:

```bash title="Aspire CLI"
aspire do deploy --list-steps --format json
```

`--format` is only valid together with `--list-steps`; using it without `--list-steps` is an error.
...
- Quickly list the steps in your pipeline for a specific step:
  ...
  This displays a compact view of every step with its dependencies and tags. Use it for a fast "what would happen?" check before running a pipeline command. Omit the `step` argument (`aspire do --list-steps`) to list every step in the pipeline instead of scoping to one step.

- List pipeline steps as JSON for scripting:

  ```bash title="Aspire CLI"
  aspire do deploy --list-steps --format json
  ```

  Emits the same step listing as machine-readable JSON instead of the tree view. `--format` is only valid together with `--list-steps`.
