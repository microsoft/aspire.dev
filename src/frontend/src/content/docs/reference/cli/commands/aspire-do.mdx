...
Before executing a pipeline step, you can discover what steps are available in your application's pipeline and understand their dependencies using the `--list-steps` option.

### Understanding pipeline structure

Running `aspire do --list-steps` (or the same option on `aspire deploy`, `aspire publish`, and `aspire destroy`) provides comprehensive information about your pipeline, including:
