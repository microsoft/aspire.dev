...
Before running the destroy pipeline, `aspire destroy` displays contextual information about the deployed resources or deployment state that the selected compute environment plans to remove, unless `--yes` is specified. After the pipeline finishes, the command prints a result summary and a step-by-step breakdown. The exact step names depend on the selected compute environment.

```text title="Output"
------------------------------------------------------------
✅ 3/3 steps succeeded • Total time: 12.34s

Steps Summary:
                Step timeline:                 0s                       12.34s
                                               │───────┬──────┬─────┬───────│
      2.10ms  ✓ validate-compute-environments  │╴                           │
       12.34s ✓ pipeline-execution             │╶──────────────────────────╴│
       12.30s ✓   destroy                      │╶──────────────────────────╴│
