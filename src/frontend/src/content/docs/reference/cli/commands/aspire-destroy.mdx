...
When running `aspire destroy` in a non-interactive context (for example, in CI/CD pipelines or automation scripts), you must pass **both** `--non-interactive` and `--yes` together. Omitting `--yes` when using `--non-interactive` is an error:
...
- **`-o, --output-path`**
  ...
  The optional path to the deployment artifacts to destroy. Defaults to the `aspire-output` folder relative to the AppHost.
