...
- **`--search <query>`**
  ...
  Search and filter telemetry fields such as trace names, attribute values, source, and IDs. Supports full-text search and field filters. For more details on supported search syntax, see [aka.ms/aspire/cli-search](https://aka.ms/aspire/cli-search).
