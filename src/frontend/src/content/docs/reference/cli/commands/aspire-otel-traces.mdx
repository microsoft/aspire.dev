...
- **`--search <search>`**
  ...
  Full-text search across trace and span text fields, such as names, attribute values, source, IDs, and timestamps. Supports plain free-text fragments and structured `field:value` qualifiers. For more information, see [Search and filter](/reference/cli/search-filter/).
