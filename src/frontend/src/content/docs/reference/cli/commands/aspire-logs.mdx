...
- **`--search <search>`**
  ...
  Full-text search to filter log content. For more details on supported search syntax and field filters, see the [CLI search documentation](https://aka.ms/aspire/cli-search).
...
- Search logs for a specific term:
  ...
  ```bash title="Aspire CLI"
  aspire logs --search "connection refused"
  ```
