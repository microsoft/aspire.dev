...
When one or more skills are installed, the command prints a single compact summary instead of a separate message per skill. For example:
...
If a skill install fails, the error message includes the exact skill path that failed. No-op runs (where every skill was already up to date) stay quiet and print only the final completion message.
