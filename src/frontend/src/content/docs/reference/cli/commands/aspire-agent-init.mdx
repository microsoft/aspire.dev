...
2. **Select skills and tools** — choose which Aspire workflow skills and companion tools to install. The Aspire skills bundle includes `aspire`, `aspire-init`, `aspire-orchestration`, `aspire-monitoring`, `aspire-deployment`, and `aspireify`. The command pre-selects the bundle skills that apply to the current flow; standalone `aspire agent init` keeps `aspireify` opt-in because it's a one-time AppHost wiring skill. Optional companion tools such as `playwright-cli` and `dotnet-inspect` can be selected explicitly.
...
Aspire workflow skills are distributed through the [`microsoft/aspire-skills`](https://github.com/microsoft/aspire-skills) GitHub release assets. Aspire 13.4 includes an embedded snapshot for default installs, and the optional remote fetch path can download matching release assets. The CLI verifies release asset provenance, validates manifest file hashes, and rejects unsafe paths before installing selected files. The `playwright-cli` and `dotnet-inspect` companion options aren't part of that bundle and are installed separately when selected.
</Aside>
