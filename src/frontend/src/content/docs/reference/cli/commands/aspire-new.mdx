...
#### Polyglot AppHosts and \*.dev.localhost URLs

When you scaffold a polyglot AppHost (for example, a TypeScript AppHost using the Express/React template) and choose **Yes**, the `ASPIRE_RESOURCE_SERVICE_ENDPOINT_URL` in `aspire.config.json` is set to a `*.dev.localhost` subdomain (for example, `https://myappname.dev.localhost`). The Aspire runtime recognizes `*.localhost` subdomains as local endpoints — per [RFC 6761](https://www.rfc-editor.org/rfc/rfc6761) — and routes the resource service connection through `ListenLocalhost`, so no extra network access is opened.

If you previously received the error **"ASPIRE_RESOURCE_SERVICE_ENDPOINT_URL must contain a local loopback address"** when running `aspire run` with a polyglot AppHost that uses `*.dev.localhost` URLs, update to Aspire 13.5 or later, which includes this fix.
