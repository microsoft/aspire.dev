...
- **`--channel`**
  ...
  The channel to use for templates. Available channels include `stable` (default) and `daily`. When the staging channel feature is enabled, a `staging` channel is also available.
