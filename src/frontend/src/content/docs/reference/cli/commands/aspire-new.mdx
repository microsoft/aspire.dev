...
- **`--channel`**
  ...
  For non-stable channels (`staging`, `daily`), `aspire new` creates a project-level `nuget.config` with source mappings for the channel's package feed. For the `stable` channel (including the default for stable CLI builds), no `nuget.config` is created because nuget.org is already the default NuGet source.
