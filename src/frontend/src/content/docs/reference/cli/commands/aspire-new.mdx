...
<Aside type="note">
  The `aspire new` command currently requires an interactive terminal even when all options are provided. The `--non-interactive` flag is not fully supported. In CI/CD pipelines or other non-interactive environments, use `dotnet new` with Aspire templates as an alternative for project creation.
</Aside>
