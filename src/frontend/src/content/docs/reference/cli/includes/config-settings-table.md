...
| `overrideStagingFeed`                     | `overrideStagingFeed`                     | Explicit staging package feed URL. On `daily`, `local`, and `pr-<N>` CLI builds, `--channel staging` refuses to synthesize a staging channel unless this is set (or the `StagingChannelEnabled` feature flag is on), since those builds have no reliable feed to derive genuine staging packages from. |
