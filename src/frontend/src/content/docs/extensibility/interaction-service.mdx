...
| `PromptTerminalAsync`     | Borrows a caller-owned terminal for a dialog while optional work runs.                              | Dashboard only     |
...
<Aside type="caution">
  ...
  `PromptConfirmationAsync`, `PromptProgressAsync`, and `PromptTerminalAsync`) will throw an exception
...
`PromptTerminalAsync(message, terminal, options, cancellationToken)` displays an existing terminal in a dashboard dialog. Use it for interactive tools, authentication flows, or automation that a user should be able to watch and cancel. To create a terminal, resolve [`TerminalService`](/app-host/terminal-service/) from the AppHost's dependency injection container.

:::caution[Experimental API]
`PromptTerminalAsync`, `TerminalInteractionOptions`, `TerminalContext`, and the terminal authoring APIs emit diagnostic `ASPIRETERMINAL001`. The examples acknowledge it explicitly. This API is part of [microsoft/aspire#19887](https://github.com/microsoft/aspire/pull/19887), which is not yet merged.
:::

:::note[Supported contexts]
Terminal interactions are dashboard-only, not CLI publish/deploy prompts. Check `IInteractionService.IsAvailable` before prompting. Creating AppHost terminals and calling `PromptTerminalAsync` aren't currently exported to TypeScript AppHosts; this limitation doesn't apply to the other interaction operations demonstrated above.
:::
...
This example adapts the upstream [terminal interaction commands](https://github.com/microsoft/aspire/blob/3e16358e0ae00e1a3846994304436be554837282/playground/Terminals/Terminals.AppHost/TerminalInteractionCommands.cs). It prompts for an upper limit, launches an ordinary console game, and solves it by binary search while the dashboard displays every guess.

To reproduce it:

1. Use a C# AppHost with the terminal APIs from the PR, Docker, and .NET 10 or later for file-based apps.
2. Copy the upstream [`numberguess.cs`](https://github.com/microsoft/aspire/blob/3e16358e0ae00e1a3846994304436be554837282/playground/Terminals/Terminals.AppHost/Scripts/numberguess.cs) into `Scripts/numberguess.cs` under your AppHost directory. Keep its prompt and reply formats unchanged. It intentionally waits for another input after the winning guess so the terminal stays alive for the final screen read.
...
Add this local function to your AppHost file after the top-level app setup, with the `using` directive and diagnostic suppression at the top. Call it from an async callback with both services and the callback's cancellation token. Each completed `Work` closes only that prompt. The shell stops only when the helper leaves its `await using` scope. A previously ended process can't be revived by prompting with its handle.
...
- **Terminal dialogs**: Caller-owned terminals appear while optional work runs, with an optional cancel button.
...
- **Limited functionality**: Message boxes, notifications, confirmation dialogs, progress dialogs, and terminal dialogs aren't available and throw exceptions if called.
...
<Aside type="caution">
  ...
  `PromptNotificationAsync`, `PromptConfirmationAsync`, `PromptProgressAsync`, or `PromptTerminalAsync`
...
- [AppHost terminal service](/app-host/terminal-service/) — Create, own, and automate terminals
- [Dashboard terminals](/dashboard/terminals/) — Use terminal views and the terminal dock
