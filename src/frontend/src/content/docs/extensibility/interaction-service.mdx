...
import terminalInteractionDark from '@assets/dashboard/terminals/terminal-interaction-dark.png';
import terminalInteractionLight from '@assets/dashboard/terminals/terminal-interaction-light.png';
...
<ThemeImage
  dark={messageDialog} light={messageDialogLight}
...
<ThemeImage
  dark={messageBar} light={messageBarLight}
...
<ThemeImage
  dark={confirmation} light={confirmationLight}
...
<ThemeImage
  dark={multipleInput} light={multipleInputLight}
...
<ThemeImage
  dark={multipleInputFilled} light={multipleInputFilledLight}
...
<ThemeImage
  dark={multipleInputLogs} light={multipleInputLogsLight}
...
An AppHost can use the experimental `TerminalService` API with `PromptTerminalAsync` to present an interactive setup tool in a dashboard dialog. Unlike a form prompt, the dialog displays the running terminal program and accepts its input. The following example shows a local configuration workflow after it saves the selected settings:
...
These APIs use the `ASPIRETERMINAL001` diagnostic. The caller owns the terminal and is responsible for its lifetime; closing a viewer isn't a substitute for disposing it. See [AppHost terminals in Aspire 13.6](/whats-new/aspire-13-6/#apphost-terminals-and-automation) and [resource terminals](/app-host/with-terminal/) for the distinction between AppHost-owned and resource-owned sessions.
