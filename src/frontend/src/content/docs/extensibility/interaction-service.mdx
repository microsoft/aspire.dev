...
The `InteractionInput.Files` property is obsolete in favor of `GetFiles()` because it doesn't provide ownership of the uploaded files.
...
- **`AllowMultipleFiles`**: When `true`, the user can select more than one file. Each client submits one file selection per input during an interaction, and multi-file selections upload sequentially as part of that selection. An input accepts at most 100 files when `AllowMultipleFiles` is `true`, and at most one file otherwise. Uploads that exceed the limit are rejected, and in-progress uploads count toward it so concurrent requests can't exceed it.
