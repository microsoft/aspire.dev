...
:::note
The `Description` property on `[AspireExport]` is still supported as a compatibility fallback, but new exports should use XML doc comments. The analyzer reports `ASPIREEXPORT015` when `Description` is set on a new export.
:::
...
| ASPIREEXPORT015 | Warning  | `[AspireExport(Description = ...)]` is deprecated — use XML doc comments instead            |
