...
description: Export Aspire hosting integrations through the Aspire Type System, then validate their generated TypeScript, Python, Go, Java, and Rust SDK surfaces.
---

import { Aside, Steps } from '@astrojs/starlight/components';
import AppHostTabs from '@components/AppHostTabs.astro';

Aspire hosting integrations are C# libraries that extend the AppHost with new resource types. C# AppHosts consume those APIs directly. TypeScript, Python, Go, Java, and Rust AppHosts consume a language-specific SDK generated from **Aspire Type System (ATS)** metadata.

This guide shows how to design one language-neutral ATS contract, preserve an idiomatic C# API, and validate every generated SDK that your integration intends to support.

## How it works

When a guest-language AppHost adds your integration, the Aspire CLI:

<Steps>

1. Loads your integration assembly
2. Scans for ATS attributes on methods, types, and properties, such as `[AspireExport]`.
3. Generates a typed SDK for the selected AppHost language
4. The generated SDK communicates with your C# code via JSON-RPC at runtime

</Steps>

Your C# code runs as-is in the orchestration host. The generated SDK is a thin guest-language client that calls into it. You don't rewrite the integration in every language, but you must validate that the exported contract maps cleanly into each supported generator.
...
```xml title="MyIntegration.csproj"
<PropertyGroup>
...
Annotate your extension methods with `[AspireExport]` and use XML doc comments to document them for generated SDKs:

```csharp title="MyDatabaseBuilderExtensions.cs"
...
The current TypeScript generator produces the following highlighted API. Treat it as one projection of the ATS contract, not as the contract itself.

<AppHostTabs limitations={{
  csharp: 'C# AppHosts call the hosting integration directly and do not use an ATS-generated guest SDK.',
  python: 'Generate and inspect the Python SDK before documenting the call shape. This sample integration has not been validated against the Python generator.',
  go: 'Generate and inspect the Go SDK before documenting the call shape. Go fluent resource calls can accumulate errors that must be checked with Err().',
  java: 'Generate and inspect the Java SDK before documenting the call shape. This sample integration has not been validated against the Java generator.',
  rust: 'Generate and inspect the Rust SDK before documenting the call shape. Rust SDK validation currently has known non-blocking failures.',
}}>
<Fragment slot="typescript">

```typescript title="Generated TypeScript SDK" {5-8}
...
XML doc comments are the primary source for generated SDK API documentation. The ATS scanner reads `<summary>`, `<param>`, `<returns>`, and `<remarks>` tags and includes them in the generated documentation format for each language.

Use `ats-*` override tags when the standard C# XML documentation doesn't translate well to generated SDK docs — for example, when a `<summary>` references C#-specific types or language constructs that don't exist in guest SDKs. The supported overrides are:
...
```csharp title="Polyglot-incompatible summary with ats-summary override"
...
```csharp title="Cross-references using ats-see"
/// <summary>
...
Generators render these references in their language-specific documentation format. TypeScript uses JSDoc `{@link ...}` links.
...
```csharp title="Duplicate capability ID"
...
```csharp title="Unique capability IDs with SDK method names"
...
4. Validate the generated TypeScript, Python, Go, Java, and Rust SDKs that your integration claims to support.
...
| Test with the analyzer and guest AppHost project references, then inspect every generated language surface.                     | Assume a C# API shape is polyglot-safe because one generated SDK compiles.                                                   |

### Design ATS-first, not C#-first

Start from the ATS contract you want generated SDKs to expose, then map that contract back to C#. TypeScript is useful for illustrating promise-based behavior, but it is not a substitute for the other generators. Prefer one clear ATS-visible operation over a set of C# overloads that need different generated names. Use DTO options bags, `[AspireUnion]`, and small exported context or editor types to represent the cross-language contract.
...
```csharp title="Unique capability IDs with friendly SDK names"
...
```typescript title="Friendly generated method names"
...
```csharp title="One exported shape for one concept"
...
```typescript title="One generated method"
...
```csharp title="Options object for overload families"
[AspireDto]
...
```typescript title="Flat options object"
...
```csharp title="Distinct concepts on one target"
[AspireExport]
...
```typescript title="Distinct generated methods"
...
```csharp title="Explicit callback context exports"
[AspireExport]
...
```csharp title="Avoid broad expansion on large contexts"
...
```typescript title="Passing ATS values"
...
```csharp title="Collection-valued DTO input"
[AspireDto]
...
```typescript title="Plain JSON DTO input"
...
```csharp title="Inline callback invocation"
...
```csharp title="Language-neutral export docs"
[AspireDto]
...
Use the analyzer and a test AppHost for every guest language your integration claims to support. After `aspire restore` or `aspire run` generates modules, inspect method names, DTO shapes, callback accessors, documentation, and error behavior.

| AppHost    | Generated surface to inspect                               | Important convention                                                                      |
| ---------- | ---------------------------------------------------------- | ----------------------------------------------------------------------------------------- |
| TypeScript | `.aspire/modules/*.mts` and generated type signatures      | APIs are promise-heavy; await builder, fluent, property-accessor, and callback operations |
| Python     | `.aspire/modules/aspire_app.py`                            | APIs use `snake_case`; runtime failures surface as exceptions                             |
| Go         | `.aspire/modules/aspire.go`, `base.go`, and `transport.go` | exported APIs use `PascalCase`; fluent builders accumulate errors for `Err()`             |
| Java       | `.aspire/modules/aspire/*.java`                            | APIs use `camelCase`; runtime failures surface as exceptions                              |
| Rust       | `.aspire/modules/aspire.rs`, `base.rs`, and `transport.rs` | APIs use `snake_case` and return `Result` values                                          |
...
```csharp title="MyDatabaseResource.cs"
...
```csharp title="Mixed property kinds"
[AspireExport(ExposeProperties = true)]
...
```typescript title="Generated interface"
...
```typescript title="Consuming the generated API"
...
```csharp title="EnvironmentEditor.cs"
/// <summary>
...
```csharp title="MyCallbackContext.cs"
[AspireExport]
public sealed class MyCallbackContext(
...
```csharp title="MyResourceBuilderExtensions.cs"
...
```typescript title="Consuming the callback API"
...
```typescript title="Using a callback service provider"
...
```typescript title="Inspecting AppHost resources"
...
```typescript title="Writing logs from a callback"
await builder.subscribeBeforeStart(async (event) => {
  const services = await event.services();
...
```typescript title="Waiting for resource state"
...
```typescript title="Using AppHost services"
await builder.subscribeBeforeStart(async (event) => {
  const services = await event.services();
...
```typescript title="Using eventing from services"
...
```typescript title="Executing a command from a callback"
...
```csharp title="MyDatabaseOptions.cs"
[AspireDto]
...
```csharp title="Override exported name"
...
```csharp title="Exclude incompatible overloads"
...
```csharp title="Union type parameter"
...
You can test your integration locally without publishing to a NuGet feed. In a guest-language AppHost's `aspire.config.json`, set the package value to a `.csproj` path instead of a version number:
...
When the CLI detects a `.csproj` path, it builds the project locally and generates the selected language SDK from the resulting assemblies. Change the `appHost.path` and `appHost.language` values for each test AppHost:

| AppHost    | `path`         | `language`          |
| ---------- | -------------- | ------------------- |
| TypeScript | `apphost.mts`  | `typescript/nodejs` |
| Python     | `apphost.py`   | `python`            |
| Go         | `apphost.go`   | `go`                |
| Java       | `AppHost.java` | `java`              |
| Rust       | `apphost.rs`   | `rust`              |
...
1. Create one test AppHost per generated SDK:

   ```bash title="Create TypeScript test AppHost"
   mkdir test-typescript && cd test-typescript
   aspire init --language typescript
   ```

   Enable the exact feature flag before initializing an experimental language:

   ```bash title="Enable experimental AppHost languages"
   ...
   Then create each AppHost in a separate directory with `aspire init --language python`, `go`, `java`, or `rust`. Python requires Python 3.11 or later, the current Go scaffold declares `go 1.26`, Java requires JDK 25, and Rust requires Cargo.

2. Add your integration via project reference in each `aspire.config.json`:

   ```json title="aspire.config.json (packages section)"
   {
     "packages": {
       "MyCompany.Hosting.MyDatabase": "../src/MyCompany.Hosting.MyDatabase/MyCompany.Hosting.MyDatabase.csproj"
     }
...
3. Run `aspire restore` to generate the SDK without starting the application:

   ```bash title="Generate SDK"
   aspire restore
   ```

4. Inspect `.aspire/modules/` and verify that exports, names, options, callbacks, docs, and error shapes are idiomatic for that language.

5. Write a minimal AppHost scenario with the generated API, run it with `aspire run`, and assert that the resource reaches its expected state.

</Steps>

:::tip[Troubleshoot generated AppHosts]
If generated behavior differs from the C# API, inspect the selected language's files under `.aspire/modules` first. Verify that the integration package was restored, the export is analyzer-clean, and the generated module contains the method. Then use `aspire doctor` for version and environment mismatches. TypeScript AppHosts may also need `aspire certs trust` when HTTPS certificate trust errors block local execution.
:::

:::caution[Rust validation status]
The upstream polyglot workflow currently runs the Rust Redis scenario with `continue-on-error`, and Rust is excluded from the required-pass condition because known SDK issues remain. Test Rust exports, but don't claim them as supported until the scenario passes reliably.
:::
...
- [Build your first app](/get-started/first-app/) — Compare the six AppHost language workflows
