...
The generator supplies `GenerateAspireProvisioningProxyAttribute` in the `Aspire.Hosting.Azure.Provisioning` namespace. Apply it at assembly level with the selected SDK root type, such as `KeyVaultService`. Set `IncludeContainingAssemblyTypes = true` to include compatible public classes and readonly structs from that root's assembly. This keeps the projection bounded to the selected SDK rather than every referenced assembly.
