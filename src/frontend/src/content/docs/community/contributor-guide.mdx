...
[Dev Hub](/dev/) highlights live in `src/data/dev-central.ts`, relative to `src/frontend`. Reuse existing documentation, sample IDs, and shared data rather than maintaining a separate resource catalog.
...
To refresh the official blog archive used by Browse, run `pnpm update:blogs` from `src/frontend`. The Hub's featured posts remain curated separately in `src/data/dev-central.ts`.
