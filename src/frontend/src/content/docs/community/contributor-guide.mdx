...
### AppHost-specific language content

When AppHost-specific content changes by language, use `AppHostTabs`. Its language order, enabled languages, Experimental treatment, and shared selection state come from `src/data/apphost-languages.json`. Provide a named slot for every supported language. When an enabled language cannot express the API, add an explicit `limitations` entry instead of showing another language's code.
...
Use `AppHostTabs` for AppHost content that switches by language. Do not hard-code language order or use bare `Tabs` with `syncKey='aspire-lang'`; the shared component reads the site-wide language registry and keeps the reader's choice synchronized across the page.

Keep shared section headings outside the tabs. For example, write `## Add a Redis resource` before `AppHostTabs`, then place each language-specific implementation in its named `Fragment` slot. This helps the **On this page** navigation stay accurate.
