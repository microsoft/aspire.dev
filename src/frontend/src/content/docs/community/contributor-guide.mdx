...
Keep shared section headings outside the pivots. For example, write `## Add a Redis resource` before the selector and the `Pivot` blocks, then place the language-specific prose and code inside the `csharp` and `typescript` pivots. This helps the **On this page** navigation stay accurate.
