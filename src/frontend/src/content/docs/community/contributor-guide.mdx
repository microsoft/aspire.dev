...
When you are documenting AppHost-specific content that changes between C# and TypeScript, use `AppHostLangPivot` instead of `Tabs syncKey="apphost-lang"`. The component is controlled by the site-wide AppHost selector in the sidebar, so it keeps AppHost walkthroughs consistent across pages.

Use `Tabs` for other choices such as package managers, deployment targets, IDEs, or CLI variants.

If a feature is only available in the C# AppHost today, show the C# example by itself and add an explanatory note. Do not wrap a single-language example in `AppHostLangPivot`.

For the **On this page** table of contents to pick up a heading reliably, define the heading outside `Pivot`, `Tabs`, and `AppHostLangPivot`. Headings inside those components are often missed or can produce incomplete results in the generated table of contents.
...
### AppHostLangPivot component

Use `AppHostLangPivot` when the surrounding AppHost content should switch between C# and TypeScript as a single unit.

Keep shared section headings outside the pivot. For example, write `## Add a Redis resource` before the `AppHostLangPivot`, then place the language-specific prose and code inside the `csharp` and `typescript` slots. This helps the **On this page** navigation stay accurate.
...
Use this pattern when entire sections differ by AppHost language, not just a single code fence. For example, the slots can contain explanatory prose, steps, asides, and code blocks together.
