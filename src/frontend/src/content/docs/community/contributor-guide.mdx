...
deployments. No end-to-end speedup or full-site equivalence has been established
yet; use the qualification workflow before proposing promotion.
...
Build identity is finalized after rendering, including restored HTML. The
...
records it with the measurements. Incremental cache partitions include this
optional input. No output timestamps or search-index files are normalized away.
