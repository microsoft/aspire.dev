...
Endpoint resolution happens during the startup sequence of the `DistributedApplication`. To safely access endpoint values (e.g., `Url`, `Host`, `Port`), you must wait until endpoints are allocated. You can use `await endpoint.GetValueAsync()` to asynchronously retrieve the allocated endpoint value.

```csharp title="Example — Accessing Endpoint Values"
var builder = DistributedApplication.CreateBuilder(args);
...
// Access the endpoint value asynchronously after allocation
var allocatedEndpoint = await endpoint.GetValueAsync();
Console.WriteLine($"Resolved Url: {allocatedEndpoint.Url}");
Console.WriteLine($"Resolved Host: {allocatedEndpoint.Host}");
Console.WriteLine($"Resolved Port: {allocatedEndpoint.Port}");
...
IsAllocated: False
Resolved Url: {redis.bindings.tcp.url}
Resolved Host: {redis.bindings.tcp.host}
Resolved Port: {redis.bindings.tcp.port}
```
