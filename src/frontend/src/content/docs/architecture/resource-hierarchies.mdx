...
Endpoint resolution happens during the startup sequence of the `DistributedApplication`. To safely access endpoint values (e.g., `Url`, `Host`, `Port`), you must wait until endpoints are allocated. You can access the `Url`, `Host`, and `Port` properties directly on the `EndpointReference` once the endpoint has been allocated.
...
// Access endpoint properties directly (available after allocation in run mode)
Console.WriteLine($"Resolved Url: {endpoint.Url}");
Console.WriteLine($"Resolved Host: {endpoint.Host}");
Console.WriteLine($"Resolved Port: {endpoint.Port}");
