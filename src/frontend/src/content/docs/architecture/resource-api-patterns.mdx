...
```csharp
// Resource-level convenience methods (preferred)
resourceBuilder.OnConnectionStringAvailable((resource, @event, ct) => { /* ... */ });
resourceBuilder.OnResourceReady((resource, @event, ct) => { /* ... */ });

// Or via builder.Eventing.Subscribe for events without convenience methods
...
| Optional wiring | `.WithEndpoint(…)`, `.WithHealthCheck(…)`, `.WithImage(…)`, `.WithEnvironment(…)`, `.WithArgs(…)`, `.OnConnectionStringAvailable(…)`, `.OnResourceReady(…)` | Configure container details, wiring, and runtime hooks |
