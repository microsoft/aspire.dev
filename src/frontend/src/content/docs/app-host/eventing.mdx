...
<Aside type="note">
  TypeScript AppHost support for the lower-level
  `IDistributedApplicationEventing` subscription API isn't yet available.
</Aside>
...
<Aside type="note">
  TypeScript AppHosts support resource callbacks such as `onInitializeResource`,
  `onBeforeResourceStarted`, `onResourceReady`, and `onResourceStopped`.
  TypeScript support for the `ResourceEndpointsAllocatedEvent` and
  `ConnectionStringAvailableEvent` callbacks shown in this C# example isn't yet
  available.
</Aside>
...
<Aside type="note">
  TypeScript AppHost support for `OnBeforePublish` and `OnAfterPublish` is not
  yet available.
</Aside>
...
await cache.onResourceStopped(async (event) => {
  console.log(`Resource ${event.resource.name} stopped`);
});
```
