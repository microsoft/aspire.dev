...
<Aside type="caution">
  ...
  Subscribing to an interface or abstract class, such as
  `IDistributedApplicationEvent`, throws an `ArgumentException` because
  the internal lookup key is `typeof(T)` while `PublishAsync<T>` is always
  invoked with the concrete event type — so an interface or abstract
  subscription could never fire. Subscribe to the concrete event type
  instead, for example `BeforeStartEvent` or `AfterResourcesCreatedEvent`.

  ```csharp
  // Throws ArgumentException: Cannot subscribe to interface or abstract
  // type 'IDistributedApplicationEvent'. Subscribe to a concrete event
  // type instead.
