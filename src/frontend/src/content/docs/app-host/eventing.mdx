...
<Aside type="caution">
  ...
  `IDistributedApplicationEvent`, throws an `ArgumentException` with the
  message `Cannot subscribe to interface or abstract type
  'IDistributedApplicationEvent'. Subscribe to a concrete event type instead.`
  Subscribe to a concrete type such as `BeforeStartEvent` or
  `AfterResourcesCreatedEvent`.

```csharp title="AppHost.cs"
builder.Eventing.Subscribe<IDistributedApplicationEvent>(
    static (@event, cancellationToken) => Task.CompletedTask);
```
