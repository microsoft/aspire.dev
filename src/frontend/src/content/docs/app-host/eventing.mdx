...
<Aside type="note">
  `ResourceStoppedEvent` fires once per stop. If a handler triggers a restart
  of the resource, the restart doesn't cause the event to fire again for the
  same stop — you'll see it again only on a subsequent, distinct stop.
</Aside>
