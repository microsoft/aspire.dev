...
The `ResourceStoppedEvent` is raised once each time a resource stops execution.
If a handler restarts the resource, the restart doesn't raise another event for
the same stop. The event is raised again only when the resource subsequently
stops:
