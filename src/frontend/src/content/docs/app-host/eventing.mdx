...
The following builder-level methods are available for AppHost events in TypeScript:

| Method | Event |
|--------|-------|
| `subscribeBeforeStart` | `BeforeStartEvent` — raised before the AppHost starts |
| `subscribeAfterResourcesCreated` | `AfterResourcesCreatedEvent` — raised after the AppHost creates resources |
| `subscribeBeforePublish` | `BeforePublishEvent` — raised before manifest publishing begins |
| `subscribeAfterPublish` | `AfterPublishEvent` — raised after manifest publishing completes |
</TabItem>
</Tabs>
