...
<Aside type="caution">
Interactive console apps that read from stdin (e.g., `Console.ReadLine()`) are not fully supported in the Aspire dashboard's **Console logs** view. The Aspire dashboard captures and displays stdout/stderr, but the console logs page does not forward user input to the process's stdin. For interactive terminal user interfaces (TUIs) or shell-based experiences that need a live, bidirectional session, use [`WithTerminal`](/app-host/with-terminal/) to expose an interactive terminal you can attach to from the dashboard or the `aspire terminal` CLI.
</Aside>

<LearnMore>
  For more information, see [Test TUI and shell apps with WithTerminal](/app-host/with-terminal/).
</LearnMore>
