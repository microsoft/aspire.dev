...
Any .NET project — including console applications and Worker Service projects — can be added to the AppHost using `AddProject`. The project doesn't need to be a web app. For non-.NET apps (Python, Node.js, or arbitrary executables), use `AddExecutable`, `AddPythonApp`, or a JavaScript app resource instead.
...
// Add a console app project to the AppHost
await builder.addProject("processor", "../MyConsoleApp/MyConsoleApp.csproj")
    .withReference(cache);
...
<Aside type="note">
The console app project should target a .NET version supported by Aspire (.NET 8 or later) and reference the service defaults project to benefit from telemetry and service discovery. For more information, see [Service defaults](/get-started/csharp-service-defaults/).
</Aside>
...
3. In the AppHost, reference the services this app depends on:
    ...
    const processor = await builder.addProject("processor", "../MyConsoleApp/MyConsoleApp.csproj");
...
const worker = await builder.addProject("data-worker", "../DataWorker/DataWorker.csproj");
...
<LearnMore>
  For more information on the Redis and PostgreSQL hosting integrations, see [Redis integration](/integrations/caching/redis/redis-get-started/) and [PostgreSQL integration](/integrations/databases/postgres/postgres-get-started/).
</LearnMore>
...
const tool = await builder.addProject("tool", "../MyInteractiveTool/MyInteractiveTool.csproj");
...
<Aside type="caution">
Interactive console apps that read from stdin (for example, `Console.ReadLine()`) aren't fully supported in the Aspire dashboard's **Console logs** view. The dashboard captures and displays stdout/stderr, but the console logs page doesn't forward user input to the process's stdin.
</Aside>

For interactive terminal user interfaces (TUIs) or shell-based experiences that need a live, bidirectional session, Aspire provides an experimental `WithTerminal` API that exposes a pseudo-terminal you can attach to from the dashboard or with the `aspire terminal` CLI command.

<Aside type="caution" title="Experimental API">
`WithTerminal` is experimental and reported under diagnostic ID `ASPIRETERMINAL001`. Its API surface and behavior may change in future releases. Using it in C# requires suppressing the corresponding diagnostic.
</Aside>
