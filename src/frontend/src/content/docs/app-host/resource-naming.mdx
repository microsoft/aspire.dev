...
In your .NET application, the configuration system maps double underscores (`__`) to the colon (`:`) separator used by `IConfiguration`. This means `ConnectionStrings__app_db` is accessible as `ConnectionStrings:app-db` in the configuration API:
...
<Aside type="tip">
When using the typed configuration pattern (e.g., `AddNpgsqlDataSource("app-db")`), pass the original resource name with dashes — Aspire integrations handle the underscore mapping internally.
</Aside>
...
- **Use dashes as separators**: Dashes (`-`) are the conventional separator for multi-word resource names in Aspire, as they are commonly used in DNS names and container networking.
...
- **Avoid underscores in names**: While underscores work, they can cause confusion because dashes are transformed to underscores in environment variables.
