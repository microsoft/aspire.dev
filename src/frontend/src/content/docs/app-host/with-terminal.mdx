...
<Aside type="note">
Because a terminal-backed executable bridges its standard output and error through the pseudo-terminal (PTY) instead of separate log streams, the **Console logs** view and [`aspire logs`](/reference/cli/commands/aspire-logs/) only show that resource's startup and system logs (for example, lifecycle and DCP diagnostics)—not the interleaved stdout/stderr you see in the **Terminal** view. Use the **Terminal** view or `aspire terminal attach <resource>` to see the process's own output.
</Aside>
