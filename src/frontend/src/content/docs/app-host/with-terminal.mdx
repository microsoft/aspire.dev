...
When you apply `WithTerminal`, Aspire runs the resource as a plain process and **does not automatically attach the debugger**. This is a deliberate trade-off: the orchestrator (DCP) cannot currently run a process under the debugger and a pseudo-terminal (PTY) at the same time, so Aspire prefers a working interactive terminal over automatic IDE execution.

If you need to debug the resource, attach the debugger manually to the running process from your IDE.
