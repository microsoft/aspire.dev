...
import terminalDetachedDark from '@assets/dashboard/terminals/terminal-detached-dark.png';
import terminalDetachedLight from '@assets/dashboard/terminals/terminal-detached-light.png';
...
<ThemeImage
  ...
  alt="Aspire dashboard resource terminal running local shell commands and displaying catalog data."
/>
...
### Open a terminal in a separate window

Use **Open in new window** to keep a terminal visible while you inspect other dashboard pages. AppHost-owned terminal dock tabs can also open in a separate window, as shown below. Their detached view contains the terminal and its controls rather than the dashboard navigation.

<ThemeImage
  dark={terminalDetachedDark}
  light={terminalDetachedLight}
  alt="An AppHost-owned terminal in a separate window, showing completed local catalog checks."
/>

Resource terminals and AppHost-owned dock terminals have different ownership and lifetimes. Closing a viewer doesn't replace disposing an AppHost-owned terminal. For more about docked terminals, see [Explore the Aspire dashboard](/dashboard/explore/#interactive-terminals).
