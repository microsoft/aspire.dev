...
By default, Aspire's internal Developer Control Plane (DCP) server uses an ephemeral certificate it generates itself for TLS. On Windows, you can opt in to using your trusted ASP.NET Core developer certificate for DCP communication instead, which avoids trust issues caused by the ephemeral certificate not being in the system trust store.
...
- Aspire checks for a trusted ASP.NET Core developer certificate.
