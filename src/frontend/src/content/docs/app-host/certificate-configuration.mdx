...
<Aside type="note" title="Firefox certificate trust on Linux">
  ...
  To trust or clean specific NSS databases instead of relying on auto-discovery, set `ASPIRE_CLI_DEV_CERTS_NSSDB_PATHS` to a `:`-separated list of NSS database directories. Prefix an entry with `firefox=` or `chromium=` to force how it's treated; unprefixed entries are classified automatically based on whether the path looks like a Firefox profile.

  ```bash title="Override the NSS databases used for certificate trust"
  ASPIRE_CLI_DEV_CERTS_NSSDB_PATHS="firefox=/path/to/firefox-profile:chromium=/path/to/chromium-nssdb" aspire certs trust
  ```

  When `ASPIRE_CLI_DEV_CERTS_NSSDB_PATHS` is unset or empty, the CLI falls back to the `DOTNET_DEV_CERTS_NSSDB_PATHS` environment variable used by the .NET SDK's `dotnet dev-certs`, so existing overrides continue to work.
</Aside>
