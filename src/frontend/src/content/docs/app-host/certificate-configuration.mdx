...
<Aside type="note" title="Firefox certificate trust on Linux">
  ...
  To trust or clean specific NSS databases instead of relying on auto-discovery, configure `certificates.nssDbPaths` as a `:`-separated list of NSS database directories. Prefix custom Firefox database paths with `firefox=`. Unprefixed paths use Chromium-family trust settings, so they might not work for Firefox-based browsers. You can also prefix Chromium-family paths with `chromium=`.
