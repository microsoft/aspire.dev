...
<Aside type="note" title="Firefox certificate trust on Linux">
  ...
  To trust or clean specific NSS databases instead of relying on auto-discovery, configure `certificates.nssDbPaths` as a `:`-separated list of directories that contain NSS databases. Firefox and Chromium-family browsers require different trust settings; prefix a custom NSS database path with `firefox=` to apply the correct trust settings for Firefox-based browsers or prefix it with `chromium=` to apply Chromium-family trust settings. If no prefix is specified, Chromium-family trust settings are the default.
  ...
  This setting takes precedence over the `DOTNET_DEV_CERTS_NSSDB_PATHS` environment variable used by the .NET SDK's `dotnet dev-certs`. When `certificates.nssDbPaths` is unset or empty, the CLI uses the environment variable instead.
</Aside>
