...
<Aside type="note" title="Linux system trust preservation">
  ...
  development certificate. If `SSL_CERT_DIR` is already set in the resource's
  environment, Aspire appends its certificate directory to the existing value
  instead of replacing it. If `SSL_CERT_DIR` is unset, Aspire appends any
  well-known system certificate directories that exist on the machine (for
  example `/etc/ssl/certs`) alongside its own. An explicit `SSL_CERT_DIR`
  value, including an empty string, is always preserved as user configuration.
  This avoids a scenario where Linux workloads launched with `dotnet run` or
  an IDE lose OpenSSL's implicit system certificate roots and fail outbound
  HTTPS requests once Aspire configures the resource with only its generated
