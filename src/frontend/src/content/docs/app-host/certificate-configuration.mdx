...
The preferred way to manage the development certificate is to use the [Aspire CLI](/get-started/install-cli/). In an interactive session, `aspire run` checks the certificate before starting the AppHost and attempts to create and trust it when needed. On Windows and macOS, the operating system requests consent before trusting a new certificate.

When interactive input isn't available on Windows or macOS, the CLI creates an untrusted certificate if needed, skips the trust step, and displays a warning. This behavior applies to non-interactive CI jobs and commands launched without an interactive terminal in Codespaces or dev containers.

To create or trust the certificate explicitly, run the following command in an interactive terminal:

```bash title="Trust the development certificate"
aspire certs trust
```

If you manage the certificate with the .NET SDK instead, run:

```bash title="Trust with the .NET CLI"
dotnet dev-certs https --trust
```

If trusting the certificate doesn't resolve stale or corrupt certificate state, you can remove the development certificates before creating and trusting a new one.

:::caution[Cleaning development certificates]
`aspire certs clean` removes all HTTPS development certificates for the current user, not only certificates used by Aspire. The `dotnet dev-certs https --clean` command has the same scope. Cleaning can disrupt unrelated ASP.NET Core apps until you create and trust a new certificate.
:::

```bash title="Remove and re-trust the development certificate"
aspire certs clean
aspire certs trust
```

### Linux trust for OpenSSL clients and browsers

On Linux, `aspire certs trust` runs without an operating system consent prompt and can configure two independent trust paths:

- **OpenSSL-aware clients**: The CLI exports the certificate to `~/.aspnet/dev-certs/trust` and adds that path to `SSL_CERT_DIR` for the AppHost process it launches. Shells and apps started separately don't inherit that value, and `SSL_CERT_DIR` doesn't configure browser trust. Add the path to your shell profile (`~/.bashrc`, `~/.zshrc`, `~/.profile`, or equivalent) when other OpenSSL-aware clients need to trust the certificate.
- **Firefox and Chromium-based browsers**: The CLI uses `certutil` to update existing browser NSS databases. Install the `certutil` utility from your distribution's NSS tools package and start each browser once to create its profile before you run `aspire certs trust`. Run the command again after you create another browser profile.
...
Reload your profile or start a new terminal session for the change to take effect.
