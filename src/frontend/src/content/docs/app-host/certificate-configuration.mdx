...
<Aside type="note" title="Firefox certificate trust on Linux">
  ...
  Firefox profiles are discovered under the legacy `~/.mozilla/firefox` directory (used by traditional installs and the Firefox Snap package) or, when no legacy profile exists, under `$XDG_CONFIG_HOME/mozilla/firefox` (or `~/.config/mozilla/firefox` when `XDG_CONFIG_HOME` isn't set), matching modern Firefox's profile layout. Chromium-family databases are discovered under `~/.pki/nssdb` and the Chromium Snap package's `~/snap/chromium/current/.pki/nssdb`.
