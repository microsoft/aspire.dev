...
Aspire 13.3 unified polyglot AppHost environment assignment behind a single `withEnvironment(name, value)` pattern. The `value` argument accepts the same kinds used by the underlying `WithEnvironment` overloads, including:
...
Before 13.3, generated SDKs exposed separate helpers per value kind:

```typescript title="TypeScript — before (older call sites)"
await app.withEnvironmentEndpoint("API_URL", api.getEndpoint("http"));
await app.withEnvironmentParameter("API_KEY", apiKey);
await app.withEnvironmentConnectionString("DB", database);
```

In 13.3 and later, use the unified method:

```typescript title="TypeScript — after (unified API)"
await app.withEnvironment("API_URL", api.getEndpoint("http"));
await app.withEnvironment("API_KEY", apiKey);
await app.withEnvironment("DB", database);
```
...
For migration, replace shimmed helpers with `withEnvironment(name, value)` as you touch existing code.
