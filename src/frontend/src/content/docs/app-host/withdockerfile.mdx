...
The callback receives a `DockerfileBuilderCallbackContext`. In TypeScript, callback context values such as `builder`, `resource`, and `services` are accessed asynchronously as methods, for example `await context.builder()`. The generated Dockerfile can use stages, build arguments, `RUN`, `COPY`, `COPY --from`, `ENV`, `EXPOSE`, `CMD`, `ENTRYPOINT`, and related Dockerfile instructions.
