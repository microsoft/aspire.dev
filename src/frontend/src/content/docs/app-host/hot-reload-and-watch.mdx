...
description: Learn how hot reload works in Aspire and how watch mode rebuilds and restarts resources automatically when project files change during development.
---
...
For supported edits, .NET Hot Reload applies changes to the running project. Blazor projects can also refresh Razor and CSS changes in the browser. Edits that Hot Reload can't apply cause `dotnet watch` to restart the affected project or application. For details, see [Supported code changes (C# and Visual Basic)](https://learn.microsoft.com/visualstudio/debugger/supported-code-changes-csharp).
