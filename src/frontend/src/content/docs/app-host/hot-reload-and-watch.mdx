...
<TabItem label='Project resources'>

Project resources use the watch and hot reload behavior of the project framework.
...
For example, use `dotnet watch` when you want the .NET SDK's hot reload loop for one .NET project:

```bash title=".NET CLI"
dotnet watch --project './src/MyService/MyService.csproj'
```

`dotnet watch` runs the selected project outside Aspire orchestration. If the service depends on Aspire-provided environment variables, connection strings, service discovery, or local resources, provide equivalent configuration for the watched project or run the required dependencies separately.
