...
TypeScript AppHosts support the standard .NET configuration stack. Place `appsettings.json` and `appsettings.{Environment}.json` files next to `apphost.mts`, and Aspire loads them into the AppHost's configuration alongside environment variables and command-line arguments:
...
Read configuration values from `apphost.mts` through the builder's configuration accessor:
...
<Aside type="note">
Configuration is resolved relative to the guest AppHost directory (where `apphost.mts` lives), not the CLI-owned directory that runs the managed Aspire server process. This lets `appsettings.json` sit next to `apphost.mts` the same way it would for a .NET AppHost. The managed server's own logging defaults are preserved, but application settings can still override them.
</Aside>
