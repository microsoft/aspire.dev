...
TypeScript AppHosts support the following package managers:

| Package manager | Minimum version |
|-----------------|-----------------|
| npm             | Any             |
| pnpm            | Any             |
| Yarn            | 4.0 or later    |
...
<Aside type="caution">
Yarn Classic (v1) is **not supported** for TypeScript AppHosts. If the Aspire CLI detects a Yarn Classic lockfile (`yarn.lock` with a `# yarn lockfile v1` header) or a `packageManager` field set to `yarn@1.x` in `package.json`, it fails early with an error message. Upgrade to Yarn 4 or later, or switch to npm, pnpm, or Bun.
</Aside>
