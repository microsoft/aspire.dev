...
TypeScript AppHosts read configuration from JSON files that live next to `apphost.mts`. Add an `appsettings.json` file to your AppHost and Aspire loads it into the AppHost's configuration at startup. You read those values back in `apphost.mts` through the builder's configuration accessor.
...
Define settings as plain JSON. Group related values with nested objects, and name keys however reads best — keys are matched case-insensitively, so camelCase fits naturally in a TypeScript project:
...
Read a value in `apphost.mts` by awaiting `getConfiguration()`, then `getConfigValue()` (returns `null` if the key isn't found). Address a nested key with a colon-separated path:
...
The active environment is passed to the AppHost with the `--environment` switch on the Aspire CLI. For example, use `Development` to load `appsettings.Development.json` over the base file:
...
<Aside type="note">
Aspire looks for `appsettings.json` and `appsettings.{environment}.json` in the same directory as `apphost.mts`. Command-line arguments take precedence over both files, so you can override any setting at run time without editing JSON.
</Aside>
