...
Before starting a TypeScript AppHost, the Aspire CLI runs `tsc --noEmit` to check for type errors. If your AppHost has TypeScript compile errors, `aspire run` and `aspire publish` stop before the AppHost launches and display the diagnostic output:

```text
apphost.ts(22,7): error TS2322: Type 'string' is not assignable to type 'number'.
```

This prevents the dashboard and resources from starting in a partially broken state.

:::note
TypeScript validation adds approximately **0.8–1.0 seconds** to startup time on a typical AppHost.
:::
