---
title: TypeScript AppHost (Preview)
...
Aspire's TypeScript AppHost support allows you to write AppHosts in TypeScript instead of C#. This enables teams to use TypeScript while still benefiting from Aspire's orchestration capabilities, including access to 40+ hosting integrations (Redis, PostgreSQL, Azure services, and more).

## How it works

When you create a TypeScript AppHost:
...
Before creating a TypeScript AppHost, ensure you have the following installed:
...
1.  Create and initialize the AppHost:
    ...
    aspire new --name my-apphost
    ```

    Use the <Kbd windows="↑" mac="↑" /> and <Kbd windows="↓" mac="↓" /> keys to navigate the options, look for `Empty (TypeScript AppHost)`. Press <Kbd windows="Enter" mac="Return" /> to confirm your selection.
...
To add Aspire hosting integrations to your TypeScript AppHost, use the `aspire add` command. The following example adds Redis and Azure Storage integrations.
...
2. Update `apphost.ts` to reference your JavaScript application:
   ...
   ```typescript title="apphost.ts" ins={11-15}
   import { createBuilder } from './.modules/aspire.js';
   ...
   <LearnMore>
     See the [JavaScript integration](/integrations/frameworks/javascript/) for
...
If you encounter issues with your TypeScript AppHost, this section provides guidance on diagnosing and resolving common problems.

### Common issues

Here are solutions to common problems you may encounter when working with TypeScript AppHosts.
...
TypeScript AppHost support is a preview feature. Your feedback helps us improve language support and prioritize new features. Please share your experiences, suggestions, and bug reports on the [Aspire GitHub repository](https://github.com/dotnet/aspire/issues).
