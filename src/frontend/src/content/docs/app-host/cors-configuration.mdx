...
const frontend = await builder.addJavaScriptApp("frontend", "../frontend", { runScriptName: "dev" });
...
<Aside type="note">
`ASPIRE_DASHBOARD_CORS_ALLOWED_ORIGINS` is the AppHost-facing setting. When you run the dashboard as a standalone container, configure the equivalent `DASHBOARD__OTLP__CORS__ALLOWEDORIGINS` setting instead. For more information, see [Enable browser telemetry](/dashboard/enable-browser-telemetry/).
</Aside>
