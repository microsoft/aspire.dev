...
app.UseCors(); // Must be called before UseAuthorization and MapControllers
...
**Cause**: The order of middleware registration matters. `UseCors()` must be called **before** `UseAuthorization()` and **before** any endpoint mapping.

**Solution**: Ensure middleware is ordered correctly:

```csharp title="C# — Program.cs"
app.UseRouting(); // If using explicit routing
app.UseCors();    // CORS must come before Authorization
...
**Solution**: `UseCors()` handles preflight automatically. Ensure it's in your middleware pipeline. If you're using `[Authorize]` or other middleware that short-circuits before CORS, move `UseCors()` earlier.
