...
await builder.addContainer("myapp", "myapp:latest")
    .withContainerFiles("/app/config", [
        {
            type: "file",
            name: "appsettings.json",
...
await builder.addContainer("worker", "worker:latest")
    .withContainerFiles("/app/config", async () => {
        const config = {
...
await builder.addContainer("myapp", "myapp:latest")
    .withContainerFiles("/app/data",
        ...
        { defaultOwner: 33, defaultGroup: 33 }); // www-data
```
</TabItem>
</Tabs>
