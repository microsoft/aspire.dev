...
| `ASPIRE_DASHBOARD_AI_DISABLED`              | `true`                                                      | When set to `true`, the GitHub Copilot UI in the dashboard is disabled and not visible. This only affects the in-dashboard Copilot UI — AI coding agents can still access telemetry data via the [Aspire CLI and MCP server](/dashboard/ai-coding-agents/). |
