...
description: Reference for the Aspire MCP server. A legacy integration path for AI coding agents. New projects should use the Aspire skill file instead.
---

import { Steps, Tabs, TabItem } from '@astrojs/starlight/components';
import LearnMore from '@components/LearnMore.astro';

:::danger[Use the Aspire skill file instead]
The **Aspire skill file** is the primary, recommended way to integrate AI coding agents with Aspire. It drives the Aspire CLI the same way a developer does and works with every supported agent without extra runtime wiring. See [Use AI coding agents](/get-started/ai-coding-agents/).

The Aspire MCP server described on this page is a **legacy** option that remains available, but:

- **Do not enable the MCP server and the skill file at the same time.** They cover overlapping ground, and running both leads to duplicate tools and conflicting guidance.
- **If your project currently uses the Aspire MCP server, migrate to the skill file.** See [Migrate from the Aspire MCP server](/get-started/ai-coding-agents/#migrate-from-the-aspire-mcp-server).

The Aspire Dashboard's MCP endpoint and the resource-level MCP APIs on the product itself are **not** affected by this guidance.
:::

The Aspire <abbr title="Model Context Protocol" data-tooltip-placement="top">MCP</abbr> server gives AI coding agents direct runtime access to your running Aspire application. Through the Model Context Protocol (MCP), agents can query resource status, read logs, inspect distributed traces, and execute commands.

When selected, the MCP server is configured by `aspire agent init`. Most teams should skip this option and install the Aspire skill file instead.
