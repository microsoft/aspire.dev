...
When you add `builder.AddServiceDefaults()`/`builder.addServiceDefaults()` to a project, you get production-ready observability and resilience out of the box.
...
`WithReference()`/`withReference()` is how you connect resources together. When you call `.WithReference(otherResource)`/`.withReference(otherResource)`, Aspire:
...
`WaitForCompletion()`/`waitForCompletion()` waits for a resource to finish and exit (not just start). Useful for setup scripts:
...
`WaitForStart()`/`waitForStart()` waits only for a resource to reach the running state, without waiting for health checks to pass:
...
<Aside type="tip">
  Use `WaitForStart()`/`waitForStart()` when you need looser coupling—your service can handle initial connection failures via retry policies. Use `WaitFor()`/`waitFor()` when you need strict dependency on health.
</Aside>
...
- Works automatically when you use `WithReference()`/`withReference()`
...
1. **AppHost level**: Determines when dependencies are ready (controls `WaitFor()`/`waitFor()` behavior)
...
- Custom values via `WithEnvironment()`/`withEnvironment()`
...
Example: `Aspire.Hosting.PostgreSQL` adds the `AddPostgres()`/`addPostgres()` method.
