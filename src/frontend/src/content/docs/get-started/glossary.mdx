...
In this repository, **polyglot** means relating to code and applications written in multiple languages, including TypeScript, C#, Python, Go, Java, and Rust. Aspire supports polyglot code in AppHosts, generated SDKs, other projects, and related tooling.
...
<ApiReference name="Aspire.Hosting.ResourceBuilderExtensions.WithReference" /> is how you connect resources together. When you call `.WithReference(otherResource)`, Aspire:
...
<ApiReference name="Aspire.Hosting.ResourceBuilderExtensions.WaitFor" /> tells Aspire to delay starting a resource until its dependency is ready:
...
<Aside type="tip">
  Without <ApiReference name="Aspire.Hosting.ResourceBuilderExtensions.WaitFor" />, your API might start and immediately crash because the database isn't ready yet. This prevents race conditions during startup.
</Aside>

### WaitForCompletion

<ApiReference name="Aspire.Hosting.ResourceBuilderExtensions.WaitForCompletion" /> waits for a resource to finish and exit (not just start). Useful for setup scripts:
...
<ApiReference name="Aspire.Hosting.ResourceBuilderExtensions.WaitForStart" /> waits only for a resource to reach the running state, without waiting for health checks to pass:
...
<Aside type="tip">
  Use <ApiReference name="Aspire.Hosting.ResourceBuilderExtensions.WaitForStart" /> when you need looser coupling—your service can handle initial connection failures via retry policies. Use <ApiReference name="Aspire.Hosting.ResourceBuilderExtensions.WaitFor" /> when you need strict dependency on health.
</Aside>
...
- Retrieved using `builder.Configuration.GetConnectionString("resource-name")`/`builder.configuration.getConnectionString("resource-name")`
...
- Works automatically when you use <ApiReference name="Aspire.Hosting.ResourceBuilderExtensions.WithReference" />

For details, see [Service Discovery](/fundamentals/service-discovery/).

### Health check

A **health check** is a mechanism to determine if a resource is ready and functioning. Aspire uses health checks in two ways:

1. **AppHost level**: Determines when dependencies are ready (controls <ApiReference name="Aspire.Hosting.ResourceBuilderExtensions.WaitFor" /> behavior)
2. **Application level**: Exposes `/health` and `/alive` endpoints for load balancers

For details, see [Health Checks](/fundamentals/health-checks/).
...
- Custom values via <ApiReference name="Aspire.Hosting.ResourceBuilderExtensions.WithEnvironment" />
...
Example: `Aspire.Hosting.PostgreSQL` adds the <ApiReference name="Aspire.Hosting.PostgresBuilderExtensions.AddPostgres" /> method.
...
**OpenTelemetry** (<abbr title="OpenTelemetry" data-tooltip-placement="top">OTEL</abbr>) is the observability standard that Aspire uses for:
...
| Concept | Definition |
|---------|------------|
| <abbr title="Directed Acyclic Graph" data-tooltip-placement="top">DAG</abbr> | Directed acyclic graph—the structure of resource dependencies. |
