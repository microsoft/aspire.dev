...
- The SDK selects the executable used to launch the AppHost as follows:
  ...
  2. When `AspireCliInvocationMode` is set to `Dnx`, the SDK selects a usable `dnx` host.
  3. Otherwise, the default `Path` mode selects `aspire` on `PATH`, then falls back to a usable `dnx` host.
...
- If forced DNX mode can't find `dnx`, ordinary builds remain valid. The run preflight emits error [`ASPIRE011`](/diagnostics/aspire011/) when it prepares the launch command.

#### Delegating through DNX

In the default `Path` mode, the AppHost can fall back to DNX when `aspire` isn't found on `PATH`. A PATH `aspire` candidate that can't support the run hook can also fall back to DNX. An explicit `AspireCliPath` remains authoritative and isn't replaced by the fallback.

The AppHost invokes DNX noninteractively as `dnx --yes aspire.cli@$(AspireHostingSDKVersion)`. This restores and executes the exact Aspire CLI version paired with the AppHost SDK from your configured NuGet sources. If DNX can't restore or probe that package, the launch fails explicitly instead of falling back to an incomplete direct launch.
