...
- At build time, the SDK resolves the invocation used to launch the AppHost in this priority order:
    1. An explicit `AspireCliPath` (or `AspireCliBundlePath`) property in the project file.
    2. Forced DNX mode, when `AspireCliInvocationMode` is set to `Dnx`.
    3. The `aspire` executable on `PATH`.
    4. The `dnx` command on `PATH`.
- If the bundle cannot be resolved, the build emits error [`ASPIRE009`](/diagnostics/aspire009/) with a message directing you to [get.aspire.dev](https://get.aspire.dev) to install the Aspire CLI.
- If `AspireCliInvocationMode` is forced to `Dnx` but `dnx` isn't found on `PATH`, the build emits error [`ASPIRE011`](/diagnostics/aspire011/).

#### Delegating through DNX

When `aspire` isn't found on `PATH` (or `AspireCliInvocationMode` is set to `Dnx`), the AppHost delegates its launch to DNX, invoked noninteractively as `dnx --yes aspire.cli@$(AspireHostingSDKVersion)`. This restores and executes the exact Aspire CLI version paired with the AppHost SDK from your configured NuGet sources, rather than an unpinned package. If DNX can't restore or probe that package, the launch fails explicitly instead of falling back to an incomplete direct launch.

To force DNX delegation instead of preferring `aspire` on `PATH`, set `AspireCliInvocationMode`:
