...
The .NET project templates are available in the [📦 Aspire.ProjectTemplates](https://www.nuget.org/packages/Aspire.ProjectTemplates) NuGet package. Some templates, such as `aspire-py-starter`, are provided by the Aspire CLI and aren't included in the NuGet package.
...
The Aspire CLI installs templates on demand when you run `aspire new`. To create a project or solution, see the [creation examples](#create-solutions-and-projects-using-templates).

To use a specific version of the project templates, pass the `--version` option. You can also use `--source` to select a NuGet source.

```bash title="Aspire CLI"
aspire new aspire-starter --version 13.1.0
```
