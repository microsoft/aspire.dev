...
- **.NET API**: Uses `AddProject<Projects.Api>()` to reference a .NET project
- **Python API**: Uses `AddUvicornApp()` with `WithUv()` for ASGI apps like FastAPI

In both cases, the PostgreSQL database and React front end remain identical. Aspire's `WithReference()` method establishes dependencies between resources, and `WaitFor()` ensures services start in the correct order.
