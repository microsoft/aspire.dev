...
<Aside type="tip" title="Use the AppHost from VS Code">
  The [Aspire VS Code extension](/get-started/aspire-vscode-extension/) discovers the AppHost in your workspace, shows it in the Aspire view, and lets you run, debug, and open the dashboard without leaving the editor.
</Aside>
...
<Aside type="tip">
  To also keep the container running between AppHost restarts (avoiding slow startup), add `.WithPersistentLifetime()`. See [resource lifetimes](/app-host/resource-lifetimes/) for details.
</Aside>
