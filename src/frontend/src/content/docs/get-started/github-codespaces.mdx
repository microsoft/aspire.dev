...
To configure GitHub Codespaces for Aspire, use the `.devcontainer/devcontainer.json` file in your repository. The simplest way to get started is by creating a new repository from the [Aspire Dev Container template](https://github.com/microsoft/aspire-devcontainer). Consider the following steps:
...
1. From the new repository, select the **Code** button, open the **Codespaces** tab, and then select **Create codespace on main**.
    ...
    At this point, the Aspire CLI has been installed and the local HTTPS development certificate for .NET workloads has been created and trusted inside the Codespace. You can use `aspire new` right away without manually installing `Aspire.ProjectTemplates`.
