...
The `aspire init` command creates the orchestration layer and delegates wiring to an AI coding agent via the **`aspireify` skill**. The command works in two phases:

1. **Phase 1 — Drop the skeleton.** `aspire init` creates a minimal AppHost file and an `aspire.config.json`, then installs the `aspireify` skill into your agent's skill directory.
2. **Phase 2 — Agent-driven wiring.** Your AI coding agent (GitHub Copilot, Claude Code, or any MCP-compatible assistant) picks up the `aspireify` skill and completes the resource discovery, `WithReference`/`WaitFor` wiring, and OpenTelemetry setup. The skill self-deletes after a successful `aspire start`.
...
2. Run `aspire init`:
   ...
   The command runs in interactive mode by default. It creates a minimal AppHost skeleton and installs the `aspireify` skill for your AI agent to complete.

3. Ask your AI coding agent to run the `aspireify` skill. The agent will scan your repo, wire up resources, configure OpenTelemetry, and validate the setup by running `aspire start`.
...
2. Run `aspire init` with the TypeScript AppHost option:
   ...
   If you omit `--language typescript`, choose **TypeScript** when the CLI prompts for the AppHost language. The command creates a minimal AppHost skeleton and installs the `aspireify` skill for your AI agent to complete.

3. Ask your AI coding agent to run the `aspireify` skill. The agent will scan your workspace, wire up resources, configure OpenTelemetry, and validate the setup by running `aspire start`.
