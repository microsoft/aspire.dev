...
Unlike the deprecated `AddNpmApp` API in Aspire 9, the new JavaScript hosting methods (`AddJavaScriptApp`, `AddViteApp`, `AddNodeApp`) do not support an `args` parameter. To pass arguments to your scripts, define custom scripts in your `package.json` with the required arguments, then reference those scripts using the `WithRunScript` method. For example:
...
This approach keeps all script configuration in `package.json`, making your scripts more discoverable and easier to run outside of Aspire.
:::
