...
2. Ask your AI coding agent to run the `aspireify` skill. The agent will:
   ...
   - Wire resources into the AppHost with <ApiReference name="Aspire.Hosting.ResourceBuilderExtensions.WithReference" />, <ApiReference name="Aspire.Hosting.ResourceBuilderExtensions.WaitFor" />, endpoints, and volumes
...
**Project-based AppHost** — a traditional `AppHost.csproj` that lives inside a `.sln` alongside your other C# projects. Uses `ProjectReference` items and the generated `Projects` namespace for strongly-typed <ApiReference name="Aspire.Hosting.ProjectResourceBuilderExtensions.AddProject" /> calls. Best when your repo is already a .NET solution and you want IDE-integrated orchestration.
...
The following example registers a Python API with <ApiReference name="Aspire.Hosting.PythonAppResourceBuilderExtensions.AddUvicornApp" />, a worker with <ApiReference name="Aspire.Hosting.PythonAppResourceBuilderExtensions.AddPythonApp" />, and a frontend with <ApiReference name="Aspire.Hosting.JavaScriptHostingExtensions.AddViteApp" />.
...
If a workload does not have a dedicated hosting API yet, model it as an executable resource with <ApiReference name="Aspire.Hosting.ExecutableResourceBuilderExtensions.AddExecutable" /> so it can still participate in the same application model.
...
Use <ApiReference name="Aspire.Hosting.ContainerResourceBuilderExtensions.AddContainer" /> for the published application images and <ApiReference name="Aspire.Hosting.ResourceBuilderExtensions.WithHttpEndpoint" /> to configure their HTTP endpoints:
