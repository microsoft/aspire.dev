...
Aspire integrations simplify common infrastructure such as Redis, PostgreSQL, RabbitMQ, and more. They are often the fastest way to make an existing setup easier to run because they replace copied connection strings and hand-managed startup order with explicit resource relationships. The earlier scenarios already showed the `aspire add ...` step; here, the focus is on wiring the integration into your workloads and configuring the corresponding clients.

<Steps>

1. Identify the shared resource you want Aspire to own, such as a database, cache, queue, or storage service.
2. Add the hosting integration once if it is not already in your AppHost.
3. Reference that resource from the workloads that depend on it.
4. Configure the matching client library inside each consuming service.

</Steps>

> [!NOTE]
> If you have not added Redis yet, run `aspire add redis` once before using the example below.

The following example starts from the AppHost after Redis has been added and shows how to wire it into existing workloads.
