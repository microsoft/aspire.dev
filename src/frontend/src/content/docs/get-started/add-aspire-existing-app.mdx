...
The language of the app you're bringing to Aspire doesn't have to match the language of the AppHost. Your services can stay in C#, Python, or JavaScript while Aspire uses a separate AppHost to orchestrate them.

This guide groups examples by the language of the existing apps you're adding. The JavaScript walkthrough uses a TypeScript AppHost so JavaScript developers can stay in the same toolchain end-to-end. If you prefer a different AppHost language, use `aspire init --language ...` to choose it explicitly.
</Aside>
...
2. Run `aspire init` to initialize Aspire support:
   ...
   <Aside type="tip" title="Choose the AppHost language explicitly">
   Use `aspire init --language csharp` or `aspire init --language typescript` to skip the prompt and create the AppHost language you want up front. The JavaScript examples in this guide use the TypeScript AppHost path.
   </Aside>
   ...
   - Install necessary Aspire packages
    ...
    - Ask which AppHost language to create
    - Create a file-based C# AppHost (`apphost.cs`) or a TypeScript AppHost (`apphost.ts`)
    - Install the supporting packages for that AppHost
    </Pivot>
    <Pivot id="javascript">
...
This step only applies to C# project-based AppHosts. Python and JavaScript file-based AppHosts don't use `.csproj` project references.
...
<Aside type="tip">
You can mix and match languages! The examples above show single-language scenarios, but Aspire excels at orchestrating multi-language applications. Add C#, Python, and JavaScript resources to the same AppHost to create truly cross-language systems.
</Aside>

### Connect services

The `WithReference` / `withReference` calls establish service dependencies and enable service discovery:
...
3. Update your AppHost to reference the integration and share it across all your services:
   ...
   ```csharp title="apphost.cs — Add Redis integration for multiple services"
   var builder = DistributedApplication.CreateBuilder(args);
...
2. The dashboard URL will appear in your terminal output:
   ...
   The CLI output also shows which AppHost Aspire found, such as `apphost.ts`, `apphost.cs`, or an `AppHost.csproj` file.
...
The exact AppHost methods differ by language and service type, but the orchestration pattern is the same: model the shared dependencies once, then connect services with `WithReference` / `withReference`.
