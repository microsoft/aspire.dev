...
The Aspire CLI is designed for agent-driven workflows. Every command works non-interactively, supports `--format Json` for structured output, and never blocks on prompts.

Key commands agents use:

- **`aspire start`** — runs the AppHost in the background (no foreground process to manage)
- **`aspire start --isolated`** — runs in isolation for parallel worktrees, avoiding port conflicts
- **`aspire wait <resource>`** — blocks until a resource is healthy before proceeding
- **`aspire describe`** — shows resource status, endpoints, and health at a glance
- **`aspire logs`** / **`aspire otel logs`** — reads console and structured logs
- **`aspire docs search`** — searches aspire.dev documentation from the CLI
- **`aspire mcp call`** — invokes resource MCP tools (e.g., SQL queries) directly

The Aspire skill file (installed by `aspire agent init`) teaches agents all of these patterns automatically.

The [Aspire MCP server](/get-started/aspire-mcp-server/) exposes the same resource data, structured logs, and distributed traces available through the CLI and the [Aspire Dashboard](/dashboard/overview/) — directly to your coding agent. Agents can diagnose issues, trace requests across services, and understand your application's runtime behavior without you switching to a browser or copying terminal output.
