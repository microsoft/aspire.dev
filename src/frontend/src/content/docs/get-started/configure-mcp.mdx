...
The [Aspire MCP server](/get-started/aspire-mcp-server/) exposes the same resource data, structured logs, and distributed traces available through the CLI and the [Aspire Dashboard](/dashboard/overview/) — giving agents the same visibility as human developers. Whether a person is debugging in the dashboard or an agent is diagnosing through MCP, they see the same picture.
