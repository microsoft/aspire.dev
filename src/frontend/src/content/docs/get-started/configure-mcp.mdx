...
Aspire provides powerful integrations with AI assistants through the Model Context Protocol (MCP). This enables _agentic development_ — a workflow where AI assistants can directly interact with your Aspire application to help you build, debug, and monitor your distributed applications.
...
<Aside type="tip">
  If you don't already have an `AGENTS.md` file in your project, one will be
  created for you. This file contains instructions that help your AI coding
  agent perform better when working with an Aspire-enabled codebase.
</Aside>
...
<Aside type="tip">
  Because the STDIO transport uses pipes rather than network sockets, there is
  no network attack surface when using `aspire agent mcp`. The MCP server is
  only accessible to the AI assistant process that started it.
</Aside>
...
<Aside type="note">
  If your AI assistant doesn't seem to detect your AppHost, verify it's running
  with `aspire run` and that the AI assistant was started from (or has access
  to) the same workspace directory as your AppHost project.
</Aside>
...
<Aside type="tip">
  The Aspire MCP server itself doesn't require any explicit exclusion — it's
  part of the Aspire CLI tooling and runs separately from your AppHost. This
  pattern is useful for other dev-only resources that might interfere with
  deployment, such as local monitoring tools or test containers.
</Aside>
