...
Beyond the CLI, the [Aspire MCP server](/get-started/aspire-mcp-server/) exposes the same resource data, structured logs, and distributed traces available in the [Aspire Dashboard](/dashboard/overview/) — but directly to your coding agent. This means agents can diagnose issues, trace requests across services, and understand your application's runtime behavior without you switching to a browser.
