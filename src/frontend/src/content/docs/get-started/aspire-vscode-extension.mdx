...
The Aspire VS Code extension is also published as a **pre-release** on the VS Code Marketplace. Pre-release builds contain the latest in-progress features and fixes ahead of stable releases. To install or switch to the pre-release version, open the extension's Marketplace page, select the dropdown arrow next to **Install**, and choose **Install Pre-Release Version**. You can switch back to the stable release at any time from the same dropdown.
:::
