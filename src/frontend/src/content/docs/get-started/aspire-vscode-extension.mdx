...
Pressing <Kbd windows='F5' /> without a saved `launch.json` entry uses a *dynamic* debug configuration that the extension generates on the fly and remembers for next time. In a multi-root workspace, each workspace folder gets its own remembered dynamic configuration, so starting the remembered configuration for a given folder always launches that folder's AppHost — even if two folders share the same display name.
