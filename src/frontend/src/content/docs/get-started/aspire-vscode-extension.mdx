...
<LoopingVideo
  aria-label="Video showing the Aspire VS Code extension happy path: create or open an Aspire app, discover the AppHost, run and debug, then inspect the dashboard."
  sources={[
    {
      src: '/aspire-vscode-extension-demo.mp4',
      type: 'video/mp4',
      title: 'Aspire VS Code extension happy-path demo.',
    },
  ]}
/>

<p><em>Open a repo, let the extension find the AppHost, press <strong>F5</strong>, then use the Aspire view and dashboard while the app runs.</em></p>
