...
| `aspire.registerMcpServerInWorkspace` | `false` | Register the Aspire <abbr title="Model Context Protocol" data-tooltip-placement="top">MCP</abbr> server (see [MCP Server Integration](#mcp-server-integration)). |
...
## <abbr title="Model Context Protocol" data-tooltip-placement="top">MCP</abbr> server integration
...
Turn it on by setting `aspire.registerMcpServerInWorkspace` to `true`. When enabled, the extension registers the <abbr title="Model Context Protocol" data-tooltip-placement="top">MCP</abbr> server definition via the Aspire CLI whenever a workspace is open and the CLI is available.
