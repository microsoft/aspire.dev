...
import paneActions from '@assets/get-started/code-extension-pane-actions.png';
...
When you're ready to ship your app, [publish or deploy from the Aspire
view](#publish-or-deploy-from-the-aspire-view), or [run and debug a pipeline
step](#run-or-debug-a-pipeline-step).
...
### Publish or deploy from the Aspire view

Select **Aspire** in VS Code's Activity Bar to open **Aspire: AppHosts**. Expand an
idle AppHost, or right-click it, to choose an action. In a workspace with multiple
AppHosts, choose the action on the AppHost you want to target.

| Action                  | Behavior                                                                                  |
| ----------------------- | ----------------------------------------------------------------------------------------- |
| **Deploy AppHost**       | Executes the deployment workflow configured in your AppHost.                               |
| **Publish AppHost**      | Generates deployment artifacts for the configured target without applying the deployment.  |
| **Run pipeline step**    | Runs a selected pipeline step and its dependencies without attaching the debugger.         |
| **Debug pipeline step**  | Runs a selected pipeline step and its dependencies with the debugger attached.              |

<Image
  src={paneActions}
  alt="An idle AppHost in the Aspire view with Deploy AppHost, Publish AppHost, Run pipeline step, and Debug pipeline step action rows."
/>

<p><em>An idle AppHost exposes publishing, deployment, and pipeline actions directly in the Aspire view.</em></p>

Configure your deployment target and pipeline in the AppHost before publishing or
deploying. The pane runs that configuration; it does not choose a target or add
deployment configuration for you.

Actions appear according to the capabilities of the Aspire CLI selected for that
AppHost. Running AppHosts expose supported actions in their context menu rather
than as idle action rows. While a publish, deploy, or pipeline operation is in
progress, the selected AppHost shows its busy state and hides conflicting actions
until the operation finishes.

Follow prompts in VS Code and review command output in the **Debug Console**. If
an operation fails, review the reported error, correct the configuration, and
retry.

<LearnMore>
  See [How Aspire deployment works](/deployment/deploy-with-aspire/) for the
  difference between publishing artifacts and executing a deployment.
</LearnMore>

### Run or debug a pipeline step

To debug pipeline code in a C# AppHost, open the AppHost or custom step source and
set a breakpoint before starting the step.

<Steps>

1. Under an idle AppHost, or in its context menu, select **Run pipeline step** or
   **Debug pipeline step**.

1. Wait for **Loading pipeline steps...** to finish. When step discovery is
   supported, the picker lists step names, descriptions, and resource names when
   provided. Type to filter by any of these fields.

1. Select a discovered step, or choose **Enter the pipeline step to execute** and
   enter the exact step name manually.

1. Review execution output in the **Debug Console**. When debugging reaches your
   breakpoint, use VS Code's debugging controls to inspect the code and continue.

</Steps>

Cancel the **Loading pipeline steps...** progress notification, or dismiss the
picker or manual-entry prompt with <Kbd windows='Esc' />, to abandon selection
without executing a step. A discovery failure is reported as an error. Depending
on CLI and AppHost capabilities, an interactive CLI prompt or a manual step-name
prompt can appear instead of the discovered-step picker.

If a step name is invalid or execution fails, review the error and command output,
correct the step name or AppHost configuration, and try again.

<LearnMore>
  See [Pipelines](/deployment/pipelines/) for step dependencies and custom pipeline
  behavior.
</LearnMore>
