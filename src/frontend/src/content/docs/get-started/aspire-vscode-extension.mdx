...
Expand an idle AppHost or open its right-click menu to select **Deploy AppHost**, **Publish AppHost**, **Run pipeline step**, or **Debug pipeline step**. The pipeline actions prompt you to choose a step. Wait for the operation to finish before starting another for the same AppHost. For launch configuration options, see [Run, debug, and deploy](#run-debug-and-deploy).
