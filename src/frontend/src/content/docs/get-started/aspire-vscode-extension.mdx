...
**Aspire: Configure launch.json file** adds a minimal launch configuration `.vscode/launch.json` that supports AppHosts written in any language. AppHosts are discovered automatically in the workspace using `aspire ls`.
...
### AppHost launch path resolution

The extension resolves the `program` path to the correct launch target before invoking the CLI:

- **SDK-style C# AppHosts** — If `program` points to a C# source file (for example `Program.cs`) inside an AppHost project folder, the extension automatically resolves that path to the containing `.csproj` file. This means starting a debug session from an open `Program.cs` in VS Code works without extra configuration.
- **Single-file C# AppHosts** — Files with the `#:sdk Aspire.AppHost.Sdk` directive at the top are standalone scripts and launch directly from their source path, not from a project file.
- **TypeScript/JavaScript AppHosts** — Files discovered by `aspire ls` as `language: "typescript/nodejs"` (for example, `apphost.ts`) continue to launch from their source file path.

:::tip
You can also point `program` directly at the AppHost `.csproj`:
