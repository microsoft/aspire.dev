...
## Agent asset selection during project setup

When you run **Aspire: New Aspire project** or **Aspire: Initialize Aspire in an existing codebase** from VS Code, the multi-select Quick Pick for AI agent skills and skill locations now opens with the same items pre-checked as the equivalent Aspire CLI terminal prompt. The Aspire CLI remains the source of truth for these defaults — it computes the recommended skill location and applicable bundled skills, and the extension only renders them as initially picked. `aspire init` also includes the one-time `aspireify` skill in that preselection, matching existing CLI policy.

You can still change or clear any preselected item before confirming the Quick Pick. If you're running an older Aspire CLI with a newer extension, the Quick Pick opens with nothing preselected, matching prior behavior.

<LearnMore>
  For the full list of Aspire workflow skills and their defaults, see [Aspire
  skills](/get-started/aspire-skills/) and the [`aspire agent init` command
