...
import codeLensGutter from '@assets/get-started/code-extension-codelens.png';
import codeLensGutterLight from '@assets/get-started/code-extension-codelens-light.png';
...
1. Open an existing Aspire project in VS Code or create one using the Command Palette command **Aspire: New Aspire project** with (<Kbd windows='Ctrl+Shift+P' mac='Cmd+Shift+P' />).
...
- **Aspire: Add an integration** offers a selection between hosting integration (`Aspire.Hosting.*`) to add to your AppHost.
...
<ThemeImage
  dark={codeLensGutter}
  light={codeLensGutterLight}
  alt="An AppHost source file with CodeLens above each resource definition showing state and Start/Stop/Restart/View Logs actions, and colored gutter icons in the margin."
/>
...
**Editor title bar buttons** add **Run apphost** and **Debug apphost** actions next to the standard Run/Debug buttons whenever you have an AppHost file open.
