...
An idle AppHost item in the tree also exposes **Deploy**, **Publish**, **Run pipeline step**, and **Debug pipeline step** actions, alongside the existing **Run** and **Debug** actions — both as action rows on the item and as right-click context menu entries. These delegate to the same AppHost launch logic as the `deploy`, `publish`, and `do` [launch configurations](#run-debug-and-deploy), so a second operation against the same AppHost is safely rejected while one is already in progress. **Run pipeline step** and **Debug pipeline step** prompt you for the pipeline step to execute.
