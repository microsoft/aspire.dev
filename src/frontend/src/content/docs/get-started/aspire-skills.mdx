...
1. Open the [`aspire-skills` marketplace install link](https://github.com/copilot/app/launch?entry_point=aspire_skills_docs&open=ghapp%3A%2F%2Fplugins%2Fmarketplace%2Fadd%3Fsource%3Dmicrosoft%2Faspire-skills) to automatically open the **Settings** > **Plugins** window in the GitHub Copilot app.
...
| `aspire-project-v2-migration` | Migrating eligible AppHosts to Project v2 resource APIs                                                | Assess legacy project resources, require approval for exact edits, and preserve supported behavior while migrating to `DotnetProjectResource` APIs                       |
...
| `aspire-project-v2-migration` | Migrating legacy .NET project resources in a C# or TypeScript AppHost targeting Aspire 13.6 or newer   | Assess existing project resources, propose an approval-first migration, preserve supported configuration, and flag unsupported patterns such as Azure Functions and custom `ProjectResource`-based integrations |
...
Use `aspire-project-v2-migration` when a C# or TypeScript AppHost targeting Aspire 13.6 or newer still has legacy project resources to migrate. The skill doesn't upgrade the AppHost's Aspire version — run the migration only after you've already updated to Aspire 13.6 or newer.

<LearnMore>
  For details on what the migration preserves and flags, see [Migrate from
  legacy project
  resources](/integrations/dotnet/project-resources/#migrate-from-legacy-project-resources).
</LearnMore>
