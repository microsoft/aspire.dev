...
- **Through the `aspire mcp` CLI** — use `aspire mcp tools` and `aspire mcp call` to discover and invoke resource tools directly from the terminal. Your AI agent can invoke these commands through the [Aspire skill file](/get-started/ai-coding-agents/).
- **Through the Aspire MCP server** — if you have enabled the legacy [Aspire MCP server](/get-started/aspire-mcp-server/), resource tools are proxied alongside the built-in MCP tools. Most projects should use the skill file + CLI path above.
...
- [Use AI coding agents](/get-started/ai-coding-agents/) — set up your project for AI agents using the Aspire skill file
- [Aspire MCP server](/get-started/aspire-mcp-server/) — legacy runtime MCP integration
