...
:::note
The Aspire CLI ships as a self-contained NativeAOT binary and its package uses a version-agnostic `tools/any/` layout, so `dotnet tool install -g Aspire.Cli` succeeds with any supported .NET SDK — even one older than the SDK used to build the CLI itself.
:::
