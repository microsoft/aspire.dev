...
You can install the Aspire CLI using an installation script (recommended) or as a .NET global tool.
...
<OsAwareTabs syncKey="terminal">
    ...
    <TabItem label=".NET CLI" icon="seti:json">
        <Code
            lang="bash"
            code="dotnet tool install -g Aspire.Cli"
        />
        
        <Aside type="tip" title="Alternative method" icon="information">
            This installs the Aspire CLI as a [.NET global tool](https://www.nuget.org/packages/Aspire.CLI). We recommend using the native executable installation script (Bash or PowerShell) for the best experience.
        </Aside>
    </TabItem>
</OsAwareTabs>
