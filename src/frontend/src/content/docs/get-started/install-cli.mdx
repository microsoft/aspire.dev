...
You can also run the npm package with [`npx`](https://docs.npmjs.com/cli/commands/npx) without installing the CLI on your `PATH`. First, confirm your [Node.js](https://nodejs.org/) version meets the [`@microsoft/aspire-cli` npm package requirements](https://www.npmjs.com/package/@microsoft/aspire-cli). Then prefix any Aspire command with `npx -y @microsoft/aspire-cli`:
