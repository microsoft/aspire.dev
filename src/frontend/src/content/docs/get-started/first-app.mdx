...
1. Examine the created template structure. The Aspire CLI creates a new folder named _AspireApp_ in the current directory. This folder contains the solution file and several projects, including:
...
1. Explore the AppHost code that orchestrates your app.
    ...
    - `AddProject` registers your API and web frontend services
    ...
    - `AddUvicornApp` registers your Python FastAPI backend
    ...
    Notice how your entire application topology is defined in code, not YAML or JSON. This makes it easy to understand, modify, and version control your app's architecture.
    </Aside>
...
1. Call `aspire run` to start dev-time orchestration:
    ...
    ```bash title="Example output of aspire run" mark={8}
    🔍  Finding apphosts...
