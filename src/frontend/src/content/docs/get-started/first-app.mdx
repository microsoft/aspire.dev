...
1. Create a new Aspire solution from a template:
    ...
    When prompted "Would you like to configure AI agent environments for this project?", select `y`. This sets up workspace configurations (such as MCP server settings and AI skills) for your project, enabling a richer experience with AI coding assistants. For more information, see [Configure the MCP server](/get-started/configure-mcp/) and the [`aspire agent init`](/reference/cli/commands/aspire-agent-init/) reference.
    </Aside>
...
1. Explore the AppHost code that orchestrates your app.
    ...
    - `WaitFor` ensures the API is healthy before starting the frontend, preventing connection errors
    ...
    <LearnMore>
    This template uses a TypeScript AppHost. To learn more about the architecture, see [Multi-language architecture](/architecture/multi-language-architecture/).
    </LearnMore>
