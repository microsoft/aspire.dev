...
1. Select the agent guidance locations and options for your environment. Keep Aspire skills selected. Add Playwright CLI for browser checks, add `dotnet-inspect` for .NET API discovery, or select **Install Aspire MCP server** if the agent needs live data from a running Aspire app. For skill descriptions and installation locations, see [Aspire skills](/get-started/aspire-skills/).
