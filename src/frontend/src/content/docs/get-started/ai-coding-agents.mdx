...
1. Select the **skills and tools** to install into those locations. All options are pre-selected by default:
   ...
   :::note[Network access required for Aspire workflow skills]
   The `aspire`, `aspireify`, and `aspire-deployment` skills are fetched from the external `@microsoft/aspire-skills` npm bundle. The first install requires internet access; subsequent installs reuse the locally cached bundle at `~/.aspire/cache/aspire-skills/` (or `%USERPROFILE%\.aspire\cache\aspire-skills\` on Windows).
   :::
