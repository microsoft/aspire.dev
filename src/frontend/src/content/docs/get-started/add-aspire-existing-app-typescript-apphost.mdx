...
2.  Run `aspire init` to initialize Aspire support:
    ...
    <LearnMore>
      For more details on the `aspire init` command and its options, see the
...
<FileTree>
  - my-store/ - **apphost.ts** (new) orchestration code - **.modules/** (new)
  generated SDK - **aspire.config.json** (new) configuration - **package.json**
  (new) AppHost dependencies - **tsconfig.json** (new) TypeScript configuration
  - packages/ - api/ - src/ - server.js - routes/ - package.json - web/ - src/ -
  App.tsx - components/ - package.json - vite.config.ts - admin/ - src/ -
  main.ts - views/ - package.json - package.json (workspace root)
</FileTree>
...
<Aside type="note">
  ...
  first Aspire app](/get-started/deploy-first-app-typescript/) for more details.
</Aside>
