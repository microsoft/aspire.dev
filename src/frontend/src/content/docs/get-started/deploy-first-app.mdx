...
The React (Vite) & FastAPI starter template consists of two resources that are deployed as a single container. The FastAPI server hosts both the API and the static frontend files.
</Pivot>
...
<Tabs syncKey="deploy-target">
    ...
    <TabItem id="azure" label="Azure">
        ...
        - [Azure CLI](https://learn.microsoft.com/en-us/cli/azure/install-azure-cli) installed and configured. You should be logged in using `az login`.
        </div>
    </TabItem>    
</Tabs>
...
<Tabs syncKey="deploy-target">
    <TabItem id="docker-compose" label="Docker Compose" icon="seti:docker">
        ...
        ```bash title="Docker CLI - Stop and remove containers"
        docker compose --env-file ./aspire-output/.env.Production -f ./aspire-output/docker-compose.yaml down
        ```
    </TabItem>
    <TabItem id="azure" label="Azure">
