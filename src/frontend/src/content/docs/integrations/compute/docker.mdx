...
For more flexible configuration, especially in CI/CD pipelines, you can use parameters:

```csharp title="C# — AppHost.cs"
var builder = DistributedApplication.CreateBuilder(args);

// Define parameters for registry configuration
var registryEndpoint = builder.AddParameter("registry-endpoint");
var registryRepository = builder.AddParameter("registry-repo");
...
You can then provide parameter values via environment variables or command-line arguments:

```bash
export Parameters__registry_endpoint=ghcr.io
export Parameters__registry_repository=your-github-username/your-repo
```
...
- [Aspire.Hosting.Docker API reference](https://learn.microsoft.com/en-us/dotnet/api/aspire.hosting.docker?view=dotnet-aspire-13.0)
