...
To prepare environment-specific configurations and build container images, use the `aspire do prepare-env` command:
...
- Output everything to the `prepare-env-{environment}` directory
...
- Outputs everything to the environment-specific directory
- Runs `docker compose up` against the generated files

#### Step 4: Clean Up Deployment (Optional)

To clean up a running Docker Compose deployment, use the `aspire do docker-compose-down-env` command:
