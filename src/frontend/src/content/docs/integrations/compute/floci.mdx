...
import flociIcon from '@assets/icons/floci-icon.svg';
import flociLightIcon from '@assets/icons/floci-icon-light.svg';
...
<Aside type="note">
  In C#, each cloud contributes a `WithReference` overload, so the compiler
...
<Aside type="note">
  For more information on data volumes and details on why they’re preferred over
...
Once a Floci resource is running and referenced by your service, use the matching cloud SDK as you normally would. `WithReference` in C# or the cloud-specific `withFloci*Reference` method in TypeScript injects the environment variables each SDK reads by convention. For example, against the AWS emulator:
...
<Aside type="note">
  Floci also has a built-in mechanism to launch the UI as a sidecar container on
...
<Aside type="note">
  In C#, `WithReference` is overloaded for each Floci UI cloud type. TypeScript
...
<Aside type="caution">
  The HTTPS endpoint APIs are experimental. The C# example suppresses
...
<Aside type="note">
  Plain HTTP remains the default until you explicitly configure a certificate.
...
Every environment variable injected by the integration carries an Aspire endpoint expression rather than a hard-coded address:
