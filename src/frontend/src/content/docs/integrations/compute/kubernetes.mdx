...
- **TypeScript / Python / Java / Go**: call `addManifest` (or `add_manifest`) on the resource. It takes the manifest's `apiVersion`, `kind`, and `metadata.name`, and returns a handle for setting labels, annotations, a namespace, and arbitrary field values using dot-notation paths.
...
The polyglot manifest handle returned by `addManifest` supports the following configuration methods:

| Method | Description |
|---|---|
| `withNamespace` / `with_namespace` | Sets the Kubernetes namespace for the manifest. |
| `withLabel` / `with_label` | Adds a metadata label to the manifest. |
| `withAnnotation` / `with_annotation` | Adds a metadata annotation to the manifest. |
| `withField` / `with_field` | Sets an arbitrary field using a dot-notation path (for example, `spec.maxReplicaCount`). |

Each manifest is emitted as a separate template file inside the generated Helm chart.

<Aside type="note">
`addManifest` (and `add_manifest`) is the recommended way to attach custom Kubernetes resources from polyglot AppHosts, where there is no strongly-typed model for arbitrary CRDs. C# AppHosts have direct access to the resource model, so they add `BaseKubernetesResource` instances to `AdditionalResources` instead.
</Aside>
