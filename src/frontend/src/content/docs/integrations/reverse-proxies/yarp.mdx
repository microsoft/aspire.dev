...
<Aside type="caution">
If the built assets aren't copied to `/app/wwwroot`, the container starts successfully but returns 404 for all static file requests. Look for the log message `The WebRootPath was not found: /wwwroot. Static files may be unavailable.` as an indicator that the path is misconfigured.
</Aside>
