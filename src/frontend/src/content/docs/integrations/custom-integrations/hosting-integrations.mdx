...
- `MailDevResource.ServiceDefaults`: The [service defaults](/get-started/csharp-service-defaults/) project for use in service-related projects.
...
Press <Kbd windows="Ctrl+C" mac="⌃+C" linux="Ctrl+C" /> to shut down the app (you can close the browser tab).
...
```xml title="MailDevResource.AppHost.csproj" 'IsAspireProjectResource="false"'
<ItemGroup>
...
```csharp title="MailDevResourceBuilderExtensions.cs"
...
```http title="HTTP request — subscribe" data-disable-copy
...
```http title="HTTP request — unsubscribe" data-disable-copy
...
```diff lang="csharp" title="MailDevResourceBuilderExtensions.cs"
public static IResourceBuilder<MailDevResource> AddMailDev(
    this IDistributedApplicationBuilder builder,
    string name,
    int? httpPort = null,
