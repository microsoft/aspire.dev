...
```diff lang="xml" title="MailKit.Client.csproj"
...
```csharp title="MailKitClientSettings.cs"
...
```csharp title="MailKitClientFactory.cs"
...
```csharp title="MailKitExtensions.cs"
using MailKit;
...
```csharp title="MailKitHealthCheck.cs"
...
Stop the application by selecting <Kbd windows="Ctrl+C" mac="⌃+C" linux="Ctrl+C" /> in the terminal window where the application is running, or by selecting the stop button in your IDE.
