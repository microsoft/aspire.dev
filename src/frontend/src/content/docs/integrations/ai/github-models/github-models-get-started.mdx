...
The `Aspire.Hosting.GitHub.Models` source, tests, and playground sample were removed from the Aspire repository in Aspire 13.6. The final `13.5.x` package remains published on NuGet for apps that already reference it, but it's hidden from `aspire add` and isn't rebuilt or serviced going forward.
