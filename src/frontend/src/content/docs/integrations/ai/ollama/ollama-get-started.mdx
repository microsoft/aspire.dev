...
Next, in the AppHost project, register and consume the Ollama integration using the `AddOllama` extension method to add the Ollama container to the application builder. You can then add models to the container, which downloads and run when the container starts, using the `AddModel` extension method:
...
In the `Program.cs` file of your client-consuming project, call the `AddOllamaApiClient` extension to register an `IOllamaClientApi` for use via the dependency injection container:
