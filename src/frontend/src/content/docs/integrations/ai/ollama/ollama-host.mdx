...
<Aside type="note">
  The TypeScript AppHost bindings for Ollama are provided by the Community Toolkit package and are not part of the core Aspire TypeScript SDK. The `.aspire/modules/aspire.mts` file in your project will include `addOllama` and related APIs after installation.
</Aside>
...
<Aside type="note">
  The TypeScript `withGPUSupport` API shape for vendor selection may differ from the C# overload. Refer to the generated `.aspire/modules/aspire.mts` in your project for the exact parameter structure.
</Aside>
