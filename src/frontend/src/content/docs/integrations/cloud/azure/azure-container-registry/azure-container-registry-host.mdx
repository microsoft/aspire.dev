...
[Private endpoints](https://learn.microsoft.com/azure/private-link/) disable public network access to the container registry and route all traffic through your virtual network. To add a private endpoint to an Azure Container Registry, call `AddPrivateEndpoint` on a subnet in your virtual network and pass the registry resource:
...
var acr = builder.AddAzureContainerRegistry("my-acr");
peSubnet.AddPrivateEndpoint(acr);
...
When you add a private endpoint to a container registry, Aspire:

- Automatically upgrades the registry SKU to **Premium** (required for private endpoint support).
- Disables public network access on the registry.
- Creates the private DNS zone `privatelink.azurecr.io` and links it to the virtual network.

<Aside type="caution">
  With public network access disabled, `aspire deploy` cannot push container
  images to the registry unless it runs from within the virtual network. Use a
  self-hosted Azure DevOps or GitHub Actions runner that is connected to the same
  VNet, or re-enable public network access on the registry for your deployment
  pipeline. For more information, see [Azure Virtual Network: private
  endpoints](/integrations/cloud/azure/azure-virtual-network/#add-private-endpoints).
</Aside>
