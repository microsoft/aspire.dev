...
> [!IMPORTANT]
> Check the model capabilities for both clients. Some models may only support chat completions, while others may only support embeddings. Ensure that the model you choose aligns with the client you intend to use.


### Add an Azure AI Inference Chat Completions client

In the _Program.cs_ file of your client-consuming project, use the `AddAzureChatCompletionsClient` method on any `IHostApplicationBuilder` to register an `ChatCompletionsClient` for dependency injection (DI).
...
- [What is Azure AI model inference?](https://azure.microsoft.com/services/ai-foundry/model-inference/) for details on Azure AI model interfence.
- [Dependency injection in .NET](https://azure.microsoft.com/dotnet/core/extensions/dependency-injection) for details on dependency injection.
- [The Azure AI Foundry SDK: C#](https://azure.microsoft.com/azure/ai-foundry/how-to/develop/sdk-overview?tabs=sync&pivots=programming-language-csharp).
...
For more information on the `IChatClient` and its corresponding library, see [Artificial intelligence in .NET](https://azure.microsoft.com/dotnet/core/extensions/artificial-intelligence).
...
For more information, see [Keyed services in .NET](https://azure.microsoft.com/dotnet/core/extensions/dependency-injection#keyed-services).
...
```csharp
public class ExampleService(IEmbeddingGenerator embeddingGenerator)
{
    public async Task<string> GetResponseAsync(string userMessage)
    {
        var response = await chatClient.CompleteAsync(userMessage);
...
For more information on the `IEmbeddingGenerator` and its corresponding library, see [Artificial intelligence in .NET](https://azure.microsoft.com/dotnet/core/extensions/artificial-intelligence).
