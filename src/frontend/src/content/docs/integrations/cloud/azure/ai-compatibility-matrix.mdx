...
|--|--|--|--|
| `Aspire.Hosting.Foundry` | ⚠️ Foundry Local only (preferred) | ⚠️ Partial | ⚠️ Legacy |
...
For Foundry Local, the [Aspire.OpenAI](https://www.nuget.org/packages/Aspire.OpenAI) package provides the recommended client integration. The `chat` deployment reference supplies the `CHAT_URI` and `CHAT_KEY` environment variables used in your service project:
