...
import aiIcon from '@assets/icons/azure-openai-icon.png';
...
|--|--|--|--|
| `Aspire.Hosting.Foundry` | ⚠️ Foundry Local only (preferred) | ⚠️ Partial | ❗ Legacy |
...
- ❗ **Legacy**: The client integration remains available for existing apps but is not recommended for new code.
...
<Aside type="note">
  **Aspire.Azure.AI.Inference** wraps the Azure AI Inference SDK, which Microsoft has retired in favor of the OpenAI SDK for calling Azure AI Foundry Models. Retain it only for existing applications that have not migrated. For cloud Foundry resources, see [Connect to Azure AI Foundry](/integrations/cloud/azure/azure-ai-foundry/azure-ai-foundry-connect/#connect-with-the-openai-sdk). See Microsoft's [migration guide](https://learn.microsoft.com/azure/foundry/how-to/model-inference-to-openai-migration) for details.
</Aside>

For more information, see [Microsoft Foundry integration](/integrations/cloud/azure/azure-ai-foundry/azure-ai-foundry-get-started/).
...
For Foundry Local, the [Aspire.OpenAI](https://www.nuget.org/packages/Aspire.OpenAI) package provides the recommended client integration. The `chat` deployment reference supplies the `CHAT_URI` and `CHAT_KEY` environment variables used in your service project. For cloud-provisioned Foundry resources that authenticate with managed identity, use the OpenAI SDK directly as shown in [Connect to Azure AI Foundry](/integrations/cloud/azure/azure-ai-foundry/azure-ai-foundry-connect/#connect-with-the-openai-sdk).
