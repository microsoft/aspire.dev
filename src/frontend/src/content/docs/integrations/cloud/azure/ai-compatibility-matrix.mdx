...
In general, use the OpenAI SDK directly (see each hosting integration's connect page) to call Azure-hosted models with Microsoft's currently recommended approach, or **Aspire.Azure.AI.Inference** for the Aspire client integration with the broadest Azure hosting support today. Use **Aspire.OpenAI** to connect directly to OpenAI.

### Microsoft Foundry

For Microsoft Foundry resources, the **Aspire.Azure.AI.Inference** client integration is the Aspire client integration with the best compatibility today, providing dependency injection, health checks, and telemetry across the diverse range of models available through Microsoft Foundry.

<Aside type="note">
  This integration wraps the Azure AI Inference SDK, which Microsoft has retired in favor of the OpenAI SDK for calling Azure AI Foundry Models. For direct (non-DI) calls, prefer the OpenAI SDK as shown in [Connect to Azure AI Foundry](/integrations/cloud/azure/azure-ai-foundry/azure-ai-foundry-connect/#connect-with-the-openai-sdk). See Microsoft's [migration guide](https://learn.microsoft.com/azure/foundry/how-to/model-inference-to-openai-migration) for details.
</Aside>
