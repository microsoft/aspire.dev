...
1. ### Model Azure SignalR Service in your AppHost
    ...
    Add the Azure SignalR Service hosting integration to your AppHost, then declare a SignalR resource and reference it from the apps that need it. The [Azure SignalR Service hosting integration](/integrations/cloud/azure/azure-signalr/azure-signalr-host/) article walks through every capability — service modes, emulator, connecting to existing resources, and Bicep customization — with side-by-side C# and TypeScript examples.
