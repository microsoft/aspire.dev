...
<Aside type="caution">
  When you call `AddAzureLogAnalyticsWorkspace` (or
...
<Aside type="note">
  TypeScript AppHost support for linking an Application Insights resource to a
  Log Analytics workspace isn't yet available.
</Aside>
...
<Aside type="note">
  The Azure Log Analytics hosting integration does not include a corresponding
