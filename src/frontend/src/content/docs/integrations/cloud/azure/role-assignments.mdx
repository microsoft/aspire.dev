...
The following example shows how to add a pipeline step that runs after `WellKnownPipelineSteps.BeforeStart` and enumerates all `AzureRoleAssignmentResource` instances that target a specific Azure resource:
...
builder.Pipeline.AddStep("inspect-role-assignments", async (context, ct) =>
{
    var roleAssignments = context.ApplicationModel.Resources
