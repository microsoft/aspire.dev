...
await storage.configureInfrastructure(infra => {
    ...
    const resources = infra.getProvisionableResources();
...
const storage = await builder.addAzureBicepResource(
    "storage",
    "./custom-storage.bicep");
await storage.withParameter("storageAccountName", "mystorageaccount");
