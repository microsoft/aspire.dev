...
When working with Azure integrations in Aspire, you often need to customize the generated infrastructure beyond the default settings. This page covers the shared Azure customization patterns that apply regardless of whether your compute ultimately deploys to Azure Container Apps, Azure App Service, or another Azure target.
...
## Common Azure customization patterns
...
### Read generated names and outputs

Azure resources expose normalized Bicep identifiers and output references for generated values. Use those references when another resource, deployment step, or app setting needs the generated Azure value.
...
builder.AddProject<Projects.Api>("api")
    ...
    .WithEnvironment("IDENTITY_NAME", identity.Resource.NameOutputReference);
...
Use `GetBicepIdentifier()` inside `ConfigureInfrastructure` or infrastructure resolvers when you need a stable identifier for generated Azure constructs. Use output references such as `NameOutputReference`, `ClientId`, or `GetOutput("...")` when you need the generated Azure value itself.
