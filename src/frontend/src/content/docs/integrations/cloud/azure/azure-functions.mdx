...
Deployment to Azure Container Apps (ACA) is supported using the SDK container publish function in `Microsoft.Azure.Functions.Worker.Sdk`. When deploying to ACA, KEDA-based auto-scaling rules will automatically be configured for your functions.

For more information about the Azure Functions integration with .NET Aspire, see [Azure Functions and .NET Aspire integration](https://learn.microsoft.com/azure/azure-functions/dotnet-aspire-integration).
