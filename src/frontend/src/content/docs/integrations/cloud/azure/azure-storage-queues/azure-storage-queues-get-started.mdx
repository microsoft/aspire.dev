...
1. ### Model Azure Queue Storage in your AppHost
    ...
    Add the Azure Queue Storage hosting integration to your AppHost, then declare an Azure Storage account, add a queue resource to it, and reference it from the apps that need to send or receive messages. The [Azure Queue Storage Hosting integration](/integrations/cloud/azure/azure-storage-queues/azure-storage-queues-host/) article walks through every capability — Azurite emulator configuration, custom ports, data volumes, persistent lifetime, connecting to existing accounts, and more — with side-by-side C# and TypeScript examples.
