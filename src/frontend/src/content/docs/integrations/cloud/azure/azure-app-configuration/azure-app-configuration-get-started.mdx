...
description: Understand how the Aspire Azure App Configuration integrations fit together — model an App Configuration store in your AppHost, then connect to it from any consuming app.
prev: false
---
...
1. ### Model Azure App Configuration in your AppHost
    ...
    Add the Azure App Configuration hosting integration to your AppHost, then declare a store resource and reference it from the apps that need to read configuration. The [Azure App Configuration Hosting integration](/integrations/cloud/azure/azure-app-configuration/azure-app-configuration-host/) reference walks through every capability — provisioning, emulator, role assignments, and infrastructure customization — with side-by-side C# and TypeScript examples.
