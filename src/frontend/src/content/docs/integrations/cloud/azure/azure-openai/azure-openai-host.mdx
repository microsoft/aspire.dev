...
- Adds an Azure OpenAI deployment resource. Replace `<deployment-name>` with a user-defined deployment name that matches the Azure OpenAI deployment you create. Replace `<model-name>` and `<model-version>` with values from the [Azure OpenAI available models](https://learn.microsoft.com/azure/ai-services/openai/concepts/models) documentation.
