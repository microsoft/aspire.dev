...
The Aspire Azure Database for PostgreSQL hosting integration models the PostgreSQL database server as the `AzurePostgresResource` type and PostgreSQL databases as the `AzurePostgresDatabaseResource` type. To access these types and APIs for expressing them within your AppHost project, install the [📦 Aspire.Hosting.Azure.PostgreSQL](https://www.nuget.org/packages/Aspire.Hosting.Azure.PostgreSQL) NuGet package:
...
The Aspire Azure Database for PostgreSQL client integration is used to connect to an Azure PostgreSQL database using the Npgsql client. To get started with the Aspire Azure Database for PostgreSQL client integration, install the [📦 Aspire.Npgsql](https://www.nuget.org/packages/Aspire.Npgsql) NuGet package.
