...
The `AddAzureAppConfiguration` method accepts an optional `Action<AzureAppConfigurationOptions> configureOptions` delegate that you use to configure the Azure App Configuration provider. This follows the same pattern as the non-Aspire `Microsoft.Extensions.Configuration.AzureAppConfiguration` package, but without needing to call `options.Connect`—Aspire automatically connects the provider to the resource produced by the AppHost.
...
App Configuration doesn't load feature flags by default. To load feature flags, use the `configureOptions` delegate to call `UseFeatureFlags()` when calling `builder.AddAzureAppConfiguration`.
