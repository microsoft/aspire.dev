...
description: Understand how the Aspire Azure Cosmos DB integrations fit together — model a Cosmos DB account in your AppHost, then connect to it from any consuming app.
prev: false
---
...
1. ### Model Azure Cosmos DB in your AppHost
    ...
    Add the Azure Cosmos DB hosting integration to your AppHost, then declare a Cosmos DB account, add databases and containers, and reference them from the apps that need them. The [Azure Cosmos DB Hosting integration](/integrations/cloud/azure/azure-cosmos-db/azure-cosmos-db-host/) reference walks through every capability — running the emulator, adding databases and containers, partition keys, access key authentication, and infrastructure customization — with side-by-side C# and TypeScript examples.
