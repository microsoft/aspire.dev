...
- `WithReference(hub)` injects the task hub connection information into the Azure Functions project.

### Use an existing scheduler

To connect to an already-deployed Durable Task Scheduler instance instead of provisioning a new one, use `RunAsExisting`:
...
var scheduler = builder.AddDurableTaskScheduler("scheduler")
    .RunAsExisting("<connection-string>");

#pragma warning restore ASPIREDURABLETASK001
```

You can also pass an `IResourceBuilder<ParameterResource>` to supply the connection string from a parameter:
...
var connectionString = builder.AddParameter("schedulerConnectionString", secret: true);
