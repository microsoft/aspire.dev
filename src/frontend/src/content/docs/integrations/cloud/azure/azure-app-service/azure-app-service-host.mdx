...
## Deploy supported resources to an App Service environment

After adding an App Service environment, Aspire automatically deploys supported compute resources in that environment as Azure App Service websites. Today that includes web-facing project resources and Dockerfile-backed web containers. Backing services such as databases, caches, and brokers should use managed Azure services instead of App Service websites.
...
- Targets the `api` project to the App Service environment automatically.
...
<Aside type="note">
  When App Service deployment configures a managed identity for a resource,
  Aspire automatically sets the `AZURE_TOKEN_CREDENTIALS` environment variable
  to `managedidentity`. This ensures that `DefaultAzureCredential`
  ...
  If you need to use a different credential type, remove this app setting by
  customizing the generated website with `PublishAsAzureAppServiceWebsite`. For
...
Use `PublishAsAzureAppServiceWebsite` when you want to customize the generated Azure App Service website or deployment slot. It isn't required for a standard App Service deployment.
