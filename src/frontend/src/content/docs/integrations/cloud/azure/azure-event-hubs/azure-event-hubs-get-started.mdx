...
- **Zero-config local development.** Aspire can run an Azure Event Hubs emulator locally using the `mcr.microsoft.com/azure-messaging/eventhubs-emulator` container image, so you can develop and test without an Azure subscription.
...
1. ### Model Azure Event Hubs in your AppHost
    ...
    Add the Azure Event Hubs hosting integration to your AppHost, then declare a namespace, one or more hubs, optional consumer groups, and reference them from the apps that need to send or receive events. The [Azure Event Hubs Hosting integration](/integrations/cloud/azure/azure-event-hubs/azure-event-hubs-host/) article walks through every capability — adding hubs, consumer groups, the local emulator, provisioning infrastructure, and role assignments — with side-by-side C# and TypeScript examples.
