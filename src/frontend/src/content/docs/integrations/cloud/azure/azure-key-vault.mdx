...
<InstallPackage packageName="Aspire.Hosting.Azure.KeyVault" shortName="azure-keyvault" />
...
<Aside type="caution">
By default, `AddAzureKeyVault` configures a [Key Vault Administrator built-in role](https://learn.microsoft.com/role-based-access-control/built-in-roles/security#key-vault-administrator).
</Aside>
...
{
    var keyVaultService = infra.GetProvisionableResources()
                               .OfType<KeyVaultService>()
                               .Single();

    keyVaultService.Sku = new KeyVaultSku
    {
        Family = KeyVaultSkuFamily.A,
        Name = KeyVaultSkuName.Standard
    };
    keyVaultService.Properties.EnableRbacAuthorization = true;
    keyVaultService.Tags["ExampleKey"] = "Example value";
});
```
