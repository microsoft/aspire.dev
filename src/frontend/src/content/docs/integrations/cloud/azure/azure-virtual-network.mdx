...
<Tabs syncKey="aspire-lang">
  <TabItem id="csharp" label="C#">
    ```csharp title="C# — AppHost.cs" var vnet =
    builder.AddAzureVirtualNetwork("vnet", "10.1.0.0/16"); ```
  </TabItem>
  <TabItem id="typescript" label="TypeScript">
    ```typescript title="TypeScript — apphost.ts" const vnet = await
    builder.addAzureVirtualNetwork("vnet", "10.1.0.0/16"); ```
  </TabItem>
</Tabs>
...
<Tabs syncKey="aspire-lang">
  <TabItem id="csharp" label="C#">
    ```csharp title="C# — AppHost.cs" var pip =
    builder.AddPublicIPAddress("nat-pip"); var natGateway =
    builder.AddNatGateway("nat") .WithPublicIPAddress(pip); ```
  </TabItem>
  <TabItem id="typescript" label="TypeScript">
    ```typescript title="TypeScript — apphost.ts" const pip = await
    builder.addPublicIPAddress("nat-pip"); const natGateway = await
    builder.addNatGateway("nat") .withPublicIPAddress(pip); ```
  </TabItem>
</Tabs>
