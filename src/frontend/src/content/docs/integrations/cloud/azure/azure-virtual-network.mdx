...
You can also pass another supported Azure resource provider path as `serviceName` if the service doesn't have a well-known constant. Aspire models and emits one service delegation for a subnet. Repeated `WithServiceDelegation` calls, or combining `WithServiceDelegation` with `WithDelegatedSubnet`, replace the previously modeled delegation with the latest value (last-write-wins).
