...
You can also pass any other Azure resource provider path as `serviceName` if you need to delegate to a service that doesn't have a well-known constant. A subnet can carry only one active delegation—calling `WithServiceDelegation` again, or combining it with `WithDelegatedSubnet`, replaces any previously configured delegation with the latest value (last-write-wins).
