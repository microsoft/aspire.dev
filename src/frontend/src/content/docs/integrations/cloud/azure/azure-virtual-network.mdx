...
<Aside type="note">
  The Azure Virtual Network integration is a **hosting** and **publish-only**
...
{
  "packages": {
    "Aspire.Hosting.Azure.Network": "13.5.0"
  }
}
```
...
<Aside type="note">
  The `SecurityRuleProtocol`, `SecurityRuleDirection`, and `SecurityRuleAccess`
...
### Delegate subnets to compute environments
...
```csharp title="AppHost.cs"
var vnet = builder.AddAzureVirtualNetwork("vnet");
var acaSubnet = vnet.AddSubnet("aca-subnet", "10.0.0.0/24");
var appServiceSubnet = vnet.AddSubnet("app-service-subnet", "10.0.1.0/24");
...
```typescript title="apphost.mts"
const vnet = await builder.addAzureVirtualNetwork("vnet");
const acaSubnet = await vnet.addSubnet("aca-subnet", "10.0.0.0/24");
const appServiceSubnet = await vnet.addSubnet("app-service-subnet", "10.0.1.0/24");
...
Each compute environment delegates the subnet to its own Azure service (for example, App Service delegates to `Microsoft.Web/serverFarms`). Regional virtual network integration affects outbound traffic only—it doesn't make ingress private and doesn't enable Route All.
...
<TabItem id="typescript" label="TypeScript">
```typescript title="apphost.mts"
import { AzureSubnetServiceDelegations } from './.aspire/modules/aspire.mjs';

const vnet = await builder.addAzureVirtualNetwork("vnet");
const subnet = await vnet.addSubnet("aci-subnet", "10.0.0.0/23");

await subnet.withServiceDelegation(AzureSubnetServiceDelegations.ContainerInstances);

// Or specify a custom delegation name.
await subnet.withServiceDelegation(
    AzureSubnetServiceDelegations.ContainerInstances,
    { name: "aci-delegation" });
````
...
You can also pass another supported Azure resource provider path as `serviceName` if the service doesn't have a well-known constant. For a given subnet, use either a compute environment's `WithDelegatedSubnet` helper or `WithServiceDelegation` directly. Repeated `WithServiceDelegation` calls collapse prior service-delegation annotations so only the latest value remains (last-write-wins). `WithDelegatedSubnet` throws if the subnet's effective delegation is for a different Azure service instead of overwriting the conflicting delegation.
...
<Aside type="caution">
  When public network access is disabled on the registry (the default when a
