...
`PublishAsConnectionString` is obsolete as of Aspire 9.5. It only changes the manifest representation and has no effect on other publishers. To express different run and publish behavior, use [`AddConnectionString`](/fundamentals/external-parameters/#connection-string-values) with the execution context:
