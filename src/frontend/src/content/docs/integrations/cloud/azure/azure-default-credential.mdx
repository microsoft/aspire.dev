...
Starting in Aspire 13.2, the default credential behavior was updated to follow [Azure SDK authentication best practices](https://learn.microsoft.com/dotnet/azure/sdk/authentication/credential-chains?tabs=dac#usage-guidance-for-defaultazurecredential). Rather than using the parameterless `DefaultAzureCredential` constructor, Aspire detects the runtime environment and selects the most appropriate credential:

- **Running in Azure**: When Aspire detects that the application is running in Azure (for example, by the presence of the `AZURE_CLIENT_ID` environment variable), it uses `ManagedIdentityCredential`. This ensures deterministic, efficient authentication in production.
