...
var dotnetWeatherAgent = builder.AddProject<Projects.Agent>("agent")
...
For .NET hosted agents, add your Service Defaults project reference and call `builder.AddServiceDefaults()` in the hosted-agent app so logs, metrics, traces, HTTP instrumentation, and OTLP export flow into the Aspire dashboard. Then enable telemetry on the Foundry Responses chat client that backs the MAF agent
...
![Aspire dashboard showing prompt and hosted agent resources with Send Message commands](../../../../../../assets/integrations/cloud/azure/azure-ai-foundry/agent-send-message.png)

![Hosted agent responses invocation result in the Aspire dashboard](../../../../../../assets/integrations/cloud/azure/azure-ai-foundry/agent-responses-result.png)
