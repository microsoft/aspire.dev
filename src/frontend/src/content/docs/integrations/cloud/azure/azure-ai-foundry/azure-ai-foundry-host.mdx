...
When the first `ContainerProtocolVersions` entry uses the `invocations` protocol, Aspire:
...
:::note
TypeScript AppHost support for selecting hosted-agent protocol versions is not yet available. The exported hosted-agent options object does not currently expose `ContainerProtocolVersions`.
:::
