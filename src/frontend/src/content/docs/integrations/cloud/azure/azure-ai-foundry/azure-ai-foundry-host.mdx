...
<Aside type="note">
  Cross-compute-environment endpoint resolution requires the
  `ReferenceRoleAssignmentAnnotation` type, which is marked
  `[Experimental("ASPIREAZURE003")]`. Suppress this diagnostic to use the
  feature. For details, see [ASPIREAZURE003](/diagnostics/aspireazure003/).
</Aside>

Before this fix, `aspire deploy` threw a "context not found" error when a
resource resolved an endpoint whose owning resource lived in a different compute
environment. The fix affects the Azure Container Apps, Azure App Service, and
Kubernetes publishers.
...
```csharp title="C# — AppHost.cs"
#pragma warning disable ASPIREAZURE003 // ReferenceRoleAssignmentAnnotation is experimental and required for cross-environment RBAC wiring.
