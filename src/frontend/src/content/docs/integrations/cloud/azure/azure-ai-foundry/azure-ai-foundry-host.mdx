...
<Aside type="caution">
  When you call `AddFoundry`, it implicitly calls `AddAzureProvisioning`—which
...
## Reference a hosted agent across compute environments

When your application spans multiple compute environments — for example, a web app deployed to Azure Kubernetes Service (AKS) calling a hosted agent deployed to a Foundry project environment — Aspire automatically resolves the cross-environment endpoint reference and wires the required Azure RBAC grant.

<Aside type="note">
  The RBAC auto-wiring described in this section uses
  `ReferenceRoleAssignmentAnnotation` internally. The public AppHost APIs shown
  here don't require you to suppress `ASPIREAZURE003`; endpoint resolution is
  handled separately by the compute-environment publishers. For details, see
  [ASPIREAZURE003](/diagnostics/aspireazure003/).
</Aside>

This support applies when publishing to Azure Container Apps, Azure App Service,
and Kubernetes. Endpoint references whose owning resource lives in a different
compute environment are resolved through that owning environment.

In addition, Aspire now auto-wires the consumer's managed identity and grants
the **Azure AI User** role on the owning Foundry account. Previously, developers
had to run `az role assignment create` manually after every deploy.

The AKS example below requires the [📦 Aspire.Hosting.Azure.Kubernetes](/integrations/cloud/azure/aks/) integration in addition to [📦 Aspire.Hosting.Foundry](https://www.nuget.org/packages/Aspire.Hosting.Foundry).
...
// The agent is published to the Foundry project compute environment.
var agent = builder.AddProject<Projects.Agent>("agent")
    .WithHttpEndpoint()
    .WithExternalHttpEndpoints()
    .AsHostedAgent(project);

// The web app is deployed to AKS and references the cross-environment agent.
// Aspire resolves the endpoint and auto-provisions the Azure AI User role
// on the Foundry account — no manual `az role assignment create` required.
...
The preceding code demonstrates the two behaviors Aspire provides automatically:

- **Cross-environment endpoint resolution**: The `web` resource deployed to AKS can reference the `agent` endpoint even though the agent lives in the Foundry project compute environment. Same-environment references are unaffected and continue to resolve locally.
- **RBAC auto-wiring**: Because `web` references a hosted agent, Aspire stamps the consumer's managed identity with the **Azure AI User** role on the owning Foundry account. This is the least-privilege grant required to invoke a Foundry hosted agent. It is only emitted when the reference is present, and it is suppressible via an explicit `WithRoleAssignments(...)` call in C# or `withFoundryRoleAssignments(...)` in TypeScript.
