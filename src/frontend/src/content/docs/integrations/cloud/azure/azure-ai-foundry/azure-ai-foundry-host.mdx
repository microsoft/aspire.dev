...
By default, Aspire configures a hosted agent for the `responses` protocol. If your agent implements the `invocations` protocol, configure the protocol versions when calling `AsHostedAgent`. The selected protocol affects both run mode (dashboard URLs and the **Send Message** command) and publish mode (the `container_protocol_versions` field in the Foundry hosted agent definition).
...
When the first `ContainerProtocolVersions` entry (C#) or the first `protocols` entry (TypeScript) uses the `invocations` protocol, Aspire:
...
:::note
C# AppHosts use `AsHostedAgent` with an `Action<HostedAgentConfiguration>` callback to access the full Azure SDK configuration surface, including `ContainerProtocolVersions`. TypeScript and other polyglot AppHosts use the `protocols` property on the hosted agent options object.
:::
...
Aspire also makes the declared agents easy to try from the dashboard. Prompt agents get a **Send Message** command from `AddPromptAgent(...)`. Hosted agents get a highlighted **Send Message** command that posts to the protocol endpoint (`/responses` by default, or `/invocations` when the invocations protocol is configured), plus dashboard links for the protocol endpoint, `/liveness`, and `/readiness`.
