...
<Aside type="note">
  If you're upgrading from earlier previews, replace the
  `Aspire.Hosting.Azure.AIFoundry` package with `Aspire.Hosting.Foundry`.
</Aside>
...
You can also use the `FoundryModel` catalog instead of specifying the model name, version, and format explicitly:
