...
Next, in the AppHost project, create an Microsoft Foundry resource and pass it to the consuming client projects:
...
The preceding code adds an Microsoft Foundry resource named `foundry` with a deployment named `chat` using the Phi-4 model to the AppHost project. The `WithReference` method passes the connection information to the `ExampleProject` project.
