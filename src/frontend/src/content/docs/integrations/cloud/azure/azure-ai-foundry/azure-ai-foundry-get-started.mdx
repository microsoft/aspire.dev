...
The preceding code adds a Microsoft Foundry resource named `foundry` with a deployment named `chat` using the Phi-4 model to the AppHost project. The `WithReference` method passes the connection information to the `ExampleProject` project.
