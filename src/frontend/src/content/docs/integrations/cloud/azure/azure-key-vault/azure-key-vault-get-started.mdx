...
description: Understand how the Aspire Azure Key Vault integrations fit together — model a Key Vault resource in your AppHost, then connect to it from any consuming app.
---
...
1. ### Model Azure Key Vault in your AppHost
    ...
    Add the Azure Key Vault hosting integration to your AppHost, then declare a Key Vault resource and reference it from the apps that need to access secrets. The [Azure Key Vault Hosting integration](/integrations/cloud/azure/azure-key-vault/azure-key-vault-host/) reference walks through every capability — connecting to existing vaults, role assignments, secret references, and infrastructure customization — with side-by-side C# and TypeScript examples.
