...
The Aspire Azure AI Foundry client integration is used to connect to an Azure AI Foundry project. To get started with the Aspire Azure AI Foundry client integration, install the [📦 Aspire.Azure.AI.Foundry](https://www.nuget.org/packages/Aspire.Azure.AI.Foundry) NuGet package.
...
If you're interested in using the [IChatClient](https://learn.microsoft.com/dotnet/api/microsoft.extensions.ai.ichatclient) interface, with the Azure AI Foundry client, simply chain the `AddChatClient` API to the `AddAzureChatCompletionsClient` method:
