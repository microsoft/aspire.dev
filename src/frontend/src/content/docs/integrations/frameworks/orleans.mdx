...
Orleans clients communicate with grains hosted on Orleans servers. In an Aspire app, for example, you might have a frontend web site that calls grains in an Orleans cluster. Reference the Orleans resource from your Orleans client using `WithReference(orleans.AsClient())`:
