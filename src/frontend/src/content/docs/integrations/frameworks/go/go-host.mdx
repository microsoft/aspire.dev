...
{
  "packages": {
    "Aspire.Hosting.Go": "%ASPIRE_VERSION%"
  }
}
```
...
For Aspire 13.4 and later, use the official package as the recommended approach for Go hosting:
...
clients, keep the server running after a client disconnects, or configure additional Delve flags,
pass a `DelveServerOptions` instance:
...
:::note
`WithDelveServer(int port = 2345)` remains available as an obsolete overload for binary
compatibility and preserves its original single-client behavior. New code should use
`WithDelveServer()` or `WithDelveServer(DelveServerOptions)` instead.
:::
