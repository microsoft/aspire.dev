...
const jarPath = await app.jarPath();
await app.setJarPath('target/java-api-optimized.jar');
...
const app = await builder
  .addJavaContainerApp('java-api', 'ghcr.io/contoso/java-api', '1.0')
  .withJvmArgs(['-Djava.awt.headless=true'])
  .withHttpEndpoint({ targetPort: 8080, env: 'SERVER_PORT' });
...
The Java application is responsible for reading `SERVER_PORT` and listening on the endpoint target port. `WithHttpHealthCheck` / `withHttpHealthCheck` makes Aspire poll the configured path. The application must expose that path; Spring Boot Actuator exposes `/actuator/health` when `spring-boot-starter-actuator` is on the classpath. Executable resources run `java` unless a Maven or Gradle task changes the command in run mode; build resources and application resources expose normal dashboard lifecycle controls.

### Pass PostgreSQL connection information

After you add a [PostgreSQL database resource](/integrations/databases/postgres/postgres-host/), pass its connection expressions to Spring Boot. The JDBC connection string contains the host, port, and database name; the username and password are separate expressions.
...
var app = builder.AddJavaApp("java-api", "../java-api")
    .WithMavenGoal("spring-boot:run")
    .WithEnvironment("SPRING_DATASOURCE_URL", database.Resource.JdbcConnectionString)
...
const app = await builder
  ...
  .withEnvironment(
    ...
    await database.jdbcConnectionString()
  )
  .withEnvironment(
    'SPRING_DATASOURCE_USERNAME',
    await postgres.userNameReference()
  )
  .withEnvironment(
    'SPRING_DATASOURCE_PASSWORD',
    await postgres.passwordParameter()
  );
