...
const api = await builder.addDenoApp('deno-api', 'main.ts', {
  workingDirectory: '../deno-api',
  permissionFlags: ['--allow-env', '--allow-net'],
  args: ['--verbose'],
});
...
const web = await builder.addDenoTask('web', {
  workingDirectory: '../web',
  taskName: 'dev',
  args: ['--host', '0.0.0.0'],
});
...
In C#, an overload also accepts a callback that configures the installer resource. That callback overload is not exported to TypeScript AppHosts.
