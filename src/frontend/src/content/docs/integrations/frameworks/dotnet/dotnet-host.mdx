...
:::note
`Aspire.Hosting.Dotnet` is a prerelease package. `AddDotnetProject` / `addDotnetProject` no longer requires suppressing the `ASPIREDOTNETPROJECT001` diagnostic, but the API surface may still change in future releases.
:::
...
{
  "packages": {
    "Aspire.Hosting.Dotnet": "%ASPIRE_VERSION%"
  }
}
```
...
builder.AddDotnetProject("inventoryservice", @"..\InventoryService.cs", options =>
{
