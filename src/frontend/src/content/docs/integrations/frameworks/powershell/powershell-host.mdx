...
Use the least-permissive language mode appropriate for the scripts you run. The TypeScript API accepts the language-mode name as a string. Invalid names fail while Aspire constructs the AppHost.
...
Use `WithArgs` / `withArgs` to bind values to a PowerShell `param()` block. C# accepts values of any type; the TypeScript export accepts strings.
...
The PowerShell-specific pool `withReference` API is not exported to TypeScript. You can use standard AppHost resource relationships such as `waitFor`, but they do not create a PowerShell connection-string variable.
