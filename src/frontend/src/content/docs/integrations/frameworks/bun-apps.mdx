...
// builder.AddBunApp("bun-api", "server.ts");

// Official Aspire.Hosting.JavaScript API:
builder.AddBunApp("bun-api", "../bun-app", "server.ts");
```

If you pass arguments positionally, add the app directory between the resource name and script path. TypeScript AppHosts continue to use `addBunApp`, but the binding is now generated from `Aspire.Hosting.JavaScript`.
