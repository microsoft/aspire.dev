...
const api = process.env.API_HTTP;
...
```typescript title="Next.js — app/api/weather/route.ts"
...
export default defineEventHandler(async () => {
  ...
  return $fetch(`${config.apiUrl}/api/weather`);
});
```
...
```typescript title="SvelteKit — src/routes/api/weather/+server.ts"
import { env } from '$env/dynamic/private';

export async function GET() {
  const response = await fetch(`${env.API_URL}/api/weather`);
  const weather = await response.json();
