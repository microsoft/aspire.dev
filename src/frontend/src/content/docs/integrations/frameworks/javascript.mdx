...
When you use `WithReference` to connect a JavaScript app to other resources, Aspire injects environment variables with the connection details. For example:
...
Access the connection string in your JavaScript code:

```javascript title="JavaScript — app.js"
const connectionString = process.env.ConnectionStrings__mydb;
```

For more details on how resource names map to environment variable names, see [Environment variables](/fundamentals/environment-variables/).
