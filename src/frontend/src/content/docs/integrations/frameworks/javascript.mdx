...
builds the front-end assets, but another resource must serve those built files in
production.
:::
...
<Aside type="tip">
  ...
  configuration to SPA front ends](#pass-runtime-configuration-to-spa-frontends).
</Aside>

## Pass runtime configuration to SPA front ends
...
1. **Pass the parameter to the API in the AppHost**
   ...
   Define the parameter in the AppHost and pass it to the API project using `WithEnvironment`. Then reference the API from the front end so it can call the endpoint:
...
2. **Expose a config endpoint in the API**
   ...
   <Aside type="caution" title="Only expose public configuration">
     ...
     API keys, or connection strings via a front-end config endpoint.
   </Aside>

3. **Fetch the config value in the SPA**

   In your front-end application, fetch the configuration value at startup instead of reading from `import.meta.env`:
...
4. Produces front-end build output that another resource can include or serve
...
server for your front end.

During publish, Aspire uses them to build front-end assets. To deploy that
front end, you must choose another resource to serve those built files in
production.

Adding `AddJavaScriptApp` or `AddViteApp` plus `.WithReference(...)` is not
enough to make the front end independently deployable.
:::

:::note
Local Vite proxy and route behavior does not automatically become production
behavior. If your front end depends on Vite development-server routing or proxy
...
<LearnMore>
  ...
  `AddViteApp`, including who serves the built front end in production, see
