...
const nodeApp = await builder
  .addNodeApp('node-app', './node-app', 'server.js')
  .withHttpEndpoint({ port: 3000, env: 'PORT' })
  .withReference(api);
...
1. **Pass the parameter to the API in the AppHost**
   ...
   Define the parameter in the AppHost and pass it to the API project using `WithEnvironment` / `withEnvironment`. Then reference the API from the frontend so it can call the endpoint:
