...
`AddNextJsApp` is marked `[Experimental]` and requires suppressing the `ASPIREEXTENSION001` diagnostic in your project.
:::
...
var api = builder.AddProject<Projects.ExampleProject>("api");
...
To opt out of the configuration validation step, call `.DisableBuildValidation()`:
