...
This page describes how consuming apps connect to a Garnet resource that's already modeled in your AppHost. For the AppHost API surface — adding a Garnet resource, data volumes, persistence, parameters, and more — see [Garnet Hosting integration](../garnet-host/).
...
<Tabs syncKey="garnet-consuming-lang">
<TabItem label="C#">
...
<Aside type="tip">
The `connectionName` must match the Garnet resource name from the AppHost. For more information, see [Add Garnet resource](../garnet-host/#add-garnet-resource).
</Aside>
...
Install [`redis-py`](https://github.com/redis/redis-py), the official Python client for RESP servers including Garnet:
...
client.ping()
# Use client to interact with Garnet...
```
...
<Aside type="tip">
    If your app expects specific environment variable names different from the Aspire defaults, you can pass individual connection properties from the AppHost. See [Pass custom environment variables](../garnet-host/#pass-custom-environment-variables) in the Hosting integration reference.
</Aside>
