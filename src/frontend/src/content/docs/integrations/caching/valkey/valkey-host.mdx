...
{
  "packages": {
    "Aspire.Hosting.Valkey": "%ASPIRE_VERSION%"
  }
}
```
...
## Add Valkey resource with a REPL command

The `WithRepl` method adds an opt-in **REPL** command to the Valkey resource in the Aspire dashboard. Selecting the command while the container is running opens an authenticated `valkey-cli` session in the dashboard's terminal dock—no local Valkey client installation is required:
