...
<ContainerImages package="Aspire.Hosting.Redis" only="RedisInsight" />
...
<ContainerImages package="Aspire.Hosting.Redis" only="Redis Commander" />
...
## Add Redis resource with a REPL command

The `WithRepl` method adds an opt-in **REPL** command to the Redis resource in the Aspire dashboard. Selecting the command while the container is running opens an authenticated `redis-cli` session in the dashboard's terminal dock—no local Redis client installation is required:
