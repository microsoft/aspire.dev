...
Use `AddConnectionString` to reference an externally managed Redis instance:
...
<TabItem id="typescript" label="TypeScript">
To connect to an existing Redis instance from a TypeScript AppHost, use [`builder.addConnectionString(...)`](/get-started/app-host/) with a parameter and reference that connection string from your consuming apps instead.
</TabItem>
</Tabs>
