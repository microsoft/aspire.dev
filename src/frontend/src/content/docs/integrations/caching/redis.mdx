---
title: Redis
description: Learn how to use the Redis integration for caching, which includes both hosting and client integrations.
---
...
[Redis](https://redis.io/) is the world's fastest data platform for caching, vector search, and NoSQL databases. The Redis integration enables you to connect to existing Redis instances, or create new instances from .NET with the `docker.io/library/redis` container image.
...
<InstallPackage packageName="Aspire.Hosting.Redis" shortName="redis" />
...
<Aside type="tip">
If you'd rather connect to an existing Redis instance, call `AddConnectionString` instead. For more information, see [Reference existing resources](/fundamentals/external-parameters/#reference-existing-resources).
</Aside>
