...
`AddDotnetProjectBlazorGateway` / `addDotnetProjectBlazorGateway` adds a Blazor Gateway that hosts a [`DotnetProjectResource`](/integrations/frameworks/dotnet/dotnet-host/) (a C# project or file-based app added with `AddDotnetProject` / `addDotnetProject`) instead of the standard `ProjectResource`-based gateway. Use the distinct `WithBlazorClientApp` overload on this resource to attach a Blazor WebAssembly client app:
