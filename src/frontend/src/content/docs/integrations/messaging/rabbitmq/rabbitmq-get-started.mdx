...
description: Understand how the Aspire RabbitMQ integrations fit together — model a RabbitMQ resource in your AppHost, then connect to it from any consuming app.
---
...
1. ### Model RabbitMQ in your AppHost
    ...
    Add the RabbitMQ hosting integration to your AppHost, then declare a RabbitMQ resource and reference it from the apps that need to talk to the broker. The [RabbitMQ Hosting integration](/integrations/messaging/rabbitmq/rabbitmq-host/) reference walks through every capability — data volumes, data bind mounts, custom parameters, and the management plugin — with side-by-side C# and TypeScript examples.
