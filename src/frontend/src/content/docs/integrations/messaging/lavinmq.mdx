...
<InstallPackage packageName="CommunityToolkit.Aspire.Hosting.LavinMQ" shortName="lavinmq" />
...
In the _:::no-loc text="Program.cs":::_ file of your client-consuming project, call the `AddRabbitMQClient` extension method to register an `IConnection` for use through dependency injection. The method takes a connection name parameter:
