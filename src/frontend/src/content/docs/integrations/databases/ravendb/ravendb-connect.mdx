...
<Aside type="tip">
    If your app expects specific environment variable names different from the Aspire defaults, you can pass individual connection properties from the AppHost using `WithEnvironment`. For an example, see [Pass custom environment variables](/get-started/app-host/#pass-custom-environment-variables) in the AppHost reference.
</Aside>
