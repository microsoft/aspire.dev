...
<Aside type="note">
  The `RavenDBServerSettings`-based overload of `AddRavenDB` is only available from C# AppHosts. It includes certificate and licensing configuration that is not compatible with the [polyglot](/get-started/glossary/#polyglot) TypeScript AppHost.
</Aside>
