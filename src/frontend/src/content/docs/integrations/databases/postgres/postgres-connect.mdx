...
When you reference a PostgreSQL resource from the AppHost with `withReference`, Aspire automatically injects connection information into the consuming application as environment variables. This page shows how to read those variables and connect to PostgreSQL from any language.
