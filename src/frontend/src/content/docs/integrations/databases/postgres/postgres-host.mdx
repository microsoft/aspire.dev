...
{
  "packages": {
    "Aspire.Hosting.PostgreSQL": "%ASPIRE_VERSION%"
  }
}
```
...
<ContainerImages package="Aspire.Hosting.PostgreSQL" only="pgAdmin" />
...
<ContainerImages package="Aspire.Hosting.PostgreSQL" only="pgweb" />
...
## Add PostgreSQL resource with a REPL command

The `WithRepl` method adds an opt-in **REPL** command to the PostgreSQL server resource in the Aspire dashboard. Selecting the command while the container is running opens `psql` in the dashboard's terminal dock, already connected to the `postgres` database with the resource's configured credentials—no local PostgreSQL client installation is required:
