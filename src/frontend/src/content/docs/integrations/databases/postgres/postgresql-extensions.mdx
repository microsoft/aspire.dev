...
Use `WithDbx` / `withDbx` to add a dbx child resource configured for the PostgreSQL server:
...
const postgres = await builder.addPostgres('postgres');
await postgres.withDbx({ containerName: 'postgres-dbx' });
...
Use `WithFlywayRepair(flyway)` in C# or `withFlywayRepair("flyway-repair", "./migrations")` in TypeScript when you need Flyway's `repair` command instead of `migrate`.
