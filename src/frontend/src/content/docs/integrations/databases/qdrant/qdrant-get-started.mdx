...
description: Understand how the Aspire Qdrant integrations fit together — model a Qdrant resource in your AppHost, then connect to it from any consuming app.
---
...
1. ### Model Qdrant in your AppHost
    ...
    Add the Qdrant hosting integration to your AppHost, then declare a Qdrant resource and reference it from the apps that need to use the vector database. The [Qdrant Hosting integration](/integrations/databases/qdrant/qdrant-host/) reference walks through every capability — API key parameters, data volumes, data bind mounts, and more — with side-by-side C# and TypeScript examples.
