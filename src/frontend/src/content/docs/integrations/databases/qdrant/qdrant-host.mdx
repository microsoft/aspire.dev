...
This article includes full details about the Aspire Qdrant Hosting integration, which models Qdrant server resources as the `QdrantServerResource` type. To access these types and APIs, you need to install the Qdrant Hosting integration in your AppHost project.
...
The Qdrant resource includes a default API key which is randomly generated. Qdrant supports configuration-based API keys. When you want to provide an API key explicitly, you can provide it as a parameter:
