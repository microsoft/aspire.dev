...
For the full emulator configuration reference — gateway port, data volumes, partition count, and the Linux-based preview emulator — see [Azure Cosmos DB Hosting integration](/integrations/cloud/azure/azure-cosmos-db/azure-cosmos-db-host/#add-azure-cosmos-db-emulator-resource).
