...
In this introduction, you'll see how to install and use the Aspire Azure Cosmos DB integrations in a simple configuration. If you already have this knowledge, see [Azure Cosmos DB Hosting integration](../azure-cosmos-db-host/) for full reference details.
...
<Aside type="tip">
    This is the simplest implementation of Azure Cosmos DB resources in the AppHost using the emulator. There are many more options you can choose from to address your requirements. For full details, see [Azure Cosmos DB Hosting integration](../azure-cosmos-db-host/).
</Aside>
...
Aspire exposes each property as an environment variable named `[RESOURCE]_[PROPERTY]`. For instance, the `AccountEndpoint` property of a resource called `mycontainer` becomes `MYCONTAINER_ACCOUNTENDPOINT`.

Use the `GetValue()` method to obtain these environment variables in consuming projects:

```csharp title="C# - Obtain configuration properties"
string accountEndpoint = builder.Configuration.GetValue<string>("MYCONTAINER_ACCOUNTENDPOINT");
string databaseName = builder.Configuration.GetValue<string>("MYCONTAINER_DATABASENAME");
```

<Aside type="tip">
    For the full set of properties that Aspire injects, see [Properties of the Cosmos DB resources](../azure-cosmos-db-client/#properties-of-the-cosmos-db-resources).
</Aside>
