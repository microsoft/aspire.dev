...
When generating the container's Dockerfile or bundle environment variables, Aspire uses the database resource from `.WithReference()` as the authoritative source of the connection string. If no `.WithReference()` is declared, Aspire falls back to inferring the connection string from `.WaitFor()` dependencies. Declaring `.WithReference(db)` explicitly is the recommended approach and takes priority over any wait-based inference.
:::
