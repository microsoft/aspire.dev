...
description: Understand how the Aspire SQL Server EF Core integrations fit together — model a SQL Server resource in your AppHost, then connect to it from C# apps using Entity Framework Core.
---
...
1. ### Model SQL Server in your AppHost
    ...
    Add the SQL Server hosting integration to your AppHost, then declare a SQL Server instance, one or more databases, and reference them from the apps that need to talk to the database. The [SQL Server Hosting integration](/integrations/databases/sql-server/sql-server-host/) reference walks through every capability — adding databases, data volumes, and more.
