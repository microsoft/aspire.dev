...
description: Understand how the Aspire Oracle EF Core integrations fit together — model a database resource in your AppHost, then connect to it from a C# consuming app using Entity Framework Core.
---
...
1. ### Model Oracle in your AppHost
    ...
    Add the Oracle hosting integration to your AppHost, then declare an Oracle server, one or more databases, and reference them from the apps that need to talk to the database. The [Oracle Hosting integration](/integrations/databases/efcore/oracle/oracle-host/) reference walks through every capability — adding databases, data volumes, password parameters, data bind mounts, and more — with C# examples.
