...
<InstallPackage packageName="Aspire.Oracle.EntityFrameworkCore" />
...
<Tabs syncKey='os'>
    ...
    ```json title='~/.microsoft/usersecrets/<user_secrets_id>/secrets.json'
    {
    "Parameters": {
    ...
    <TabItem label="Windows" icon="set:windows">
    ```json title='%APPDATA%\Microsoft\UserSecrets\<user_secrets_id>\secrets.json'
    {
    "Parameters": {
        "password": "Non-default-P@ssw0rd"
    }
...
> [!TIP]
> To find the `<user_secrets_id>`, open the `.csproj` file of your AppHost project and look for the `UserSecretsId` property.
