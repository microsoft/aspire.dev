...
description: Learn how to use the PostgreSQL hosting integration to create and manage PostgreSQL instances.
---
...
For an introduction to the PostgreSQL integration, see [Get started with the PostgreSQL Entity Framework Core integrations](/integrations/databases/efcore/postgres/postgresql-get-started/).
...
4. The `WithReference` method configures a connection in the `ExampleProject` named `"postgresdb"`.
...
For applications that don't use the [client integration](/integrations/databases/efcore/postgres/postgresql-client/), you can access the connection information through environment variables. Here's an example of how to configure environment variables for a non-.NET application:
