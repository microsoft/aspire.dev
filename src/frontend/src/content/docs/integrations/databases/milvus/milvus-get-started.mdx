...
description: Understand how the Aspire Milvus integrations fit together — model a vector database resource in your AppHost, then connect to it from any consuming app.
---
...
1. ### Model Milvus in your AppHost
    ...
    Add the Milvus hosting integration to your AppHost, then declare a Milvus server, one or more databases, and reference them from the apps that need to talk to the database. The [Milvus Hosting integration](/integrations/databases/milvus/milvus-host/) reference walks through every capability — adding databases, Attu GUI, data volumes, API key parameters, and more — with side-by-side C# and TypeScript examples.
