...
If you're new to the MongoDB integration, start with the [Get started with MongoDB integrations](/integrations/databases/mongodb/mongodb-get-started/) guide. For how consuming apps read the connection information this page exposes, see [Connect to MongoDB](../mongodb-connect/). If your app needs transactions or change streams, see [Enable replica set mode](#enable-replica-set-mode).
...
Use replica set mode when your app uses MongoDB transactions or change streams. For everyday CRUD-only workloads, keep the default standalone configuration.
...
The `{...}` values are placeholders for the runtime host, port, and secret values that Aspire injects into your consuming app.

:::note
Aspire still provisions a local MongoDB container, but it starts it as a one-node replica set and initializes the replica set during startup.
:::
