...
description: Understand how the Aspire Elasticsearch integrations fit together — model an Elasticsearch resource in your AppHost, then connect to it from any consuming app.
---
...
1. ### Model Elasticsearch in your AppHost
    ...
    Add the Elasticsearch hosting integration to your AppHost, then declare an Elasticsearch resource and reference it from the apps that need to talk to it. The [Elasticsearch Hosting integration](/integrations/databases/elasticsearch/elasticsearch-host/) reference walks through every capability — data volumes, data bind mounts, password parameters, and more — with C# examples.
