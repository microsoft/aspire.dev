...
You can use the Elasticsearch hosting integration with app resources that consume environment variables. When you add a reference to an Elasticsearch resource in an app resource such as a Python or JavaScript app, Aspire injects connection properties into the consuming project's environment variables, which you can use to interact with Elasticsearch.
