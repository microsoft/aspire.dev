...
description: Understand how the Aspire SQLite integrations fit together — model a SQLite database resource in your AppHost, then connect to it from any consuming app.
---
...
1. ### Model SQLite in your AppHost
    ...
    Add the SQLite hosting integration to your AppHost, then declare a SQLite database resource and reference it from the apps that need to use the database. The [SQLite Hosting integration](/integrations/databases/sqlite/sqlite-host/) reference walks through every capability — custom database file paths, the SQLiteWeb UI, SQLite extensions, and more.
