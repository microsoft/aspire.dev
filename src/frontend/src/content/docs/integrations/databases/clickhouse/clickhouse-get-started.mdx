...
1. ### Model ClickHouse in your AppHost
    ...
    Add the ClickHouse hosting integration to your AppHost, then declare a ClickHouse server, one or more databases, and reference them from the apps that need to talk to the database. The [ClickHouse Hosting integration](/integrations/databases/clickhouse/clickhouse-host/) article walks through every capability — adding databases, data volumes, bind mounts, custom parameters, and more.
