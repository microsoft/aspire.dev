...
- `MYSQLDB_HOST` - The hostname or IP address
- `MYSQLDB_PORT` - The port number
- `MYSQLDB_USERNAME` - The username for authentication
- `MYSQLDB_PASSWORD` - The password for authentication
- `MYSQLDB_DATABASE` - The database name
- `MYSQLDB_URI` - The full connection URI
- `MYSQLDB_JDBCCONNECTIONSTRING` - JDBC-format connection string
...
For the complete list of properties available, see [Properties of the MySQL resources](../mysql-client/#properties-of-the-mysql-resources).

## Client integration

To learn how to connect to and interact with MySQL databases from your consuming projects, see [MySQL Client integration](../mysql-client/).
