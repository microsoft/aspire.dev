...
1. ### Model MySQL in your AppHost
    ...
    Add the MySQL hosting integration to your AppHost, then declare a MySQL server, one or more databases, and reference them from the apps that need to talk to the database. The [MySQL Hosting integration](/integrations/databases/mysql/mysql-host/) article walks through every capability — adding databases, phpMyAdmin, data volumes, init files, custom parameters, and more — with side-by-side C# and TypeScript examples.
