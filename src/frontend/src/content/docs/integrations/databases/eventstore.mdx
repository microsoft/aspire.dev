...
[EventStore](https://eventstore.com) is a database designed for Event Sourcing, offering high availability and reliability for storing events. The Aspire EventStore integration enables you to connect to existing EventStore instances or create new instances from Aspire using the `docker.io/eventstore/eventstore` container image.

## Hosting integration

To run the EventStore container, install the [📦 CommunityToolkit.Aspire.Hosting.EventStore](https://nuget.org/packages/CommunityToolkit.Aspire.Hosting.EventStore) NuGet package in the AppHost project.

<InstallPackage packageName="CommunityToolkit.Aspire.Hosting.EventStore" />
...
To get started with the Aspire EventStore client integration, install the [📦 CommunityToolkit.Aspire.EventStore](https://nuget.org/packages/CommunityToolkit.Aspire.EventStore) NuGet package in the client-consuming project.
