...
These APIs are primarily used by integration authors to enable F5 debugging for custom resource types in Visual Studio and other IDEs. `WithLaunchToolArgs` declares the tool-invocation prefix that hosts a program (for example `run ./cmd/api` for `go`, or `-m flask` for `python`), separating it from the arguments a caller adds with `WithArgs` so the prefix isn't accidentally passed to an IDE launch configuration that already performs the invocation itself.
