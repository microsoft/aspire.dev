...
Aspire currently emits this diagnostic as `ASPIREDOTNETTOOL` (without the numeric suffix). `ASPIREDOTNETTOOL001` is the canonical identifier that aligns with every other `ASPIRE*` diagnostic and will be emitted in a future release. Until then, suppress the `ASPIREDOTNETTOOL` ID that appears in your build output. The examples below suppress both IDs so they keep working across the change.
:::

## Example

The following code generates `ASPIREDOTNETTOOL` (canonically `ASPIREDOTNETTOOL001`):
