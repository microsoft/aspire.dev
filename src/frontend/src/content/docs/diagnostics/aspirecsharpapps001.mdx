...
Aspire provides a way to add C# file-based apps to the Aspire AppHost with the `AddCSharpApp` extension. This allows you to reference single C# files (.cs) or project files (.csproj) directly without needing a full project structure. Since the shape of this API is expected to change in the future, it's experimental.
