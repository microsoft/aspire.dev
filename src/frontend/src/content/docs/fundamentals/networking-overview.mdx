...
<Aside type="caution">
  Binding to `0.0.0.0` exposes your service on all network interfaces, including
  local network and any externally routable interfaces. Only do this in a
  trusted development environment. Never deploy with this configuration to
  production.
</Aside>

### For ASP.NET Core projects

Configure a proxyless endpoint whose public and target ports match, and set its target host to `0.0.0.0`. Aspire derives Kestrel's binding from the endpoint, so the AppHost and workload stay aligned:
...
const api = await builder.addProject('api', '../Api/Api.csproj');
await api.withHttpEndpoint({ port: 5000, targetPort: 5000, isProxied: false });
await api.withHttpEndpointCallback(
  async (endpoint) => {
...
For container resources, configure a proxyless host-to-container port mapping and make the application inside the container listen on `0.0.0.0`:
...
builder.AddContainer("myapp", "myregistry/myapp")
    .WithHttpEndpoint(port: 8080, targetPort: 80, isProxied: false)
...
await myapp.withHttpEndpoint({ port: 8080, targetPort: 80, isProxied: false });
...
For executable resources, pass `0.0.0.0` as the bind address and configure a proxyless endpoint whose public and target ports match:
...
const api = await builder.addExecutable('api', 'uvicorn', '.', [
  'main:app',
  '--host',
  '0.0.0.0',
  '--port',
...
await api.withHttpEndpoint({ port: 8000, targetPort: 8000, isProxied: false });
...
Starting with Aspire 13.3, the DCP container tunnel is enabled by default and provides container-to-host connectivity when you use `WithReference`. Runtime-specific host gateway names such as `host.docker.internal` are fallbacks for when the tunnel is disabled.

For tunnel behavior, configuration, and examples, see [Container networking](/fundamentals/container-networking/).
