...
const api = await builder.addProject("api", "../Api/Api.csproj");
await api.withEnvironment("ASPNETCORE_URLS", "http://0.0.0.0:5000");
...
// Map host port 8080 to container port 80, and configure the app inside to bind on all interfaces
const myapp = await builder.addContainer("myapp", {
  image: "myregistry/myapp",
  tag: "latest",
});
await myapp.withHttpEndpoint({ port: 8080, targetPort: 80 });
await myapp.withEnvironment("ASPNETCORE_URLS", "http://0.0.0.0:80");
...
const api = await builder.addExecutable("api", "uvicorn", ".", ["main:app", "--host", "0.0.0.0", "--port", "8000"]);
await api.withHttpEndpoint({ port: 8000 });
...
When containers need to communicate with services running directly on the host (not in containers), the standard `localhost` address doesn't work from inside a container. Container runtimes provide a special hostname for this purpose.

### The host gateway address

Aspire selects the host gateway hostname based on the container runtime in use:

| Container runtime | Host gateway hostname |
|-------------------|----------------------|
| Docker | `host.docker.internal` |
| Podman | `host.containers.internal` |

<Aside type="note">
On Linux, Docker doesn't always resolve `host.docker.internal` by default. Aspire routes container-to-host traffic through its own container tunnel, so `WithReference` connections resolve to the correct address across platforms without extra configuration.
</Aside>
...
// The container reaches the host-side service via WithReference,
...
// The container reaches the host-side service via withReference,
// which automatically uses the correct host gateway address for the runtime
const containerApp = await builder.addContainer("mycontainer", {
  image: "myimage",
  tag: "latest",
});
await containerApp.withReference(api);
...
When using `AddSqlServer` or similar, the resource runs as a container that Aspire manages. However, if you have an existing SQL Server on the host machine and want containers to reach it, reference it as a connection string and point that connection string at the host gateway:
...
// Reference an existing SQL Server running on the host machine
const existingSqlServer = await builder.addConnectionString("sqlserver");

const containerApp = await builder.addContainer("myapp", {
  image: "myimage",
  tag: "latest",
});
await containerApp.withReference(existingSqlServer);
...
The `KnownNetworkIdentifiers` class provides constants for well-known network vantage points. Use it with the network-context `GetEndpoint` overload when you need to resolve an endpoint URL as it appears from a specific network:
...
// The endpoint URL as it appears from the host (localhost)
var hostEndpoint = api.GetEndpoint("http", KnownNetworkIdentifiers.LocalhostNetwork);

// The endpoint URL as it appears from within Aspire's container network
...
|------------|-------------|
| `KnownNetworkIdentifiers.LocalhostNetwork` | The endpoint as seen from the host machine (`localhost`). |
| `KnownNetworkIdentifiers.DefaultAspireContainerNetwork` | The endpoint as seen from within Aspire's container network — use when a container needs to reach a host-side service. |
| `KnownNetworkIdentifiers.PublicInternet` | The endpoint as seen from the public internet. |

<Aside type="note">
`KnownNetworkIdentifiers` and the network-context `GetEndpoint` overload are a lower-level C#-only AppHost API with no TypeScript equivalent. For most scenarios, prefer `WithReference`, which automatically injects the correct host gateway address (`host.docker.internal` on Docker) instead of `localhost` when a container references a host-side resource — you don't need to specify the host gateway manually.
</Aside>
