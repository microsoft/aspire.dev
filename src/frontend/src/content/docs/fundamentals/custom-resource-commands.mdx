...
For an interactive tool rather than a progress indicator, a command can [open an AppHost-owned shell](/app-host/terminal-service/) or display a [terminal interaction](/extensibility/interaction-service/#terminal-interactions). Use a dedicated terminal interaction, not a terminal-valued command argument, and keep the terminal's lifetime separate from the command's progress UI.
