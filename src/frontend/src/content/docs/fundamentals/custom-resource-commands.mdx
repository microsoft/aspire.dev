...
- `Logger` _(`ILogger`)_ — scoped to the resource's log stream. Use this to write log messages directly to the resource's logs in the Aspire dashboard, without needing to resolve a logger from the service provider.
