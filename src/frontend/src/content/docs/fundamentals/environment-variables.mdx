...
When you use `WithReference` to connect resources in the AppHost, Aspire automatically injects environment variables into the consuming resource. The names of these environment variables follow specific conventions based on the resource name and the type of reference. Understanding these conventions is essential, especially for non-.NET applications (Python, Node.js, Go, and others) where you must read environment variables directly rather than using typed Aspire client integrations.
...
// Connection strings
const cacheConn = process.env.ConnectionStrings__my_cache;
