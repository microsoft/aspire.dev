...
The recommended way to use hot reload with Aspire is to configure the AppHost to launch a specific project using `dotnet watch`:

```csharp title="C# — AppHost.cs"
var builder = DistributedApplication.CreateBuilder(args);

// Run the frontend with dotnet watch for hot reload
...
<Aside type="tip">
You can also open the individual service project in a separate terminal and run `dotnet watch run` there while Aspire manages the other services.
</Aside>
