...
Starting with Aspire 13.6, executable resources can also resolve `KnownNetworkIdentifiers.DefaultAspireContainerNetwork` endpoints—including a reference to their own endpoint or to another executable's endpoint. Aspire creates the same container tunnel used for host-to-container communication for the referenced endpoints, and the consuming executable waits until that tunnel is ready before its configuration is resolved:
