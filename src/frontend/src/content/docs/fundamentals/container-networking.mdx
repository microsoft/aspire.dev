...
### Container network endpoints for executables

Starting with Aspire 13.6, executable resources can successfully resolve their own or other executable resource endpoints scoped to the Aspire container network using `KnownNetworkIdentifiers.DefaultAspireContainerNetwork`. Previously such endpoint references could only work when a container resource also had a reference to the same executable endpoint. This can be used in dynamic configuration scenarios such as an executable registering itself with a container resource via API. The referenced endpoint will be registered with the container tunnel used for container to host communication in Aspire and will be valid to be used by container resources the same as direct container to executable references:
...
<Aside type="note">
  Resolving a `DefaultAspireContainerNetwork` endpoint doesn't add a startup
  or readiness dependency on the referenced resource—only the endpoint
  allocation is awaited. The default container network is only created when
  the app model contains a container resource; if it doesn't, resolving one
  of these endpoints fails with an actionable, resource-scoped error instead
  of silently creating a network. Executables configured with
