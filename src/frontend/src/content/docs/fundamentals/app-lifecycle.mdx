...
Consider [this example](https://github.com/BethMassi/VolumeMount/). You have a distributed application that consists of a Blazor web project that relies on a SQL Server database with a persistent data volume as well as a persistent writable file volumme to capture user file uploads. 
You want to distribute your Blazor app as a Docker container image via a GitHub Container Registry. You will need the [Aspire.Hosting.Docker](/integrations/compute/docker) and [Aspire.Hosting.SqlServer](/integrations/databases/sql-server) integrations. 
...
- Restores dependenceies and builds the project
...
The `aspire publish` command does the follwing:
- Analyzes your `AppHost.cs` configuration
