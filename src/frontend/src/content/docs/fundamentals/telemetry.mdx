...
<LearnMore>
  ...
  configuration](/fundamentals/service-defaults/#opentelemetry-configuration).
</LearnMore>
...
<Aside type="note">
  Aspire telemetry works best in environments that support OTLP. OTLP exporting
  is disabled if `OTEL_EXPORTER_OTLP_ENDPOINT` isn't configured.
</Aside>
