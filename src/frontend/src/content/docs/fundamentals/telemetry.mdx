...
When you create an Aspire project, the Aspire dashboard provides a UI for viewing app telemetry by default. Telemetry data is sent to the dashboard using <abbr title="OpenTelemetry Protocol" data-tooltip-placement="top">OTLP</abbr>, and the dashboard implements an <abbr title="OpenTelemetry Protocol" data-tooltip-placement="top">OTLP</abbr> server to receive telemetry data and store it in memory.
...
2. **App configuration runs** — The AppHost project runs its configuration:
   ...
   - The <abbr title="OpenTelemetry Protocol" data-tooltip-placement="top">OTLP</abbr> endpoint is an HTTP/2 port started by the dashboard, set in `OTEL_EXPORTER_OTLP_ENDPOINT`
...
Aspire deployment environments should configure OpenTelemetry environment variables that make sense for their environment. For example, `OTEL_EXPORTER_OTLP_ENDPOINT` should be configured to the environment's local <abbr title="OpenTelemetry Protocol" data-tooltip-placement="top">OTLP</abbr> collector or monitoring service.

<Aside type="note">
Aspire telemetry works best in environments that support <abbr title="OpenTelemetry Protocol" data-tooltip-placement="top">OTLP</abbr>. <abbr title="OpenTelemetry Protocol" data-tooltip-placement="top">OTLP</abbr> exporting is disabled if `OTEL_EXPORTER_OTLP_ENDPOINT` isn't configured.
</Aside>
