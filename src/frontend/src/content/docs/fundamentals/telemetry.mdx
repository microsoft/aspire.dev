...
When running an Aspire AppHost, agents use CLI commands like `aspire otel logs`, `aspire otel trace`, and `aspire describe` to fetch structured logs, distributed traces, and resource status directly from the dashboard. A typical agent workflow starts the app with `aspire start`, waits for resources with `aspire wait`, then queries telemetry to diagnose issues and verify fixes.
...
- **Aspire CLI** — The `aspire otel logs`, `aspire otel trace`, and `aspire otel spans` commands query telemetry directly from the terminal. Use `--format Json` for structured output suitable for scripting and automation.
