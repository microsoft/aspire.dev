...
1. Add a development-time Dockerfile for your Linux service that publishes in Debug configuration and includes the .NET debugger tools:

    ```dockerfile title="Dockerfile — Linux service with debugger"
    FROM mcr.microsoft.com/dotnet/sdk:10.0 AS build
    WORKDIR /src
    COPY . .
    # Publish in Debug configuration so that .pdb files are included
    ...
    Building with `-c Debug` ensures `.pdb` symbol files are published alongside the assemblies, which are required for the remote debugger to map execution back to your source code.
