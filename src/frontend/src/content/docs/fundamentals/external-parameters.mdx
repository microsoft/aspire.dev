...
Parameters can be set using environment variables, which is particularly useful in CI/CD pipelines and deployment scenarios. The environment variable name follows the pattern `Parameters__{parameter-name}`, using double underscores (`__`) to separate the configuration section from the parameter name.
...
- `Parameters__registry-endpoint` - Registry URL (e.g., `ghcr.io`)
- `Parameters__registry-repository` - Repository path (e.g., `username/reponame`)
...
- name: Push images with Aspire
  env:
    ...
    Parameters__registry-repository: your-org/your-repo
  run: aspire do push
```
