...
import dashboardUnresolvedParamsLight from '@assets/fundamentals/dashboard-unresolved-parameters-message-light.png';
import customizedParameterUi from '@assets/fundamentals/customized-parameter-ui.png';
import customizedParameterUiLight from '@assets/fundamentals/customized-parameter-ui-light.png';
...
<Aside type="note">
  You usually don't need to read this value yourself. Prefer passing parameter
  resources to dependent resources, as shown in the examples in this article.
</Aside>
...
<ThemeImage
  dark={dashboardUnresolvedParams} light={dashboardUnresolvedParamsLight}
...
<ThemeImage
  dark={customizedParameterUi} light={customizedParameterUiLight}
...
<Aside type="caution" title="WithCustomInput is an experimental API">
  The `WithCustomInput` method uses the experimental interaction service APIs,
  which are marked with the `[Experimental("ASPIREINTERACTION001")]` attribute.
  If you have `TreatWarningsAsErrors` enabled (or prefer warning-free builds),
  suppress this warning with `#pragma warning disable ASPIREINTERACTION001` when
  using the API. The `WithDescription` method is _not_ experimental and doesn't
  require this pragma. For more information about the diagnostic and other ways
  to suppress it, see
  [ASPIREINTERACTION001](/diagnostics/aspireinteraction001/).
</Aside>

<Aside type="note">
  When using `WithCustomInput`, the `InteractionInput` object you create
  replaces the default input generation. To preserve markdown rendering in the
  parameter dialog, copy `EnableDescriptionMarkdown =
  p.EnableDescriptionMarkdown` from the parameter. The `enableMarkdown: true`
  parameter in `WithDescription` sets this property on the parameter resource,
  but you must explicitly copy it to your custom `InteractionInput` object.
</Aside>
...
// Read from a custom configuration key
const dbPassword = await builder.addParameterFromConfiguration(
  'db-password',
  'CustomConfig:Database:Password',
  { secret: true }
);
...
const api = await builder.addProject('api', '../ApiService/ApiService.csproj');
await api.withEnvironment('DB_PASSWORD', dbPassword);
...
await api.withReference(redis).waitFor(redis);
...
<Aside type="note">
  ...
  that the connection string connects to. For connection strings backed only by
  configuration, the dependency is still modeled but there isn't an
  Aspire-managed resource to wait for.
</Aside>
...
const secretKey = await builder.addParameter('secretkey', { secret: true });

const connectionString = await builder.addConnectionString(
  'composedconnectionstring',
  {
    environmentVariableNameOrExpression: refExpr`Endpoint=https://api.contoso.com/v1;Key=${secretKey}`,
  }
);

const catalogApi = await builder.addProject(
  'catalogapi',
  '../CatalogApi/CatalogApi.csproj'
);
await catalogApi.withReference(connectionString).waitFor(connectionString);
```
...
const database = await postgres.addDatabase('db');

const pgConnectionString = await builder.addConnectionString('pgdatabase', {
  environmentVariableNameOrExpression: refExpr`${database};Include Error Details=true`,
});

const customerApi = await builder.addProject(
  'customerapi',
  '../CustomerApi/CustomerApi.csproj'
);
await customerApi.withReference(pgConnectionString).waitFor(pgConnectionString);
```
...
const db = isRunMode
    ? (await builder.addSqlServer('sql')).addDatabase('db')
...
await api.withEnvironment('InsertionRows', insertionRows);
