...
<Aside type="note">
  TypeScript AppHost support for reading AppHost configuration directly through
  `IDistributedApplicationBuilder.Configuration` isn't available. In TypeScript
  AppHosts, pass parameter resources to dependent resources instead of reading
...
<Aside type="note">
  TypeScript AppHost support for `withCustomInput` is not yet available.
</Aside>
...
<Aside type="note">
  TypeScript AppHost support for `waitFor` with connection string resources
  isn't yet available.
</Aside>
...
<Aside type="note">
  TypeScript AppHost support for constructing connection strings with
  `ReferenceExpression.Create(...)` is not yet available.
</Aside>
...
<Aside type="note">
  TypeScript AppHost support for appending to generated connection strings with
  `ReferenceExpression.Create(...)` is not yet available.
</Aside>
