...
When `WithReference` is used, Aspire generates environment variables with names derived from the resource name. For details on how resource names are transformed into environment variable names, see [Environment variables](/fundamentals/environment-variables/).
