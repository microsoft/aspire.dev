...
var chartVersion = builder.AddParameter("chart-version");
...
The `WithChartVersion` method also accepts a string literal:
...
These values are written to the generated `Chart.yaml`:

```yaml title="Chart.yaml"
name: "my-aspire-app"
version: "1.2.3"
appVersion: "1.2.3"
description: "My Aspire application Helm chart"
```
...
const service = await builder.addContainer('service', { image: 'nginx', tag: 'latest' });
...
<Aside type="tip">
  Use `WithHelm` on the Kubernetes environment for global Helm chart settings,
  and `PublishAsKubernetesService` on individual resources for per-resource
  customization.
</Aside>
