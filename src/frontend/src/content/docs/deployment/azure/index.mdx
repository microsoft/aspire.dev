...
After authentication succeeds, `aspire deploy` still needs a small set of shared Azure settings. The tenant comes from your Azure sign-in context, while the settings below tell Aspire which subscription, location, and resource group to target.
...
After your CI/CD system authenticates to Azure, provide the shared Azure settings and any AppHost parameters to the deploy step.
...
| Aspire environment | `aspire deploy --environment <name>` | Select the deployment context, such as `Production` |
