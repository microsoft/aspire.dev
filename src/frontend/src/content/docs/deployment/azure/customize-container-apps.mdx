...
await api.publishAsAzureContainerApp((infrastructure, app) => {
    app.template.scale.minReplicas = 1;
    app.template.scale.maxReplicas = 10;
});
...
await serviceBus.publishAsExisting(existingBusName);
```
...
const serviceBus = await builder.addAzureServiceBus("messaging");
await serviceBus.withResourceGroup("my-shared-rg");
```
