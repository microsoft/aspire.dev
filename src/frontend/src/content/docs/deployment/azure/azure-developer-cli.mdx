...
For new Azure deployments, we recommend using `aspire deploy` directly. It offers a simpler workflow, supports more Azure targets, and doesn't require an intermediate manifest. See [Deploy using the Aspire CLI](/deployment/azure/aca-deployment-aspire-cli/).
</Aside>
...
<Aside type="caution">
The deployment manifest format is deprecated and no longer being evolved. New features and deployment targets are delivered through the `aspire deploy` path. See [Deploy using the Aspire CLI](/deployment/azure/aca-deployment-aspire-cli/).
</Aside>
