...
<Aside type="note">
  The exact C# object model helpers shown here, such as `AddVolume` and
  `Volume`, don't have one-to-one TypeScript examples in this article.
  TypeScript AppHosts can still customize Docker Compose output with
  `addDockerComposeEnvironment().configureComposeFile(...)` and
