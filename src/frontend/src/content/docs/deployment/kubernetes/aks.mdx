...
### Plain HTTP returns 404 after adding `WithTls`

This is expected. Once a gateway has both an HTTP and HTTPS listener and the route resolves to the HTTPS path on the AGC frontend, AGC stops serving the route on plain HTTP. Use `https://` (and `curl -k` while on a staging certificate) to verify the gateway. If you want to keep an HTTP-only path for health probes, add a second route bound explicitly to the HTTP listener with `sectionName: http` in the generated `HTTPRoute`.
