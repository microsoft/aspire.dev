...
In run mode, `DATA_PATH` points to a persistent directory in the AppHost's local store — normally under the AppHost's intermediate-output directory, so cleaning build outputs can remove the local data. In publish and deploy modes, `DATA_PATH` contains the mount path (`/data` above), so the application reads the same environment variable in both environments.
