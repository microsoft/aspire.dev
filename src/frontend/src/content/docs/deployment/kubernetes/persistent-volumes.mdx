...
Stateful workloads — databases, message brokers, or any service that writes durable data — need storage that survives pod restarts and rescheduling. A `KubernetesPersistentVolumeResource` lets you describe a durable disk once in your AppHost, with its own storage class, capacity, access modes, and metadata annotations, then bind it to one or more workloads. At publish time, the volume renders as a `v1.PersistentVolumeClaim` in the generated Helm chart.

This mirrors how [ingress and gateway resources](/deployment/kubernetes-ingress/) work: the volume is a first-class resource you configure once and reference from workloads, rather than relying on a single environment-wide storage shape for every mount.
...
Call `AddPersistentVolume` on a Kubernetes environment, then configure its shape with the fluent methods:
...
|---|---|---|
| `WithStorageClass(string)` | `withStorageClass` | Sets `spec.storageClassName` on the PVC. Accepts a literal string or an Aspire parameter. |
| `WithCapacity(string)` | `withCapacity` | Sets the requested storage on `spec.resources.requests.storage` (for example, `"20Gi"`). Accepts a literal string or an Aspire parameter. |
...
| `WithVolumeAnnotation(string, string)` | `withVolumeAnnotation` | Adds a key-value pair to the PVC's `metadata.annotations`. Useful for CSI driver hints, dynamic provisioner parameters, or backup tooling tags. Accepts a literal string or an Aspire parameter. |
...
await pg.withDataVolume('pg-data');
