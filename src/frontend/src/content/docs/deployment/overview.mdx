...
```plaintext data-disable-copy title="Error: No publish support"
...
```plaintext data-disable-copy title="Error: No deploy support"
Step 1: Analyzing model.
...
This example shows how you could explicitly map services to different compute environments. For example, a front end in Kubernetes and a backend in Docker Compose.
