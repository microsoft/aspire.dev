...
<Aside type="tip">
  Aspire's AppHost allows you to define your app's architecture in a way that is
  platform-agnostic, meaning you can run the same code locally or deploy it to
  any environment without changes.
</Aside>
...
<Aside type="note">
  Pipeline-based deployment is an experimental feature in Aspire 13. APIs and
...
Add `--format json` to get the same listing as machine-readable JSON, which is useful for scripting. `--format` is only valid together with `--list-steps`.
