...
| API Service | Local process | AWS Lambda | Functions | Custom |
...
Pipeline steps operate at different levels of granularity. For example, when deploying a typical web application with a front end, API service, database, and storage, the pipeline might include:
...
- `build-webfrontend`: Build container image for the front end

**Deployment steps** (depend on builds and infrastructure):
- `push-apiservice`: Push API service image to registry
- `push-webfrontend`: Push front-end image to registry
- `deploy-apiservice`: Deploy API service to compute platform
- `deploy-webfrontend`: Deploy front end to compute platform
...
- **`WellKnownPipelineSteps.Push`**: The entry point for the `aspire do push` command, which pushes container images to registries after they have been built.
...
:::note
The `builder.pipeline` API is not yet available in the TypeScript AppHost SDK.
:::
...
- **Incremental deployments**: After making changes to only your front-end code, run just the front-end-related steps rather than rebuilding and redeploying your entire application
