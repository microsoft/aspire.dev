...
Once that's in place, `/api/*` resolves to the backend in both dev (via the framework's dev proxy) and production (via YARP) — no `VITE_*` build-time variables, no CORS configuration.
