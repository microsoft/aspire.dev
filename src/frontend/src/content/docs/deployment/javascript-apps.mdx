...
Aspire 13.3 introduces three dedicated publish extension methods on JavaScript resources. These are marked `[Experimental]` and require suppressing the `ASPIREEXTENSION001` diagnostic.
...
frontend.PublishAsStaticWebsite(options =>
{
    options.OutputPath = "dist";
    options.StripPrefix = false;
    options.TargetEndpointName = "https";
}, apiPath: "/api/{**catch-all}", apiTarget: api);
...
| `TargetEndpointName` | `https` | The endpoint name on the API resource to use for proxying. YARP uses service discovery to resolve the backend address. |

The two overloads are:

- **Without proxy**: `PublishAsStaticWebsite(options?)` — serves only the built static files, no API proxying.
- **With proxy**: `PublishAsStaticWebsite(options?, apiPath, apiTarget)` — adds a YARP route to forward matching requests to the API resource.
...
For SSR frameworks that fetch from the backend at request time, the examples below assume an explicit `WithEnvironment("API_URL", api.GetEndpoint("http"))` on the frontend resource. `API_URL` is not a built-in Aspire convention — it's a custom variable name that the framework code reads. If you'd rather use the service-discovery variable described in [Dev-mode API proxying](#dev-mode-api-proxying) above, read `<RESOURCENAME>_<SCHEME>` (for example `API_HTTP`) instead.

#### Nuxt

- **Directory structure**: Nuxt 4 places pages in `app/pages/`, not a root `pages/` directory.
- **Environment variables**: Nuxt maps [`runtimeConfig`](https://nuxt.com/docs/getting-started/configuration#runtime-config) keys to env vars with a `NUXT_` prefix. To pass `apiUrl` at runtime, set `NUXT_API_URL` on the resource — not `API_URL`.
