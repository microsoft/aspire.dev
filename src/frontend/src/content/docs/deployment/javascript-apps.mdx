...
If a build-only JavaScript resource is not consumed by another resource, publish/deploy validation fails because that resource would not participate in the deployed app. Prefer fixing the app model by choosing one of the deployment shapes in this article. The app-wide `builder.Pipeline.DisableBuildOnlyContainerValidation()` escape hatch exists for exceptional cases, but it should not be the normal way to deploy JavaScript frontends.
