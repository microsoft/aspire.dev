...
Use `PublishAsStaticWebsite` when the framework produces static files and you want the JavaScript app resource to become the deployed static website. The generated container uses YARP to serve the built files and can optionally proxy API requests to a backend resource using [service discovery](/fundamentals/service-discovery/).
...
<LearnMore>
  For setting up `AddNextJsApp` with its deploy-time `output: "standalone"`
  validation, see [JavaScript integration — Add Next.js
  application](/integrations/frameworks/javascript/#add-nextjs-application).
</LearnMore>
...
| Nuxt                  | `PublishAsPackageScript` | `node .output/server/index.mjs` (via `start`)    | `NUXT_` prefix on `runtimeConfig` env vars                             |
| Astro SSR             | `PublishAsPackageScript` | `node ./dist/server/entry.mjs` (via `start`)     | `@astrojs/node`, `prerender: false` per page                           |
| Remix / React Router  | `PublishAsPackageScript` | `react-router-serve` (via `start`)               | None                                                                   |
| Qwik City             | `PublishAsPackageScript` | `node server/entry.node-server.js` (via `start`) | Node server adapter, Node 22.x or 24.x LTS                             |

<LearnMore>
  For per-framework AppHost snippets, see [JavaScript integration — Framework
  examples](/integrations/frameworks/javascript/#framework-examples).
</LearnMore>
...
```typescript title="Nuxt — server/api/weather.ts"
...
```astro title="Astro SSR — src/pages/index.astro"
---
...
```tsx title="Remix — app/routes/home.tsx"
...
```typescript title="Qwik City — adaptors/node-server/vite.config.ts"
...
```tsx title="Qwik City — src/routes/index.tsx"
...
```json title="Angular — angular.json"
{
  "projects": {
    "angular": {
      "architect": {
...
```typescript title="Vite / React / Vue — fetch weather"
...
- Modeling a Next.js standalone deployment with generic package-script publishing instead of `AddNextJsApp`.
