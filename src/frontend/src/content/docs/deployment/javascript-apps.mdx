...
<Aside type="caution">
  `WithEnvironment(...)` configures runtime environment variables. Vite and many
  SPA toolchains usually read values such as API URLs at build time instead. To
  pass build-time values into the generated Dockerfile, use Docker build
  arguments such as `WithBuildArg(...)`.
</Aside>

<Aside type="caution">
  The backend URL is usually not known when the frontend image is being built,
  so you often cannot solve this by passing the backend URL with
  `WithBuildArg(...)`. If the backend also needs the frontend origin for CORS,
  this creates a deployment-time cycle between the frontend and backend that
  Aspire does not automatically resolve.
</Aside>
