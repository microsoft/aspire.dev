...
- my-apphost/
  - .aspire/modules/  生成された TypeScript SDK（編集しないでください）
    - aspire.mts
    - base.mts
    - transport.mts
...
```json title="aspire.config.json"
{
  ...
  "packages": {
    "Aspire.Hosting.JavaScript": "%ASPIRE_VERSION%"
  },
  "profiles": {
    "https": {
...
<Aside type="tip">
`.aspire/` を `.gitignore` に追加してください。生成された成果物を含み、`aspire.config.json` からいつでも再作成できます。
</Aside>
...
:::note
Bun には組み込み TypeScript サポートがあるため、`tsx` なしで `apphost.mts` を直接実行します。
:::
