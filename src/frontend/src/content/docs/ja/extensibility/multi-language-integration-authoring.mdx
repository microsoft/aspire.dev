---
title: 多言語統合
description: TypeScript AppHost で動作するように Aspire hosting integration に注釈を付ける方法を学びます。
---
...
<PackageReference Include="Aspire.Hosting.Integration.Analyzers" Version="%ASPIRE_VERSION%">
...
```typescript title="生成された SDK" {5-8}
...
```csharp title="重複する capability ID"
...
```csharp title="SDK メソッド名付きの一意な capability ID"
...
```typescript title="生成されたインターフェイス"
...
```typescript title="生成された API の利用"
...
```typescript title="コールバック API の利用"
...
2. `aspire.config.json` にプロジェクト参照として統合を追加します:
    ...
    ```json title="aspire.config.json（packages セクション）"
   {
     "packages": {
       "MyCompany.Hosting.MyDatabase": "../src/MyCompany.Hosting.MyDatabase/MyCompany.Hosting.MyDatabase.csproj"
     }
