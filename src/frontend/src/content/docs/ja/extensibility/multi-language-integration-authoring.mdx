...
```typescript title="TypeScript — 生成された SDK" {5-8}
...
```csharp title="C# — 重複する capability ID"
...
```csharp title="C# — SDK メソッド名付きの一意な capability ID"
...
```typescript title="TypeScript — 生成されたインターフェイス"
...
```typescript title="TypeScript — 生成された API の利用"
...
```typescript title="TypeScript — コールバック API の利用"
...
```csharp title="C# — 互換性のないオーバーロードを除外"
...
2. `aspire.config.json` にプロジェクト参照として統合を追加します:
    ...
    ```json title="JSON — aspire.config.json（packages セクション）"
   {
     "packages": {
       "MyCompany.Hosting.MyDatabase": "../src/MyCompany.Hosting.MyDatabase/MyCompany.Hosting.MyDatabase.csproj"
     }
