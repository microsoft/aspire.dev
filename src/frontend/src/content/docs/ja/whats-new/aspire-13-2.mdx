...
`aspire add` を実行すると、CLI は統合の .NET アセンブリを検査し、TypeScript SDK を `.aspire/modules/` に生成します。 `aspire restore` を実行すると、これらの SDK が再生成されます。これは、ブランチのアップグレードまたは切り替え後に役立ちます (これは、`aspire run` でも自動的に実行されます)。
...
await builder.addContainer("worker", { image: "contoso/worker", tag: "latest" })
    .withDockerfile("../worker")
    .withBuildSecret("ACCESS_TOKEN", accessToken);
```
...
Aspire 13.2 は、以前の Azure AI Foundry に重点を置いたホスティング サーフェスを、より広範な Microsoft Foundry 統合に置き換えます。ホスティング パッケージは `Aspire.Hosting.Foundry` になりました。
...
<Aside type="caution">
これは破壊的な変更です。移行手順における破壊的な変更については [Foundry パッケージと名前空間の
移行](#foundry-パッケージと名前空間の移行) を
参照してください。
</Aside>
...
### Foundry パッケージと名前空間の移行

Aspire 13.2 では、Foundry ホスティング サーフェスが `Aspire.Hosting.Foundry` に統一されました。

| 以前の 13.1 名称 | 13.2 の名称 |
| --- | --- |
| ホスティング パッケージ: `Aspire.Hosting.Azure.AI Foundry` | ホスティング パッケージ: `Aspire.Hosting.Foundry` |
| 名前空間 import: `Aspire.Hosting.Azure.AI Foundry` | 名前空間 import: `Aspire.Hosting.Foundry` |
| `Azure AI Foundry Resource` | `FoundryResource` |
| `Azure AI Foundry Extensions` | `FoundryExtensions` |
| `Azure AI Foundry Deployment Resource` | `FoundryDeploymentResource` |
| `AI Foundry Model` | `FoundryModel` |

移行のヒント: パッケージ参照と using ディレクティブを `Aspire.Hosting.Foundry` に更新し、`AddFoundry` 関連コードと型参照を上記の新しい名称に置き換えてください。
...
次の TypeScript AppHost API の名前が変更されました。影響を受ける `apphost.mts` ファイルを適宜更新します。
