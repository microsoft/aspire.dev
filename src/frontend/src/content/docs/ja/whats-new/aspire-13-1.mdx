...
- **AI コーディングエージェント向け MCP**: 新しい `aspire agent init` コマンドと、AI アシスタントが統合を発見し Aspire アプリケーションと連携するための MCP ツール。
...
フィードバックやご質問、Aspire への貢献にご興味がある場合は [<Icon name="github" class='d-inline' /> GitHub](https://github.com/microsoft/aspire) でコラボレーションするか [<Icon name="discord" class='d-inline' /> Discord](https://aka.ms/aspire-discord) に参加して、チームやコミュニティメンバーと交流してください。
...
<Aside type="note">
Aspire CLI のバージョンが 13 未満の場合は、 [CLI をインストールする](/ja/get-started/install-cli/) を参照して、最新の安定版をインストールしてください。
</Aside>
...
新しい `aspire agent init` コマンドは、開発環境を検出し、対応する AI コーディングエージェント向けに MCP サーバーを設定します:

```bash title="MCP 構成を初期化"
aspire agent init
```

<LearnMore>
コマンド リファレンス: [`aspire agent init`](/ja/reference/cli/commands/aspire-agent-init/).
</LearnMore>
...
<LearnMore>
詳細については、 [MCP サーバー統合](/ja/get-started/aspire-mcp-server/) をご参照ください。
</LearnMore>
...
```csharp title="Azure マネージド Redis (これまで)" data-disable-copy
// Before (Aspire 13.0)
var redis = builder.AddAzureRedisEnterprise("cache");
```

```csharp title="Azure マネージド Redis (これから)"
// After (Aspire 13.1)
...
```csharp title="明示的な Azure Container Registry"
var builder = DistributedApplication.CreateBuilder(args);
...
```csharp title="Docker Compose の .env file をカスタマイズ"
var builder = DistributedApplication.CreateBuilder(args);
...
```csharp title="カスタム証明書による TLS 終端"
...
```csharp title="カスタム HTTPS ポートを使用する YARP"
var builder = DistributedApplication.CreateBuilder(args);
...
```csharp title="構成コールバックによる TLS 終端"
var builder = DistributedApplication.CreateBuilder(args);
...
```csharp title="OTLP エクスポートを有効化した Keycloak"
var builder = DistributedApplication.CreateBuilder(args);
...
```csharp title="ジェネリック型なしの Azure Functions"
var builder = DistributedApplication.CreateBuilder(args);
...
<LearnMore>
詳細については [Azure Functions 統合](/ja/integrations/cloud/azure/azure-functions/azure-functions-get-started/) または [Aspire と Azure Functions の使い方](https://learn.microsoft.com/azure/azure-functions/dotnet-aspire-integration) をご参照ください。
</LearnMore>
...
- Windows の VS Code ターミナルで `aspire agent init` がハングする問題を修正しました。
...
4. **MCP 構成の確認**: `aspire agent init` を実行して、AI コーディングエージェントのサポートを設定します。
...
**フィードバックと貢献**: Aspire 13.1 のご利用体験について、ぜひお聞かせください！ [GitHub](https://github.com/microsoft/aspire/issues) でフィードバックを共有するか、 [Discord](https://aka.ms/aspire-discord) でディスカッションにご参加ください。
