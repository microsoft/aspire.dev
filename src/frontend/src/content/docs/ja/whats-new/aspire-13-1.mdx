...
生成 AI AI ビジュアライザーには、次のような改善が加えられています:
...
<LearnMore>
Fコンテナー レジストリの改善点や、それが Azure デプロイに与える影響について詳しく知りたい場合は、 [Safia Abdalla's blog post on fixing Aspire's image problem](https://blog.safia.rocks/2025/12/15/aspire-image-push/) をご覧ください。
</LearnMore>
...
2. **プロジェクトを更新s**: プロジェクト ディレクトリで `aspire update` を実行します。
