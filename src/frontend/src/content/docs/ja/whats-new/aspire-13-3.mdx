...
```bash title="dotnet tool でインストール"
...
```typescript title="apphost.mts の例"
...
<LearnMore>
  詳細については、[Kubernetes にデプロイ](/ja/deployment/kubernetes/clusters/) を参照してください。
...
```csharp title="cert-manager を使った Ingress"
// Kubernetes 環境を宣言します。
var k8s = builder.AddKubernetesEnvironment("k8s");
...
```typescript title="cert-manager を使った Ingress"
import { createBuilder } from './.aspire/modules/aspire.mjs';
...
```csharp title="特権モードの Docker Compose サービス"
...
```typescript title="特権モードの Docker Compose サービス"
import { createBuilder } from './.aspire/modules/aspire.mjs';
...
```csharp title="結果ボディ付き HTTP コマンド"
...
```typescript title="結果ボディ付き HTTP コマンド"
import { createBuilder } from './.aspire/modules/aspire.mjs';
...
```csharp title="リソースコマンドでアクセストークンを発行"
...
```typescript title="リソースコマンドでアクセストークンを発行"
import {
  createBuilder,
  CommandResultFormat,
...
```typescript title="ライフサイクルイベントを購読"
import { createBuilder } from './.aspire/modules/aspire.mjs';
...
```csharp title="WithReference からエンドポイントを除外"
...
```typescript title="withReference からエンドポイントを除外"
import { createBuilder } from './.aspire/modules/aspire.mjs';
