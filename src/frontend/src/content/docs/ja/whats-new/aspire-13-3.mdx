...
`aspire otel logs` コマンドと `aspire otel trace` コマンドが `--dashboard-url` と `--api-key` オプションを受け入れるようになったため、AppHost を起動せずに [スタンドアロンダッシュボード](/ja/dashboard/standalone/)から テレメトリをクエリできます。
...
aspire otel trace --dashboard-url https://localhost:18888/login?t=TOKEN
```
...
<TabItem id='typescript' label='apphost.mts'>

```typescript title="TypeScript — apphost.mts の例"
