...
[Docker Compose](https://docs.docker.com/compose/) は、複数コンテナーの Docker アプリケーションを定義および実行するためのツールです。 YAML ファイルを使用してアプリケーションのサービス、ネットワーク、ボリュームを構成できるため、複雑なアプリケーションをローカルまたはさまざまな環境で管理およびデプロイすることが容易になります。
        ...
        ```bash title="Aspire CLI — Add Docker Compose" data-funnel="deployment_intent" data-funnel-step="prerequisite_copied" data-funnel-trigger="copy" data-funnel-surface="deployment_page" data-funnel-target="docker_compose"
        aspire add docker
        ```
...
Aspire CLI は対話型です。追加する [📦 Aspire.Hosting.Docker](https://www.nuget.org/packages/Aspire.Hosting.Docker) バージョンに適切な検索結果を必ず選択してください。
    ...
    <TabItem id="azure" label="Azure">
        ...
        ```bash title="Aspire CLI — Add Azure App Containers" data-funnel="deployment_intent" data-funnel-step="prerequisite_copied" data-funnel-trigger="copy" data-funnel-surface="deployment_page" data-funnel-target="azure_container_apps"
...
<Tabs syncKey="deploy-target">
  <TabItem id="docker-compose" label="Docker Compose">
    ...
    ```csharp title="AppHost.cs project-based orchestrator" {3-4}
...
<Tabs syncKey="deploy-target">
  <TabItem id="docker-compose" label="Docker Compose">
    ...
    ```typescript title="apphost.mts" {5-6} twoslash
...
Docker Compose にデプロイすると、コンテナー イメージが構築され、Docker Compose を使用してサービスがローカルで開始されます。
    ...
    ```bash title="Aspire CLI — Deploy your app" data-funnel="deployment_intent" data-funnel-step="deploy_command_copied" data-funnel-trigger="copy" data-funnel-surface="deployment_page" data-funnel-target="docker_compose"
    aspire deploy
    ```
...
Azure App Containers にデプロイすると、コンテナー イメージが構築され、サービスが Azure App Containers にデプロイされます。
    ...
    ```bash title="Aspire CLI — Deploy your app" data-funnel="deployment_intent" data-funnel-step="deploy_command_copied" data-funnel-trigger="copy" data-funnel-surface="deployment_page" data-funnel-target="azure_container_apps"
    aspire deploy
    ```
...
<Tabs syncKey="deploy-target">
  <TabItem id="docker-compose" label="Docker Compose">
    ...
    <span aria-hidden="true" data-funnel="deployment_intent" data-funnel-step="verification_or_troubleshooting" data-funnel-trigger="view" data-funnel-surface="deployment_page" data-funnel-target="docker_compose" data-funnel-destination="verification" />
...
フロントエンドは、ASP.NET Core Blazor アプリケーションに天気予報データを表示します。
  ...
  <TabItem id="azure" label="Azure">
    ...
    <span aria-hidden="true" data-funnel="deployment_intent" data-funnel-step="verification_or_troubleshooting" data-funnel-trigger="view" data-funnel-surface="deployment_page" data-funnel-target="azure_container_apps" data-funnel-destination="verification" />
