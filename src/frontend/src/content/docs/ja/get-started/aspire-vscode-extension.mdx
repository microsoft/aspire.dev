...
1. VS Code を開きます。
1. **拡張機能** ビュー (<Kbd windows='Control+Shift+X' mac='Command+Shift+X' />) を開きます。
...
1. VS Code で既存の Aspire プロジェクトを開くか、コマンド パレット (<Kbd windows='Control+Shift+P' mac='Command+Shift+P' />) から **Aspire: New Aspire project** を実行して新しいプロジェクトを作成します。
