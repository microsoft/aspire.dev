...
コンテナーのライフタイムの詳細は、[永続コンテナーのライフタイム](/ja/app-host/resource-lifetimes/) を参照してください。
...
**解決策**:
1. <Kbd windows="Ctrl+C" mac="Command+C" linux="Ctrl+C" /> で実行中の Aspire セッションを停止するか、ダッシュボードを閉じる
