...
- `WithCustomInput`: コールバックメソッドを指定して、パラメーターダイアログをカスタマイズします。たとえば、既定値、入力タイプ、ラベル、プレースホルダー テキストなどを設定できます。
...
<Aside type="note">
  ダッシュボードのパラメーターダイアログには **Save to user secret** チェックボックスが含まれています。
