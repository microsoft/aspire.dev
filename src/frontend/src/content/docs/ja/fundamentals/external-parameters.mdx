...
<Aside type="caution" title="WithCustomInput は実験的な API です">
  `WithCustomInput` メソッドは実験的なインタラクション サービス API を使用しており、`[Experimental("ASPIREINTERACTION001")]` 属性でマークされています。この API を使用するには、`#pragma warning disable ASPIREINTERACTION001` で警告を抑制する必要があります (`TreatWarningsAsErrors` を有効にしている場合、エラーとして扱われます)。`WithDescription` メソッドは実験的ではなく、このプラグマは必要ありません。診断の詳細やその他の抑制方法については、[ASPIREINTERACTION001](/diagnostics/aspireinteraction001/) をご参照ください。
</Aside>
