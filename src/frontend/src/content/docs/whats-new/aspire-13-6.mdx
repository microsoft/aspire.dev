...
- **Fixed Python callback handle wrapping.** Typed callbacks exported as `Action<IResourceBuilder<T>>` (for example, `configure_container` on `WithRedisCommander`) now invoke wrapper methods on the correct concrete resource type instead of failing at runtime with a raw, unwrapped handle.
