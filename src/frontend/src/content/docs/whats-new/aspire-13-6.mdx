...
Azure Container Apps environments can also opt into the Express preview with `AsExpress`. Express defaults apps to zero minimum replicas and omits the implicit Consumption workload profile, managed Aspire Dashboard, and automatic .NET data-protection configuration. Referenced endpoints must be explicitly public because Express apps have no private `.internal` hostname, and HTTPS ingress is required.
...
<LearnMore>
  ...
  [Kubernetes ingress](/deployment/kubernetes-ingress/), [Kubernetes
  clusters](/deployment/kubernetes/clusters/), and [Express
