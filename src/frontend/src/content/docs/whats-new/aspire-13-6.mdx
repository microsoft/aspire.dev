...
- **Sharper CLI workflows** — select launch profiles, export canonical TypeScript API data, run bundled NuGet operations in-process with credential provider support, preserve acquisition channels, and observe resources more reliably.
...
### In-process NuGet operations, with credential provider support

Bundled Aspire CLI installs no longer start a separate `aspire-managed` process to search for, restore, or inspect NuGet packages. The Native AOT `aspire` executable now calls the NuGet client libraries in-process, which removes a process hop while keeping the same package source mapping, signature verification, dependency resolution, and manifest generation behavior.

This also fixes an authentication gap: the previous `aspire-managed` helper never initialized NuGet's credential service, so bundled search and restore could only use credentials already stored in `nuget.config`. Feeds that require a credential provider plugin—such as Azure Artifacts—returned `401` errors. The in-process client now initializes the credential service, so installed credential provider plugins are used for bundled NuGet operations.

<Aside type="note">
  Non-bundled package search (used when the .NET SDK is available) is
  unaffected—it continues to call `dotnet package search`.
</Aside>
