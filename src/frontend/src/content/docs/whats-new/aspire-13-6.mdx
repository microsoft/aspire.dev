...
- **Executable references to container network endpoints.** Projects and executables can now resolve `KnownNetworkIdentifiers.DefaultAspireContainerNetwork` endpoints for themselves or other executables. Aspire creates the container tunnel for the referenced endpoint and shares its allocation, so configuration resolves instead of waiting indefinitely.
...
<LearnMore>
  ...
  [container network endpoints for executables](/fundamentals/container-networking/#container-network-endpoints-for-executables).
</LearnMore>
