...
<Aside type="caution" title="Narrow deployment target">
  ...
  and security review are also follow-up work. The `Aspire.Hosting.Azure.Sandboxes`
  package remains prerelease, but its APIs no longer require suppressing
  `ASPIREAZURE001` because the entire package is already prerelease.
</Aside>
...
<Aside type="note">
  Radius recipe APIs and the Dotnet project Blazor gateway remain experimental.
  Azure Container Apps Sandboxes no longer requires suppressing `ASPIREAZURE001`;
  the package itself remains prerelease.
</Aside>
