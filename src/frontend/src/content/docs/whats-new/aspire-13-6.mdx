...
- **Clearer startup error reporting.** Container runtime startup failures now include the DCP-provided reason (for example, an unsupported Docker CLI version) in the default warning and thrown exception, not just at debug verbosity. Executable process-start failures also include the resolved working directory in system logs, so an invalid working directory no longer looks like an invalid executable path.
