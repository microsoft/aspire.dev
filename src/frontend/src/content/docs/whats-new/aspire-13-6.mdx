...
- **Sharper CLI workflows** — update repository-pinned CLIs, select launch profiles, export TypeScript API data, automate terminals, and run a faster native agent telemetry hook.
...
- **Breaking changes** — review Cosmos DB emulator defaults, Front Door origin names, the smaller default OTLP request size, portable connection-string aliases, and the experimental terminal namespace move.
- …and much more.
...
<Aside type="caution">
  ...
  if you use the Cosmos DB emulator, Azure Front Door, large OTLP telemetry
  batches, custom connection-string consumers, or experimental terminal APIs.
</Aside>
...
#### Faster agent telemetry hook

The `PostToolUse` hook installed by `aspire agent init` now runs the CLI's native `aspire agent telemetry --hook` executable directly instead of the previous shell-script wrapper, removing shell startup and synchronous upload waits from every agent tool call. GitHub Copilot CLI and Claude Code hook registrations still match every tool call (`matcher: "*"` for Claude, unfiltered `postToolUse` for Copilot); coverage of Aspire skills, MCP tools, and skill reference files is unchanged. Refresh existing hook registrations after upgrading:

```bash title="Aspire CLI — Refresh agent hook configuration"
aspire agent init
```

Set `ASPIRE_AGENT_TELEMETRY_MAX_PAYLOAD_CHARACTERS` to change the maximum size of the hook input the CLI reads before parsing. The default is `65536` UTF-16 characters, and the accepted range is `1`-`1048576`; oversized input is drained without being retained, and an invalid value produces a diagnostic without breaking the agent.

<LearnMore>
  See [AI agent skill usage
  data](/reference/cli/microsoft-collected-cli-telemetry/#ai-agent-skill-usage-data).
</LearnMore>
...
#### OpenTelemetry packages upgraded, smaller default OTLP request size

Aspire 13.6 coordinates its bundled OpenTelemetry packages (the Console, in-memory, and OTLP exporters, `Extensions.Hosting`, ASP.NET Core/HTTP/Runtime/SQL Client instrumentation, and gRPC client instrumentation) on the `1.18.x` line, and the Azure Monitor Exporter on `1.9.0`. Generated `ServiceDefaults` projects pick up the updated versions; existing customer projects aren't rewritten.

OpenTelemetry 1.18 reduces the default maximum serialized OTLP request size from 128 MiB to 64 MiB. Oversized telemetry batches are dropped instead of exported. If your app produces large batches (for example, high-cardinality traces or bulk log export), restore the previous limit:

```csharp title="AppHost.cs or Program.cs"
...
The default maximum OTLP response size remains 4 MiB.

<LearnMore>
  See [Telemetry in Aspire](/fundamentals/telemetry/) and the [OpenTelemetry .NET
  exporter
