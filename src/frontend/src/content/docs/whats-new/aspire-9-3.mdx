...
import dashboardCopilot from '@assets/whats-new/aspire-9.3/dashboard-copilot.png';
import dashboardFilter from '@assets/whats-new/aspire-9.3/dashboard-filter.png';
import dashboardMetricsWarning from '@assets/whats-new/aspire-9.3/dashboard-metrics-warning.png';
import dashboardTracesUninstrumentedResources from '@assets/whats-new/aspire-9.3/dashboard-traces-uninstrumented-resources.png';
import dashboardContextMenu from '@assets/whats-new/aspire-9.3/dashboard-context-menu.png';
...
Historically the only way to keep a resource out of the Dashboard was to put it in the **`Hidden`** _state_—a hack that also made the resource look "terminal" to APIs such as `WaitForResourceAsync`. In 9.3 every snapshot now carries a **boolean `IsHidden` flag**, completely decoupling _visibility_ from _lifecycle state_.
...
<Aside type="tip">
  💡 Want to learn more about authoring YARP configs? See the official [YARP
  documentation](https://aka.ms/yarp). 🧪 This integration is in preview—APIs
  and behavior may evolve. Feedback welcome!
</Aside>
...
<Image
  src={dashboardCopilot}
  alt="Screenshot of dashboard with GitHub Copilot chat open."
/>
...
<Image
  src={dashboardFilter}
  alt="Screenshot of dashboard resource's page showing the filter popup."
/>
...
<Aside type="caution">
  💡 This is especially useful for services like SQL Server, PostgreSQL, Redis,
  or blob storage where outgoing client telemetry exists, but the service itself
  doesn't participate in distributed tracing.
</Aside>

🧪 No instrumentation changes are needed—Aspire infers the mapping based on resource references.

<Image
  src={dashboardTracesUninstrumentedResources}
  alt="Screenshot of dashboard traces's page showing added resources in results."
/>
...
<Image
  src={dashboardContextMenu}
  alt="Screenshot of dashboard resource graph view with a context menu."
/>
...
<Image
  src={dashboardMetricsWarning}
  alt="Screenshot of dashboard metrics page with a pause warning."
/>
...
<Aside type="note">
  In multi-replica scenarios, Aspire still uses full replica IDs so you can
...
<Aside type="tip">
  ✅ This architectural shift lays the groundwork for **hybrid and heterogeneous
  deployments**, where different services within the same app can be deployed to
  different targets (cloud, edge, local).
</Aside>
...
<Aside type="note">
  💡 Imagine a real-world case where your frontend is deployed to a CDN or
  GitHub Pages, and your backend runs in Azure Container Apps. This new model
...
<Aside type="tip">
  🧠 This tightly couples your infrastructure parameters with the Docker Compose
  model—without hardcoding values—unlocking composability across environments.
</Aside>
...
<Aside type="caution">
  🧠 Aspire emits standard Kubernetes manifests under the hood—you can still use
  `kubectl`, `helm`, or GitOps workflows to deploy them, but now you can shape
  them directly from your app definition.
</Aside>
...
<Aside type="note">
  ⚠️ **The Aspire 9.3 CLI is not compatible with Aspire 9.2 projects.** You must
  upgrade your project to Aspire 9.3+ in order to use the latest CLI features.
</Aside>
...
<Aside type="caution">
  📢 Hosted dashboard support is coming soon—we're actively developing this.
  Feedback is welcome!
</Aside>
...
Aspire 9.3 changes the default SKU used when provisioning **Azure SQL databases** to the **GP_S_Gen5_2** (General Purpose Serverless) tier with the [**Free Offer**](https://learn.microsoft.com/azure/azure-sql/database/free-offer?view=azuresql&preserve-view=true). This helps reduce unexpected costs during development and experimentation.

Previously, Aspire defaulted to the **General Purpose (GP)** tier _without_ the Free Offer, which could incur charges even for small or test apps.
