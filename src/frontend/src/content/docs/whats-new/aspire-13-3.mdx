...
Aspire 13.3 is here, and it's a packed release. This is one of the largest Aspire releases yet — bringing **Azure Front Door** and **AKS** as first-class hosting targets, **Network Security Perimeter** support for Azure resources, **Kubernetes Ingress and Gateway routing** with Helm-based deployment, the **container tunnel** enabled by default, deeper **polyglot AppHost** parity for TypeScript, Python, and Java, and a brand-new **`aspire destroy`** command for tearing down deployments.
...
- **Azure Front Door**, **Network Security Perimeter**, and **Azure Kubernetes Service (AKS)** hosting integrations.
...
- New **EF Core migration management** integration, **`WithBrowserLogs()`** for capturing browser console logs and screenshots, and **Foundry Prompt Agent** support.
