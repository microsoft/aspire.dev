...
### `WithEnvironment` API unification and compatibility shims
...
Aspire 13.3 introduces a unified `withEnvironment(name, value)` API for polyglot AppHosts (TypeScript, Java, Python, Go, Rust). The unified signature accepts a `value` of:
...
```typescript title="TypeScript — before (older helpers)"
...
Use the unified method for new code, and migrate shimmed helpers incrementally.
