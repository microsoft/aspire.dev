...
- **Unified `withEnvironment` API** — a single method now handles all environment variable value kinds (endpoints, parameters, connection strings, expressions). Apps using the earlier per-kind helpers must migrate to the unified method.
...
<Aside type="tip">
  For SSR frameworks that need `node_modules` at runtime (Nuxt, Remix, Astro
...
The extension now renders live resource state right in the editor. **CodeLens** above each resource definition exposes one-click `Start`, `Stop`, `Restart`, and `View Logs` actions, and **gutter decorations** in the left margin reflect each resource's current health at a glance. The same health-aware icons appear next to resources in the **Aspire** Activity Bar.
