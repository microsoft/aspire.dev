...
description: Aspire 13.3 introduces Azure Network Security Perimeter support, enables the container tunnel by default, removes the in-dashboard GitHub Copilot UI, and more.
sidebar:
  label: Aspire 13.3
  order: 0
tableOfContents:
...
### In-dashboard GitHub Copilot UI removed

Starting with Aspire 13.3, the GitHub Copilot-based chat UI previously built into the Aspire dashboard has been removed.

In earlier versions of Aspire, the dashboard could show a GitHub Copilot button in the top-right corner when your app was launched from Visual Studio or VS Code with a GitHub Copilot subscription. Context menus throughout the dashboard also offered **"Ask GitHub Copilot"** and **"Explain error"** actions for dashboard telemetry. These dashboard UI entry points are no longer available.

If you previously configured `ASPIRE_DASHBOARD_AI_DISABLED` or `Dashboard:AI:Disabled`, those settings no longer restore the in-dashboard Copilot UI. The settings are reserved for future use.

The recommended replacement is to use [Dashboard and AI coding agents](/dashboard/ai-coding-agents/) with the [Aspire CLI and agent skills](/get-started/ai-coding-agents/). This gives GitHub Copilot, Claude, Cursor, and other supported AI coding agents access to your app's structured logs, distributed traces, resource status, and console output through skills and MCP tools.

Set up agent integration with a single command:

```bash title="Aspire CLI — Initialize AI agent configuration"
aspire agent init
```

The `aspire agent init` command detects your development environment and configures skills and MCP tools for supported AI coding agents. Agents connect to the dashboard through the CLI regardless of how you launched your app.
