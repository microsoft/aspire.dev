...
resourceImage: ~/assets/whats-new/aspire-13.3/notification-center-open.png
---
...
Aspire 13.3 expands deployment and observability with `aspire destroy`, browser telemetry and screenshots, preview Kubernetes deployment through a Helm engine, and first-class JavaScript publishing. It also enables the container tunnel by default and brings TypeScript AppHosts closer to C# parity.
...
- The new `aspire destroy` command tears down provisioned resources across Azure, Kubernetes, and Compose, while `aspire dashboard` runs the dashboard standalone. The Aspire CLI is also now available as a `dotnet tool`.
- The new `Aspire.Hosting.Browsers` integration captures frontend console logs, network requests, and screenshots and surfaces them in the dashboard next to server-side telemetry.
- Preview support for `aspire deploy` to Kubernetes generates a Helm chart and runs an end-to-end deployment pipeline from an `AddKubernetesEnvironment`. New Ingress and Gateway API routing resources describe traffic at the AppHost level.
- Preview JavaScript publish methods provide a unified `PublishAs*` family for static SPAs, SSR Node servers, and npm-script-based deployments, plus the new `AddNextJsApp` helper and first-class support for Bun, Yarn, and pnpm.
...
- TypeScript AppHost improvements close most of the remaining gap with C# AppHosts.
- The preview `aspire init` command bootstraps an AppHost into an existing repository with the new `aspireify` agent skill.
- New hosting integrations support Azure Front Door, Network Security Perimeter, and Azure Kubernetes Service (AKS). This release also adds Foundry Prompt Agent support.
- …and much more.
...
<Aside type="caution">
  Aspire 13.3 includes several breaking changes — most notably the rename of
...
```bash title="Install via dotnet tool"
...
<Aside type="note">The new `aspire init` flow is in **preview** in 13.3.</Aside>
...
<Aside type="caution">
  This is a behavior change from prior releases — `aspire init` no longer fully
...
- `--list-steps` prints the pipeline steps that would run for `aspire do`, `aspire publish`, `aspire deploy`, or `aspire destroy` without executing them.
- The built-in `check-container-runtime` pipeline step fails fast when no container runtime is available, preventing late-stage build or publish failures.
- Independent steps continue when a sibling fails, so a single failed step no longer blocks unrelated work in the same pipeline run.

### Improved AI agent init

`aspire agent init` gained a location selection step and now installs a refreshed standard skill set:

- The `aspire` skill orchestrates Aspire apps through the CLI by starting and stopping the AppHost, inspecting resources, viewing logs and telemetry, and adding integrations.
...
- The CLI's global configuration now validates AppHost paths to prevent accidentally pointing at the wrong project.
- A container runtime health check runs before `aspire deploy` to catch missing or broken Docker and Podman setups early.
- `aspire ps` displays the dashboard URL alongside running AppHosts.
- Many CLI commands have new options that improve support for `--non-interactive`.
...
- Azure resources are deleted through Azure Resource Manager.
- Kubernetes Helm releases and namespaces created during deployment are uninstalled.
- Published Docker Compose stacks are stopped and removed.
...
<Aside type="note" title="Startup delay">
  Containers that reference host-based endpoints will start slightly later (~3–4
...
<OsAwareTabs>
  <Fragment slot="unix">
    ...
    ```bash title="Disable container tunnel"
  ...
  <Fragment slot="windows">
    ...
    ```powershell title="Disable container tunnel"
    # Set the env var only for this aspire run invocation, then start the AppHost.
...
<Aside type="note">
  Native Kubernetes deployment (and the new Ingress / Gateway routing resources
...
<LearnMore>
  ...
  Kubernetes](/deployment/kubernetes/clusters/).
</LearnMore>
...
```csharp title="Ingress with cert-manager"
...
```typescript title="Ingress with cert-manager"
import { createBuilder } from './.aspire/modules/aspire.mjs';
...
```csharp title="Privileged Docker Compose service"
...
```typescript title="Privileged Docker Compose service"
import { createBuilder } from './.aspire/modules/aspire.mjs';
...
`HttpCommand` now supports a result mode that returns the response body to the dashboard's notification center, so HTTP-triggered commands can return structured payloads instead of just success or failure:

<Tabs syncKey='aspire-lang'>
<TabItem id='csharp' label='AppHost.cs'>

```csharp title="HTTP command with result body"
...
```typescript title="HTTP command with result body"
import { createBuilder } from './.aspire/modules/aspire.mjs';
...
```csharp title="Issue an access token from a resource command"
...
```typescript title="Issue an access token from a resource command"
import {
  createBuilder,
  CommandResultFormat,
...
```csharp title="Subscribe to lifecycle events"
...
```typescript title="Subscribe to lifecycle events"
import { createBuilder } from './.aspire/modules/aspire.mjs';
...
```csharp title="Exclude an endpoint from WithReference"
...
```typescript title="Exclude an endpoint from withReference"
import { createBuilder } from './.aspire/modules/aspire.mjs';
...
- A unified `withEnvironment` API handles all environment variable value kinds, including endpoints, parameters, connection strings, and expressions. Apps using the earlier per-kind helpers must migrate to the unified method.
...
- `PublishAsDockerComposeService`-equivalent hooks and post-processing provide Docker Compose API parity.
- Azure Container Apps custom domain configuration is now exposed in the TypeScript AppHost SDK.
- Unified YARP TypeScript route helpers, `addRoute(...)` and `addCatchAllRoute(...)`, replace several earlier ad-hoc helpers.
...
- The new `[AspireValue]` attribute and predefined catalogs (for example, `FoundryModels.OpenAI.Gpt41Mini`) make it easier to reference well-known values from TypeScript AppHosts.
- Diagnostic [`ASPIREEXPORT013`](/diagnostics/aspireexport013/) detects duplicate exported capability IDs at build time.

### Unified `withEnvironment` API

Aspire 13.3 introduces a unified `withEnvironment(name, value)` API for non-C# AppHosts (TypeScript, Java, Python, Go, Rust). The public TypeScript API accepts plain strings, reference expressions, endpoint references, parameter resources, supported resources that expose connection strings, expression values, and awaitable forms of supported values:

```typescript title="apphost.mts"
const primaryEndpoint = await cache.primaryEndpoint();
const api = await builder.addProject('api', '../Api/Api.csproj');
await api
  .withEnvironment('SERVICE_URL', primaryEndpoint)
  .withEnvironment('API_KEY', apiKeyParam)
  .withEnvironment('DB', database);
```

The 13.3 TypeScript SDK doesn't generate the earlier per-kind helper aliases. Update every existing call site to use `withEnvironment(name, value)` when you upgrade.
...
- `PublishAsStaticWebsite` (preview) publishes a JS app as a static website served by a YARP image, with an optional API reverse proxy to a backend resource. It is intended for SPAs such as Vite and plain Next.js exports.
- `PublishAsNodeServer` publishes a self-contained Node entry point such as `server.js` without copying `node_modules` at runtime. It is intended for pre-bundled Node servers.
- `PublishAsNpmScript` publishes a Node app that runs an npm script (`start` or `serve`) at runtime, copying `package.json` and production dependencies. It is intended for full Nitro Next.js, Remix, and Astro SSR apps.
...
<Aside type="caution">
  The `PublishAsStaticWebsite` method is experimental in 13.3. Use diagnostic
...
For SSR frameworks that need `node_modules` at runtime, including Nuxt, Remix, Astro SSR, and Qwik, use `PublishAsNpmScript` instead. It copies `package.json` and runs `npm start`.
...
The dashboard now has a built-in Rebuild command for container and project resources. When triggered, it rebuilds the resource image and restarts the resource, making it faster to pick up code or Dockerfile changes without stopping and restarting the whole AppHost. The result (success or failure) appears in the notification center.

<Aside type="caution">
  Aspire 13.3 removes the in-dashboard GitHub Copilot UI. Please review the
...
### AppHost CodeLens and gutter decorations

The extension now renders live resource state in the editor. CodeLens above each resource definition exposes one-click `Start`, `Stop`, `Restart`, and `View Logs` actions, while gutter decorations in the left margin reflect each resource's current health. The same health-aware icons appear next to resources in the Aspire Activity Bar.

<ThemeImage
  dark={extensionSidebar}
  light={extensionSidebarLight}
  alt="The Aspire view in VS Code showing a tree of resources with health-aware icons reflecting each resource's state."
/>

### Other extension changes

- Set `aspire.enableAutoRestore` to have the extension automatically run `aspire restore` when a workspace opens or its configuration changes.
- Launch configurations can now specify `env` and `args` for the AppHost, just like a regular .NET launch configuration.
- Open AppHost source from the _Running AppHosts_ view.
- Open the Aspire Dashboard from the VS Code Command Palette.
- Use right-click context menus on resource endpoint URLs to open or copy them.
...
<Aside type="caution">
  This integration is experimental in 13.3. Use diagnostic code
...
<Aside type="note">
  The AKS hosting integration is in **preview** and may evolve before it's
  marked stable.
</Aside>

Aspire 13.3 adds first-class support for provisioning and deploying to Azure Kubernetes Service without writing manifests by hand. Use `AddAzureKubernetesEnvironment` to declare an AKS environment, customize node pools, and generate a type-safe Bicep and Helm deployment pipeline from your AppHost.
...
Aspire 13.3 ships a new working implementation of Azure AI Foundry Prompt Agent support through `AddPromptAgent`. The previous `AddAndPublishPromptAgent` API and the original shape of `AzurePromptAgentResource` (which never functioned end-to-end) have been replaced. `AddPromptAgent` now returns a working `AzurePromptAgentResource`.
...
- Multi-environment deployments now require explicit compute environment binding. Every resource must specify which environment it deploys to, preventing accidental cross-environment leakage.
...
- The AppHost is now authored in TypeScript, so the **.NET SDK is no longer required** to scaffold or run the Python starter.
...
<Aside type="caution" title="Breaking change">
  `dotnet new aspire-py-starter` is no longer available. Existing projects
...
| TypeScript AppHost per-kind `withEnvironment*` helper methods removed                                           | Replace every per-kind helper call with the unified `withEnvironment(name, value)` API.            |
...
1. Update the CLI by running `aspire update --self`.
2. Update your projects by running `aspire update` from the root of your repository.
3. Audit `--log-level` usage in CI/CD pipelines and rename it to `--pipeline-log-level`.
4. Search for `NameOutput`, `AddAndPublishPromptAgent`, and `AksSkuTier` in your AppHost code and update them according to the table above.
...
7. Re-pin Node versions in your Dockerfiles if you relied on `package.json` `engines.node` for image selection.

</Steps>

We'd love to hear about your experience with Aspire 13.3. Share what you're building on [GitHub](https://github.com/microsoft/aspire/issues) or join us on [Discord](https://aka.ms/aspire-discord).
