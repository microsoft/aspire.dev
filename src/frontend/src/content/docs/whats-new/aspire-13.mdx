...
Unlike the deprecated `AddNpmApp` API, `AddJavaScriptApp` does not support an `args` parameter for passing command-line arguments directly to scripts. Instead, define custom scripts in your `package.json` that include the necessary arguments:

```json title="package.json — Define custom scripts with arguments"
{
  "scripts": {
    "dev": "vite",
    "hasura:console": "hasura console --no-browser --admin-secret mysecret"
  }
}
```

Then reference the custom script in your AppHost:

```csharp title="C# — Use custom script with arguments"
...
builder.AddJavaScriptApp("hasura-console", "../Web/ClientApp", runScriptName: "hasura:console");
```

This approach keeps all script configuration in `package.json`, making your scripts more discoverable and easier to run outside of Aspire (e.g., `npm run hasura:console`).
