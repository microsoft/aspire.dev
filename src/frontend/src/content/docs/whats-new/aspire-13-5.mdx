...
- **AI Assistant chat removed**: The remaining in-dashboard AI Assistant chat UI, `AssistantChat`, `AssistantModalDialog`, `AssistantSidebarDialog`, `ExplainErrorsButton`, and all related configuration, services, telemetry, and translations have been removed. This completes the migration to the agentic development model started in Aspire 13.3—AI coding agents connect to your Aspire app through the [Aspire CLI and MCP server](/dashboard/ai-coding-agents/) instead.
...
7. **Dashboard AI Assistant chat removed**: The `Dashboard:AI:Disabled` configuration option, along with the underlying AI Assistant chat components and services, has been removed from the dashboard. If you referenced this option, remove it—it no longer has any effect. Use the [Aspire CLI and MCP server](/dashboard/ai-coding-agents/) for AI-assisted debugging instead.
