...
- **Improved metrics chart filter rendering**: Dimension tag filters now display up to 20 values inline, with a **+N** overflow badge for any additional values. Values are ordered numerically when all values parse as numbers, or alphabetically otherwise. This prevents browser reflows that could disconnect SignalR when a dimension has hundreds of tag values.
