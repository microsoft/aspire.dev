...
- **Stable channel skips nuget.config creation**: `aspire new` and `aspire update` no longer create a project-level `nuget.config` when using the stable channel, since nuget.org is already the default NuGet source. If an existing `nuget.config` is present it is still updated to clean up stale feed entries. Non-stable channels (`staging`, `daily`) continue to create `nuget.config` with channel-specific source mappings.
