...
- **Improved `aspire docs search` relevance**: The docs search ranker now normalizes version separators (so `whats new 13.5`, `13-5`, and `135` all match the same pages), applies IDF weighting so common terms like `new` or `azure` count less than specific terms, and rewards exact phrase matches in page titles, slugs, and headings. Top-1 accuracy on an offline corpus improved from ~32% to ~87%. See [`aspire docs search`](/reference/cli/commands/aspire-docs-search/).
