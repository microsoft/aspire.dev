...
- **Design-token system and UX polish**: The Dashboard's colors, spacing, radii, and typography now come from a shared `tokens.css` design-token layer, with self-hosted Geist and Poppins fonts, a collapsible left navigation rail, left-aligned page titles, and responsive toolbars that stack on tablet/mobile widths. A new automated accessibility (axe-core) test suite guards WCAG 2.x A/AA compliance across core pages, themes, and dialogs.
