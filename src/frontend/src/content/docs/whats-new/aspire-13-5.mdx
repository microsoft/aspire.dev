...
- **Open terminal for terminal-enabled resources**: Resources configured with `WithTerminal()` now show an **Open terminal** item in the right-click context menu. Selecting it runs `aspire terminal attach <resource>` and opens an editor-style terminal tab in VS Code.
