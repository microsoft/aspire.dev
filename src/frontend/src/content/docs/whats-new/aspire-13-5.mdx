...
`WithServiceDelegation` now defaults its delegation `name` to `serviceName`, so you no longer repeat the service string, and `AzureSubnetServiceDelegations` provides discoverable constants (`ContainerInstances`, `ContainerAppEnvironments`, `AppServiceEnvironments`, `ApplicationGatewayForContainers`) for well-known Azure services instead of hardcoding resource provider strings.
