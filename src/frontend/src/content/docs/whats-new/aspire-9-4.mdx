...
resourceImage: ~/assets/whats-new/aspire-9.4/dashboard-connection-strings.png
...
import dashboardInteractionService from '@assets/whats-new/aspire-9.4/dashboard-interaction-service.gif';
import dashboardHiddenResources from '@assets/whats-new/aspire-9.4/dashboard-hidden-resources.png';
import dashboardParameters from '@assets/whats-new/aspire-9.4/dashboard-parameters.png';
import dashboardUpdateNotification from '@assets/whats-new/aspire-9.4/dashboard-update-notification.png';
import dashboardConnectionStrings from '@assets/whats-new/aspire-9.4/dashboard-connection-strings.png';
import dashboardTracingPeers from '@assets/whats-new/aspire-9.4/dashboard-tracing-peers.png';
import dashboardConsoleLogsWrapping from '@assets/whats-new/aspire-9.4/dashboard-console-logs-wrapping.gif';
...
If you have feedback, questions, or want to contribute to Aspire, collaborate with us on [GitHub](https://github.com/microsoft/aspire) or join us on our new [Discord](https://aka.ms/aspire-discord) to chat with the team and other community members.
...
<span id='-upgrade-to-aspire-94'></span>
...
1. In your AppHost project file (that is, _MyApp.AppHost.csproj_), update the [📦 Aspire.AppHost.Sdk](https://www.nuget.org/packages/Aspire.AppHost.Sdk) NuGet package to version `9.4.0`:
    ...
    For more information, see [Aspire SDK](`microsoft/aspire/sdk`).
...
The `aspire deploy` command introduced extensible deployment workflows, enabling custom logic and richer integration with external systems during deployment operations.

**Key capabilities:**

- **Custom deployment steps** using `WithPipelineStepFactory` to execute custom logic during the `aspire deploy` command
- **Workflow activity reporting** through tasks created from `PipelineStepContext.ReportingStep`, with prompts provided by `IInteractionService`
- **Pipeline ordering and dependencies** using `dependsOn`, `requiredBy`, and well-known pipeline steps

The example below shows the current pipeline-step approach for custom deployment behavior and showcases [CLI-based prompting](#-enhanced-publish-and-deploy-output) and progress notifications.
...
internal static class DataSeedJobBuilderExtensions
{
        ...
        string seedDataPath = "data/seeds")
    {
        ...
        resourceBuilder.WithPipelineStepFactory("seed-initial-data", async ctx =>
        {
            CancellationToken ct = ctx.CancellationToken;

            // Prompt the user for deployment input using the interaction service
            ...
            // ReportingStep is the progress-reporting surface for this pipeline step.
            var seedTask = await ctx.ReportingStep.CreateTaskAsync($"Loading seed data from {seedDataPath}", ct);
            ...
            try
            {
                ...
                await seedTask.SucceedAsync("Seed data loaded", ct);
            }
            catch (Exception ex)
            {
                await seedTask.FailAsync(ex.Message, ct);
                throw;
            }
...
![Terminal output showing the aspire deploy command running the custom deployment steps in sequence](https://github.com/user-attachments/assets/15c6730d-8154-496a-be70-c67257ce5523)
...
<Aside type="note">
In Aspire 9.4, this feature was preview-only and built-in resources did not yet natively support deployment callbacks. In current releases, use pipeline steps for custom deployment behavior.
</Aside>
...
<span id='-interaction-service'></span>
...
<Image src={dashboardInteractionService} alt="Recording of using the interaction service in the dashboard." />
...
Aspire 9.4 improves support for [persistent containers](https://aspire.dev/app-host/resource-lifetimes/) with better lifecycle management and networking capabilities, ensuring containers can persist across application restarts while maintaining proper connectivity.
...
Aspire 9.4 improves [Docker Compose publish](https://aspire.dev/deployment/deploy-with-aspire/) security with smart port mapping - only external endpoints are exposed to the host while internal services use Docker's internal networking.
...
<Image src={dashboardUpdateNotification} alt="Screenshot of dashboard showing an update notification." />
...
<Image src={dashboardConnectionStrings} alt="Screenshot of dashboard showing connection string." />

External parameters are no longer hidden. The parameter state and value is visible in the dashboard.

<Image src={dashboardParameters} alt="Screenshot of dashboard showing parameters." />

For more information, see [external parameters](/fundamentals/external-parameters/).
...
<Image src={dashboardTracingPeers} alt="Screenshot of a span linked to a GitHub model resource defined in Aspire." />
...
<Image src={dashboardConsoleLogsWrapping} alt="Recording of toggling line wrapping on console logs page." />
...
<Image src={dashboardHiddenResources} alt="Dashboard resources page with the show/hide hidden resources UI visible." />
...
The [GitHub Models integration](https://aspire.dev/integrations/ai/github-models/github-models-get-started/) provides:
...
<Aside type="note">
  These Aspire 9.4 examples intentionally use `AddAzureAIFoundry`, the hosting
  API available in that release. Aspire 13.2 renamed it to `AddFoundry`; see the
  [Foundry package and namespace
  transition](/whats-new/aspire-13-2/#foundry-package-and-namespace-transition).
</Aside>

#### Client integration

Once you've configured the [Azure AI Foundry resource](https://aspire.dev/integrations/cloud/azure/azure-ai-foundry/azure-ai-foundry-get-started/) in your AppHost, consume it in your services using the [Azure AI Inference SDK](https://aspire.dev/integrations/cloud/azure/azure-ai-inference/azure-ai-inference-get-started/) or [OpenAI SDK](https://aspire.dev/integrations/cloud/azure/azure-openai/azure-openai-get-started/) for compatible models:
...
Aspire 9.4 introduces significant improvements to the [Azure Key Vault integration](https://aspire.dev/integrations/cloud/azure/azure-key-vault/azure-key-vault-get-started/) with new secret management APIs that provide strongly typed access to secrets:
...
Aspire 9.4 introduces **OpenTelemetry tracing support** for [Azure App Configuration](https://aspire.dev/integrations/cloud/azure/azure-app-configuration/azure-app-configuration-get-started/), completing the observability story for this integration. The Azure App Configuration integration now automatically instruments configuration retrieval operations and refresh operations with distributed tracing.
...
**Migration impact**: Replace auto-injected parameters with explicit resource modeling for better resource graph representation and support for multiple Azure compute environments. See [Azure resource customization docs](/integrations/cloud/azure/customize-resources/) for more details.
...
Aspire is built in the open, and this release wouldn't be what it is without you. A huge thank you to everyone who helped make Aspire 9.4 possible.

<ReleaseCommunity version="9.4" />
