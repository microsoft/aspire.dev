...
internal static class DataSeedJobBuilderExtensions
{
        ...
        string seedDataPath = "data/seeds")
    {
        ...
        resourceBuilder.WithPipelineStepFactory("seed-initial-data", async ctx =>
        {
            ...
            // Use the reporting step to report progress on the seeding process
            var task = await ctx.ReportingStep.CreateTaskAsync($"Loading seed data from {seedDataPath}", ct);
