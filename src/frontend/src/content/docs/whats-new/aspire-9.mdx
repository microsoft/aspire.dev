...
For more information, see [Add Redis resource with Redis Insights](/integrations/caching/redis/redis-host/#add-redis-resource-with-redis-insights).
...
Set a breakpoint on the first `logger.LogInformation` line of the `Run` method and press <kbd>F5</kbd> to start the Functions host. Once the Aspire dashboard launches, you observe the following:
