...
Aspire 13.2 introduces Docker Compose as a publishing target. When you add a Docker Compose environment to your app model, Aspire generates a `docker-compose.yaml` from your resources at publish time, making it easy to deploy your Aspire application to any environment that supports Docker Compose.
...
<Aside type="note">
  The Foundry hosting APIs are not yet available in TypeScript AppHosts.
</Aside>
