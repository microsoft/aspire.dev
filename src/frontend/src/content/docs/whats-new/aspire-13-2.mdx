...
### Language-specific templates and polyglot setup
...
This unified format replaces the older split across `.aspire/settings.json` and `apphost.run.json` and makes polyglot AppHosts much easier to move, inspect, and automate.
...
- The unified self-extracting Aspire CLI bundle continues to improve Windows and polyglot experiences, especially when .NET isn't already installed globally.
...
Project reference support and broader exported hosting APIs also make polyglot AppHosts less special-cased than they were in 13.1. If you're building on TypeScript or JavaScript AppHosts, 13.2 is a meaningful step up in day-to-day usability.
...
This all adds up to a much more polished Aspire experience in VS Code, especially for teams working on polyglot solutions or jumping frequently between the terminal, the dashboard, and the editor.
...
`ReferenceExpression` now exposes `GetValueAsync()` and supports conditional mode in polyglot code generation, which makes advanced hosting extensions easier to build without relying on synchronous value access patterns.
...
- Better polyglot export coverage for Azure integrations such as Cosmos DB and AppContainers
...
Follow the directions in [Upgrade to Aspire 13.2](#upgrade-to-aspire-132) to move forward and start using the new CLI, polyglot AppHost, dashboard, hosting, and integration capabilities in this release.

For a complete list of issues addressed in this release, see the [Aspire 13.2 milestone on GitHub](https://github.com/dotnet/aspire/issues?q=is%3Aissue%20state%3Aclosed%20milestone%3A13.2).