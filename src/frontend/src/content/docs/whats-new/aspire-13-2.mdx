...
```bash title="Bash — Detached mode and process management"
...
```bash title="Bash — Environment diagnostics"
aspire doctor
```
...
```bash title="Bash — Agent integration"
...
```bash title="Bash — Configuration management"
...
```bash title="Bash — Update to Aspire 13.2"
...
```bash title="Bash — Install Aspire CLI"
curl -sSL https://aspire.dev/install.sh | bash
```
