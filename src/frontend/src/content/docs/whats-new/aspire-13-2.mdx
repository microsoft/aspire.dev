...
Aspire 13.2 introduces a wave of new CLI commands that dramatically expand what you can do from the terminal. The CLI is evolving into a comprehensive platform for managing, observing, and debugging your Aspire applications.
...
# Use a git repository as a template source
aspire new --template https://github.com/myorg/aspire-template
...
- Aspire SDK installation and version
...
### Non-interactive mode for CI/CD and agents

The `--non-interactive` flag received significant refinement in 13.2. When passed, all interactive prompts are suppressed—commands that would normally ask for user input instead auto-select when there's a single option, or return a clear error message when disambiguation is required. The animated startup banner is also suppressed in non-interactive environments.

```bash title="Aspire CLI — Non-interactive mode"
# Run without any interactive prompts
aspire run --non-interactive

# Stop the running apphost non-interactively (auto-selects if only one)
aspire stop --non-interactive

# Combine with --format Json for fully scriptable output
aspire ps --non-interactive --format Json
```

Non-interactive mode is detected automatically from the `--non-interactive` flag, but can also be triggered by standard CI environment indicators. This makes the Aspire CLI much more reliable in CI/CD pipelines, editor extensions, and AI agent toolchains where no human is available to respond to prompts.
...
### Integration scaffolding with `aspire template`

The new `aspire template` command provides scaffolding for creating custom Aspire integrations. If you're building a hosting or client integration package, this gives you a starting point with the right project structure, package references, and boilerplate already wired up.

```bash title="Aspire CLI — Integration scaffolding"
aspire template
```
...
- `aspire new` now supports `--template` with git repository URLs as template sources.
...
One of the most anticipated features in Aspire 13.2 is first-class support for writing your apphost in TypeScript.
...
### Volume mounts for .NET project resources

.NET Project resources can now use volume mounts, not just containers. This is handy for scenarios where a service needs persistent local storage or access to shared data on disk:
...
```typescript title="TypeScript — apphost.ts"
await builder.addProject("api", "../api")
    .withVolume("data", "/app/data");
```
...
<LearnMore>
  ...
  #7153](https://github.com/dotnet/aspire/issues/7153) - [GitHub PR
  #14278](https://github.com/dotnet/aspire/pull/14278) - [Context-based endpoint
...
Aspire 13.2 introduces Docker Compose file import support, letting you bring existing Docker Compose services into your Aspire application. If you already have a `docker-compose.yml` for databases, caches, or third-party services, you can now reference it directly from your apphost instead of re-declaring every container.
...
Imported services appear in the dashboard and participate in the normal resource lifecycle, so you get health checks, logs, and telemetry without extra configuration.

### Durable Task Scheduler

The new Durable Task Scheduler integration adds support for the Durable Task emulator, letting you develop and test durable workflows locally with the same Aspire integration patterns (DI, health checks, telemetry).
...
await builder.addProject("worker", "../worker")
    .withReference(scheduler);
...
The new `Aspire.Hosting.Certbot` integration adds a Certbot container for automated SSL certificate management. This is useful for scenarios where you need Let's Encrypt certificates during local development or in staging environments.
...
const chat = await project.addModelDeployment("chat", "OpenAI.Gpt5Mini");
