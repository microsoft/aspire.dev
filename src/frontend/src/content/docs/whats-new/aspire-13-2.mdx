...
import exportAllButton from '@assets/whats-new/aspire-13.2/export-all-button.png';
...
import { createBuilder } from "@aspect/apphost-sdk";
...
The new **Manage logs and telemetry** dialog (accessible via **Settings → Manage**) provides a centralized interface for managing and exporting telemetry data across all resources.

<Image src={exportAllButton} alt="Manage logs and telemetry dialog showing Export selected, Remove selected, and Import logs and telemetry buttons" />
...
const scheduler = builder.addDurableTaskScheduler("scheduler");
...
const project = foundry.addProject("agents");

const chat = project.addModelDeployment("chat", "OpenAI.Gpt5Mini");
...
const dataLake = storage.addDataLake("data-lake");
const fileSystem = storage.addDataLakeFileSystem("data-lake-file-system");
