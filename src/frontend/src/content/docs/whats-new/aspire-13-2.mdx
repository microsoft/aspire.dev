...
resourceImage:
  light: ~/assets/whats-new/aspire-13.2/resources-light.png
  dark: ~/assets/whats-new/aspire-13.2/resources-dark.png
...
Aspire 13.2 expands the CLI, adds preview TypeScript AppHost support, and improves the dashboard, AI agent integrations, multi-language development, and cloud deployment. It also strengthens the infrastructure used to ship stable releases more frequently. Share questions and feedback on [Discord](https://aka.ms/aspire-discord) or [GitHub](https://github.com/microsoft/aspire/issues).
...
2. Update your Aspire apphost using the [`aspire update` command](/reference/cli/commands/aspire-update/):
...
One of the most requested features in 13.2 is detached mode for running Aspirified apps. You can now run your apphost in the background, freeing up your terminal for other tasks.
...
<LearnMore>
  ...
  describe`](/reference/cli/commands/aspire-describe/).
</LearnMore>
...
<LearnMore>
  For more details on parameters and secrets, see [External parameters](/fundamentals/external-parameters/).
  Command reference: [`aspire secret`](/reference/cli/commands/aspire-secret/).
</LearnMore>
...
- `-v` is now a short alias for `--version`, following standard CLI conventions.
- The `--format` option description is now standardized to "Output format (Table or Json)." across all commands that support it.
- Commands that support log-level filtering now use `--log-level` and `-l` consistently.
...
- Azure Functions projects referenced from an apphost can now be debugged directly.
- TypeScript apphosts now participate in debugging more naturally, with better tool detection, build and launch coordination, and support for browser debugging scenarios.
...
### `aspire.config.json` and <abbr title="Model Context Protocol" data-tooltip-placement="top">MCP</abbr> support
...
<LearnMore>
  For more details, see [Multi-language
...
The new Manage logs and telemetry dialog, available from Settings → Manage, provides one place to export, import, and remove dashboard data. Select resources and telemetry types, then export, remove, or import as needed.

Data is exported as JSON in a zip file. Share it with AI, teammates, or import it back into the dashboard later.

<Image src={manageDialog} alt="Manage logs and telemetry dialog showing Export selected, Remove selected, and Import logs and telemetry buttons" />
...
Export data for specific resources and telemetry directly from the dashboard. Select Export JSON in context menus, then copy the data to your clipboard or download it to your machine.

<Image src={exportJsonMenu} alt="Export JSON button in the resource context menu" />

<Image src={exportJsonDialog} alt="Export JSON dialog with options to copy to clipboard or download" />
...
<Image src={exportEnvDialog} alt="Export Env dialog showing environment variables exported in .env format" />
...
The API is served under `/api/telemetry` and returns data in <abbr title="OpenTelemetry Protocol" data-tooltip-placement="top">OTLP</abbr> JSON format. Available endpoints:
...
<Image src={dashboardSetParameters} alt="Dashboard dialog for setting a parameter value with an option to save to user secrets" />
...
These improvements make the dashboard more useful for inspecting AI-heavy workloads when traffic is incomplete or generated by real agents rather than controlled samples.
...
<Image src={resourceGraphAfter} alt="Resource graph after improvements, showing a cleaner layout with better spacing around highly-connected resources" />
...
If you host the MCP server on a non-default endpoint or path, pass a custom `path` or `endpointName`.
...
```csharp title="Custom certificate trust configuration"
...
- The `Never` pull policy is exposed for scenarios that require local images.
- The Docker Compose Service class adds a `PullPolicy` property.
- Docker build secrets can now be files such as `.npmrc`, allowing private registry authentication without baking credentials into the image.
- The data volume path now supports PostgreSQL version 18 and later.
...
These APIs existed in Aspire 13.1 but did not behave as expected. In Aspire 13.2, they correctly resolve endpoints based on the specified context.

<LearnMore>
  For more details, see: - [GitHub Issue
  #7153](https://github.com/microsoft/aspire/issues/7153) - [GitHub PR
  #14278](https://github.com/microsoft/aspire/pull/14278) - [Context-based endpoint
  resolution](/architecture/resource-hierarchies/#context-based-endpoint-resolution)
</LearnMore>
...
<Aside type="caution">
  ...
  by `Aspire.Hosting.Foundry`. See the [Foundry package and namespace
  transition](#foundry-package-and-namespace-transition) in breaking changes
  for migration steps.
</Aside>

<LearnMore>
  For more details, see [Microsoft Foundry
  integration](/integrations/cloud/azure/azure-ai-foundry/azure-ai-foundry-get-started/).
</LearnMore>
...
- Create virtual networks with `AddAzureVirtualNetwork` and add subnets with `AddSubnet`.
- Provide deterministic outbound public IP addresses with `AddNatGateway` and `WithNatGateway`.
- Control traffic with network security group shorthand methods (`AllowInbound`, `DenyInbound`, `AllowOutbound`, `DenyOutbound`) or explicit `AddNetworkSecurityGroup` with `WithSecurityRule`.
- Connect securely to Azure services over private networks with `AddPrivateEndpoint`, which automatically creates private DNS zones and virtual network links and disables public access on the target resource.

<LearnMore>
  For more details, see [Azure Virtual Network
  integration](/integrations/cloud/azure/azure-virtual-network/).
</LearnMore>
...
Keyed embeddings are also supported with `AddKeyedEmbeddingGenerator`.
...
```csharp title="Schedule ACR image cleanup"
...
- MongoDB connection string options now correctly prepend a forward slash.
- The new [`Aspire.MongoDB.EntityFrameworkCore` client integration](/integrations/databases/efcore/mongodb/mongodb-efcore-get-started/) adds Entity Framework Core support for MongoDB.
- Oracle EF Core adds multi-targeting support.
- SQL Server adds new `Aspire.Hosting.SqlServer` exports.
...
For an Event Hubs consumer group resource named `orders-cg`, the resource-name encoder produces `ORDERS_CG`; separately, uppercasing the `ConsumerGroupName` property key produces `CONSUMERGROUPNAME`. Aspire joins them as `ORDERS_CG_CONSUMERGROUPNAME`.
...
```diff lang="csharp" title="Read injected connection property"
...
```diff lang="python" title="Read injected connection property"
...
```diff lang="typescript" title="Read injected connection property"
...
```diff lang="go" title="Read injected connection property"
- database := os.Getenv("SQLDB_DATABASE")
+ database := os.Getenv("SQLDB_DATABASENAME")
```
...
```diff lang="csharp" title="AppHost.cs"
...
### Foundry package and namespace transition

Aspire 13.2 standardizes the Foundry hosting surface under `Aspire.Hosting.Foundry`.

| Previous 13.1 name | 13.2 name |
| ------------------ | --------- |
| Hosting package: `Aspire.Hosting.Azure.AIFoundry` | Hosting package: `Aspire.Hosting.Foundry` |
| Resource and model namespace: `Aspire.Hosting.Azure` | Resource and model namespace: `Aspire.Hosting.Foundry` |
...
Migration tip: update your package reference, add `using Aspire.Hosting.Foundry;` for direct resource or model type references, and rename `AddAzureAIFoundry(...)` to `AddFoundry(...)`. Remove `using Aspire.Hosting.Azure;` only if your code no longer uses other types from that namespace. Extension methods remain in the `Aspire.Hosting` namespace.
...
```csharp title="Migration example"
// Before
builder.AddContainer("worker", "contoso/worker")
    .WithDockerfile("../worker")
...
Aspire is built in the open, and this release wouldn't be what it is without you. A huge thank you to everyone who helped make Aspire 13.2 possible.

<ReleaseCommunity version="13.2" />
