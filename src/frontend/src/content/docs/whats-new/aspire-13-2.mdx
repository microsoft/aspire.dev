...
Aspire 13.2.1 is a patch release focused on the TypeScript AppHost SDK.
...
<Aside type="note">
  `withConnectionPropertyValue` is restored as a compatibility alias in the generated SDKs, so it is not a runtime SDK break. However, the updated `apphost.ts` samples have been migrated to use `withConnectionProperty`.
</Aside>
