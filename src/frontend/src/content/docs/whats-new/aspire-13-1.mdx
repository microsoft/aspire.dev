...
New APIs allow configuring HTTPS certificate key pairs for resources that need to terminate TLS connections. This is useful for securing traffic to containers and other resources with the ASP.NET Core developer certificate:

```csharp title="C# — TLS termination with developer certificate"
builder.AddYarp("gateway")
    .WithDeveloperCertificateKeyPair();
```

```csharp title="C# — TLS termination with custom certificate"
var passwordParam = builder.AddParameter("tls-password", secret: true);

builder.AddYarp("gateway")
    .WithCertificateKeyPair(certificate, password: passwordParam);
```

Built-in support is included for YARP, Uvicorn, Keycloak, and Redis containers.
...
```csharp title="C# — Endpoint proxy support"
var redis = builder.AddRedis("cache", 1234)
