...
The Aspire Type System (ATS) APIs that power polyglot AppHost authoring are now generally available — projects that previously suppressed the `ASPIREATS001` experimental warning can remove those suppressions. The generated TypeScript SDK now also carries JSDoc derived from the C# XML docs (summaries, parameters, return values, and enum values), supports fluent async method chaining so builder chains keep working even when an async method appears in the middle, and accepts an `ExternalServiceResource` in `withEnvironment` to match the C# `WithEnvironment` API.
...
aspire otel traces --min-duration 500ms
```

The `aspire otel traces` command also accepts `--min-duration` to drop spans shorter than a given threshold, which makes it easier to surface slow operations when profiling a large trace.
...
[`aspire doctor`](/reference/cli/commands/aspire-doctor/) now reports the current Aspire CLI version, shows an update notice when a newer version is available, and reports the AppHost SDK version when an AppHost is detected. This makes CLI/SDK version mismatches easier to spot before they become restore or runtime issues. It also lists every Aspire CLI installation discovered on the machine — the active binary, peer installs, shadowed binaries, and `dotnet tool` store installs — so conflicting installs are easy to find.
...
`aspire agent init` now bundles a dedicated Aspire deployment skill that walks AI coding agents through deploying an app — target selection across Docker Compose, Kubernetes, Azure, and AWS, preflight checks, and CI/CD guidance including GitHub Actions workflow templates. The workflow skills (`aspire`, `aspireify`, and the new `aspire-deployment`) are now distributed through the external [`@microsoft/aspire-skills`](https://www.npmjs.com/package/@microsoft/aspire-skills) npm package, verified with npm provenance and cached locally, so skills can be updated independently of the CLI.
...
- On macOS and Linux, DCP now uses the ASP.NET Core developer certificate for TLS, bringing the HTTPS developer experience previously available only on Windows (opt out with `ASPIRE_DCP_USE_DEVELOPER_CERTIFICATE=false`).
...
<Aside type="note">
  This release also includes a security fix for the JavaScript/TypeScript
  AppHost templates (GitHub advisory
  [GHSA-q7rr-3cgh-j5r3](https://github.com/microsoft/aspire/security/advisories/GHSA-q7rr-3cgh-j5r3)).
  If you generated a polyglot AppHost from an earlier template, regenerate or
  update it to pick up the patched dependencies.
</Aside>
...
### Polyglot SDK modules consolidated under `.aspire/modules/`
