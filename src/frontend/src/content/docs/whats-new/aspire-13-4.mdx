...
The `tmp` npm package (a dev-only transitive dependency used by the Visual Studio Code extension and the Angular playground sample) was updated to address [GHSA-ph9p-34f9-6g65](https://github.com/advisories/GHSA-ph9p-34f9-6g65), a high-severity path traversal vulnerability affecting versions prior to `0.2.6`. This dependency is not included in any published Aspire NuGet packages or runtime artifacts; only the VS Code extension tooling and playground sample were affected.
