...
- **CLI discoverability and diagnostics** with `aspire integration list`, `aspire integration search`, `--search` for logs and telemetry, version checks in `aspire doctor`, better logs output, more consistent command error handling, and Aspire workflow skills now sourced from an external bundle.
...
### Aspire workflow skills from external bundle

The `aspire`, `aspireify`, and `aspire-deployment` skills installed by `aspire agent init` are now fetched from the external `@microsoft/aspire-skills` npm bundle instead of being embedded in the CLI binary. The skill content now lives in a single, independently-updatable package at [`microsoft/aspire-skills`](https://github.com/microsoft/aspire-skills), while the install flow remains unchanged.

The CLI resolves and verifies the pinned bundle on first use — checking npm provenance and tarball integrity, validating manifest file hashes, and rejecting unsafe paths — then caches the extracted bundle locally:

- **Windows**: `%USERPROFILE%\.aspire\cache\aspire-skills\`
- **Linux / macOS**: `~/.aspire/cache/aspire-skills\`

Subsequent installs reuse the cached bundle without network access.
...
<LearnMore>
  ...
  walkthrough, see [Use AI coding agents](/get-started/ai-coding-agents/).
</LearnMore>
