...
Aspire 13.4 broadens the app model and streamlines daily operations. TypeScript AppHosts reach **general availability** alongside C#, with new Go, Blazor WebAssembly, and Bun hosting integrations. The release also expands Kubernetes and AKS deployment APIs, AI agent support, resource commands, CLI discovery and diagnostics, the dashboard, and Visual Studio Code tooling.
...
- C# and TypeScript AppHosts are both supported as first-class options, with stronger TypeScript startup validation, more complete SDK generation, new TypeScript samples, and documentation parity with C# AppHost examples.
- New hosting integrations provide AppHost APIs for [Go](/integrations/frameworks/go/go-get-started/) (`AddGoApp`) and first-party Bun support in `Aspire.Hosting.JavaScript` for [Bun](/integrations/frameworks/bun-apps/) (`AddBunApp`).
- Kubernetes and AKS deployment improvements include [cert-manager integration, Gateway API and Azure Application Gateway for Containers (AGC) support](/deployment/kubernetes/aks/#expose-your-app-to-the-internet), Kubernetes manifest resources, external Helm charts, and consolidated Helm chart configuration.
- Aspire workflow skills move into the new [`microsoft/aspire-skills`](https://github.com/microsoft/aspire-skills) bundle, with clearer separation between first-run setup, lifecycle orchestration, monitoring, AppHost wiring, and deployment guidance, plus eval-driven iteration on agent routing.
- Resource commands gain typed arguments, visibility controls, immediate result display, named CLI options, resource-scoped help, built-in parameter command options, and the experimental `WithProcessCommand` API.
- CLI discovery and diagnostics add `aspire integration list`, `aspire integration search`, `--search` for logs and telemetry, version checks in `aspire doctor`, better logs output, and more consistent command error handling.
- Dashboard and editor updates include the AI Agents dialog, resource state fixes, CodeLens actions that appear without opening the Aspire panel, and improved VS Code terminal and debug output integration.
- NATS, Azure Front Door, Foundry hosted agents, and other integrations are updated.
- …and much more.
...
Search, tail, and resource filters are applied before data is streamed back to the CLI, which keeps large log streams responsive. It also means your coding agent no longer has to load all the output into context and `grep`, speeding up the diagnostic process while using fewer tokens.
...
The Aspire extension for Visual Studio Code now shows AppHost CodeLens actions directly in the editor without requiring the Aspire panel to be open. Run, Stop, Restart, Open Dashboard, and View Logs actions make common AppHost operations available where you're editing code.
...
1. Update the CLI by running `aspire update --self`. This step is **required** for TypeScript AppHosts; running `aspire update` against a 13.3.x TypeScript project before updating the CLI fails with `No code generator found for language: TypeScript` and leaves the project in a partially-upgraded, unrunnable state (see [microsoft/aspire#17077](https://github.com/microsoft/aspire/issues/17077)).
2. Update your projects by running `aspire update` from the root of your repository. For non-C# AppHosts, this regenerates SDK modules under `.aspire/modules/`.
3. Run `aspire doctor` to check your environment setup and spot conflicting CLI installs.
4. Audit scripts and CI for removed or changed commands, including `aspire exec` and non-interactive `aspire update` calls.
5. Update Kubernetes deployment code by moving Helm chart properties into `WithHelm(...)`, renaming `ingress.WithRoute(...)` to `ingress.WithPath(...)`, and marking routed endpoints external with `WithExternalHttpEndpoints()`.
6. Update renamed APIs: `PublishAsNpmPackageScript` became `PublishAsPackageScript`, and Foundry hosted agents use `WithComputeEnvironment` instead of `PublishAsHostedAgent`, with the new `AddPromptAgent` parameter order.
7. Review Keycloak references for the HTTPS primary endpoint and Azure Front Door deployments for naming conflicts with Azure.Provisioning. Set explicit names if needed.
...
We'd love to hear about your experience with Aspire 13.4. Share what you're building on [GitHub](https://github.com/microsoft/aspire/issues) or join us on [Discord](https://aka.ms/aspire-discord).
