...
### Aspire workflow skills bundle

The `aspire`, `aspireify`, and `aspire-deployment` skills installed by `aspire agent init` are now packaged as the Aspire skills bundle from [`microsoft/aspire-skills`](https://github.com/microsoft/aspire-skills). Aspire 13.4 includes an embedded snapshot for default installs, and the optional remote fetch path can download matching GitHub release assets instead of relying only on skill content baked directly into the CLI binary. The install flow remains unchanged.

When remote fetching is enabled, the CLI verifies the GitHub release asset with GitHub artifact attestation (Sigstore-backed SLSA provenance), validates manifest file hashes, rejects unsafe paths, and caches the extracted bundle locally:

- **Windows**: `%USERPROFILE%\.aspire\cache\aspire-skills\`
- **Linux / macOS**: `~/.aspire/cache/aspire-skills/`

Subsequent installs reuse the cached bundle when present.
