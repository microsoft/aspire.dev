...
### Security patch: undici dependency updated

The `undici` HTTP library used by the VS Code extension was updated from `7.21.0` to `7.27.0` to address three security advisories:

- [GHSA-cxjh-pqwp-8mfp](https://github.com/advisories/GHSA-cxjh-pqwp-8mfp) — CRLF Injection via the `upgrade` option (medium)
- [GHSA-f269-vfmq-vjvj](https://github.com/advisories/GHSA-f269-vfmq-vjvj) — Malicious WebSocket 64-bit length overflows the parser (high)
- [GHSA-9f74-3xc5-r7g4](https://github.com/advisories/GHSA-9f74-3xc5-r7g4) — HTTP Request/Response Smuggling (medium)

No extension configuration or behavior changes are required.
