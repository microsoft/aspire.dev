...
## 🛸 Aspire skills for AI coding agents
...
Aspire 13.4 validates TypeScript AppHosts before startup, so type-checking and compile errors fail early. New TypeScript projects use `.aspire/modules/` for generated SDK files, and `aspire init --language typescript` creates a nested `aspire-apphost/` package when run in an existing JavaScript or TypeScript app.

Docs now include TypeScript examples alongside C# where the API is available, and the release adds TypeScript samples you can start from instead of translating C# projects.
...
await web.withReference(bunApi);
