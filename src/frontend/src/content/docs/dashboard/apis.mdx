...
When a `resource` filter is specified with `?follow=true`, the stream waits for the named resource(s) to appear in the dashboard before it starts yielding data. This means you can begin streaming before a resource has emitted any telemetry — the connection stays open and data flows once the resource is discovered.
