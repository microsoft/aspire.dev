...
How you configure the dashboard depends on whether it's being started by the Aspire AppHost project, or whether it is started in [standalone mode](overview#standalone-mode).

### Aspire AppHost

The AppHost automatically configures the dashboard. However, it is possible to override values if required. The recommeded way to configure the dashboard from the Aspire AppHost is to add environment variables to the _launchSettings.json_ file.
...
The proceeding launch settings increases `Dashboard:TelemetryLimits:MaxLogCount` [telemetry limit](#telemetry-limit) to 50,000. The `:` delimiter must be replaced with double underscore (`__`) in environment variable names.
