...
These launch settings increase the `Dashboard:TelemetryLimits:MaxLogCount` [telemetry limit](#telemetry-limit) to 50,000. The `:` delimiter must be replaced with double underscore (`__`) in environment variable names.
