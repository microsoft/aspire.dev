...
The `x-mcp-api-key` value is used to secure access to the MCP but the AI assistant must have access to it. Use features in the AI assistant to store the value as securely as possible. For example, if you're using VS Code then [input variables](https://code.visualstudio.com/docs/copilot/customization/mcp-servers#_input-variables-for-sensitive-data) can be used to securely configure the API key and avoid adding the API key to source control in an *mcp.json* file.
