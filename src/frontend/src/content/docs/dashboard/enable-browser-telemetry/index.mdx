...
The Aspire dashboard can be configured to receive telemetry sent from browser apps. This feature is useful for monitoring client-side performance and user interactions. Browser telemetry requires additional dashboard configuration. Generic JavaScript browser apps also need the [JavaScript OTEL SDK](https://opentelemetry.io/docs/languages/js/getting-started/browser/); Blazor WebAssembly apps should use the Aspire Blazor hosting gateway/proxy support.
...
```json title="launchSettings.json" {12,25}
{
...
<LearnMore>
For more information, see [Aspire dashboard configuration: OTLP CORS](/dashboard/configuration/#otlp-cors).
</LearnMore>
...
API key authentication is automatically enabled when the dashboard is run from the AppHost. Dashboard authentication can be disabled by setting `ASPIRE_DASHBOARD_UNSECURED_ALLOW_ANONYMOUS` to `true` in [AppHost configuration](/app-host/configuration/). See [Dashboard security considerations](/dashboard/security-considerations/#anonymous-access) for the security implications.
...
<LinkCard
  ...
  description="Configure a generic JavaScript browser app to send OTLP telemetry to the dashboard."
  href="/dashboard/enable-browser-telemetry/browser-app-configuration/"
/>
<LinkCard
  title="Blazor WebAssembly integration"
  description="Use Aspire Blazor hosting to proxy Blazor WebAssembly telemetry through the app gateway."
