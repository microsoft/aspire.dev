...
description: Enable browser telemetry for Blazor WebAssembly apps with Aspire 13.4 Blazor hosting, gateway proxying, and client service defaults.
---

import { Tabs, TabItem } from '@astrojs/starlight/components';

Blazor WebAssembly apps run .NET in the browser, so they need a startup path that makes telemetry configuration available before the WebAssembly runtime starts. In Aspire 13.4, use the Aspire Blazor hosting integration instead of configuring a Blazor WebAssembly app like a generic JavaScript browser app that sends OTLP directly to the dashboard.
