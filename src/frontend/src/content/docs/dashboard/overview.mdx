...
import architectureDiagram from '@assets/dashboard/architecture-diagram.png';
...
- User interface to [stop, start, and restart resources](explore#resource-actions).
- Collects and displays logs and telemetry; [view structured logs, traces, and metrics](explore#monitoring-pages) in an intuitive UI.
- Enhanced debugging with [GitHub Copilot](copilot), your AI-powered assistant built into the dashboard.
...
For more information about using the dashboard during Aspire development, see [Explore dashboard features](explore).
...
For more information, see the [Standalone Aspire dashboard](standalone).
...
For more information, see [Aspire dashboard configuration](configuration).
...
<Image src={architectureDiagram} alt="A diagram showing the architecture of the Aspire dashboard." />
...
For more information, see [Aspire dashboard security considerations](security-considerations).

## Next steps

- [Explore the Aspire dashboard](explore)
