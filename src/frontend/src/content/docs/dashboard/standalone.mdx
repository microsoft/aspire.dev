...
- <abbr title="OpenTelemetry Protocol" data-tooltip-placement="top">OTLP</abbr> endpoint, which should match the dashboard's configuration, for example, `http://localhost:4317` for gRPC or `http://localhost:4318` for HTTP.
- <abbr title="OpenTelemetry Protocol" data-tooltip-placement="top">OTLP</abbr> protocol. The dashboard supports all <abbr title="OpenTelemetry Protocol" data-tooltip-placement="top">OTLP</abbr> protocols:
