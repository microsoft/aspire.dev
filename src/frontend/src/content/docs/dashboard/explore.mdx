...
Use the `timestamp` search qualifier in the filter bar to narrow structured log results to a specific date/time range. The qualifier accepts ISO 8601 date/time strings together with a comparison operator:

```text
timestamp:>2024-01-15T09:30:00
timestamp:<=2024-06-01T12:00:00Z
timestamp:>=2024-03-01T08:30:00+05:00
```
...
When using the **Add filter** dialog, selecting the **Timestamp** parameter shows a text input with a calendar icon. Clicking the calendar icon opens the browser's native date/time picker, which inserts the chosen date/time value in the correct format.
...
Use the `timestamp` search qualifier in the filter bar to narrow trace results to a specific date/time range. The qualifier accepts ISO 8601 date/time strings together with a comparison operator:
