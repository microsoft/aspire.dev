...
Each dimension filter row lists available tag values. Values are ordered numerically when all values parse as numbers, or alphabetically otherwise. Values that don't fit inline are shown as a **+N** count badge. Selecting the badge opens a popover with the full list of values. Tag values keep their natural width and are never truncated with an ellipsis while multiple tags are visible; a tag is only ellipsized when it's the sole displayed value and the available width is too narrow for its full text.
