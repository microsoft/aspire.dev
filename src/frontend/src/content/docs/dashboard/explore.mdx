...
- **Endpoints**: One or more URLs to reach the running resource directly. When a resource exposes a single endpoint and the column is too narrow to display the full URL, the text truncates with an ellipsis. When a resource exposes multiple endpoints, the ones that don't fit collapse into a **+N** badge—select the badge to view the remaining URLs in a popover.
...
- **Spans**: The resources involved in the request. When there isn't enough room to display every resource, the remaining ones collapse into a **+N** badge. Hover over the badge to see the hidden resources listed vertically in a tooltip.
...
When the header is too narrow to display every detail inline, the remaining details collapse into a **+N** badge. Hover over the badge to see the hidden details listed vertically in a tooltip.
...
Each dimension filter row lists available tag values. Values are ordered numerically when all values parse as numbers, or alphabetically otherwise. Values that don't fit inline are shown as a **+N** count badge. Selecting the badge opens a popover with the full list of values.
