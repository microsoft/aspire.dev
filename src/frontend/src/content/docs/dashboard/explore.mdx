...
Each dimension filter row lists available tag values. Values are ordered numerically when all values parse as numbers, or alphabetically otherwise. When a dimension has many tag values, up to 20 are displayed inline; any additional values are shown as a **+N** count badge. Selecting the badge opens a popover with the full list of values.
