...
<Aside type="note">
The **Panels** shortcuts are always listed, even when no panel is currently open. The rest of the shortcuts only appear in the Help dialog, and only respond to key presses, when the page or feature they navigate to is actually available. For example, in a standalone dashboard that's running without a connected resource service, the **<u>R</u>esources** and **<u>C</u>onsole Logs** shortcuts (and the corresponding pages) aren't listed or actionable, since there are no resources to show. Similarly, the terminal dock shortcut only appears when the terminal dock is available.
</Aside>
