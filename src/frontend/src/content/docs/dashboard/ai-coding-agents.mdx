...
## AI Agents button in the dashboard header

The Aspire dashboard includes an **AI Agents** button in the header (between the Help and Settings buttons). Clicking the button opens a dialog with setup instructions for using AI coding agents with the current dashboard instance. The dialog content adapts based on context:

- **AppHost-connected dashboard** — shows the CLI commands to connect an AI coding agent to the running Aspire app, including a pre-filled dashboard URL with the authentication token.
- **Standalone dashboard** — shows the commands to start the MCP server in `--dashboard-url` mode pointing at the standalone instance.

To hide the button, set `Dashboard:UI:DisableAgentHelp` to `true`. See [dashboard configuration](/dashboard/configuration/#general) for details.
...
:::caution
The `--allow-anonymous` flag starts the dashboard without authentication. Only use this on your local machine. See [Dashboard security considerations](/dashboard/security-considerations/) for more information.
:::
