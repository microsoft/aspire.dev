...
- Browser front end authenticated with a browser token.
...
When enabled, the telemetry API is served on the same endpoint as the dashboard front end (`ASPNETCORE_URLS`). Enable and secure the API with the following configuration:
