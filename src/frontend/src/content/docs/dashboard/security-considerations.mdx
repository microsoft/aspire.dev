...
The SQLite database has no independent authorization or encryption layer. Its security boundary is the local file system. Application-name scoping organizes data but isn't an authorization boundary.
...
- On Unix-like systems, the dashboard creates the shared `runs` and `resumes` directories, and `Resume` mode's application directories, with owner-only permissions (`0700`), and repairs an existing directory to that mode.
