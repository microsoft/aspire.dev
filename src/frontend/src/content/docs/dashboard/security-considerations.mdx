...
When the dashboard is launched by an Aspire AppHost, the <abbr title="OpenTelemetry Protocol" data-tooltip-placement="top">OTLP</abbr> and telemetry API endpoints are both enabled and secured with API key authentication automatically. However, when the dashboard is run in [standalone mode](/dashboard/standalone/), additional configuration is required to enable and secure these endpoints.

### Incoming <abbr title="OpenTelemetry Protocol" data-tooltip-placement="top">OTLP</abbr> endpoint
...
To prevent untrusted apps from sending telemetry to Aspire, the <abbr title="OpenTelemetry Protocol" data-tooltip-placement="top">OTLP</abbr> endpoint should be secured. The <abbr title="OpenTelemetry Protocol" data-tooltip-placement="top">OTLP</abbr> endpoint is automatically secured with an API key when the dashboard is started by Aspire tooling. Additional configuration is required for standalone mode.
...
- Starts the Aspire dashboard image and exposes <abbr title="OpenTelemetry Protocol" data-tooltip-placement="top">OTLP</abbr> endpoints as port `4317` (gRPC) and port `4318` (HTTP)
- Configures the <abbr title="OpenTelemetry Protocol" data-tooltip-placement="top">OTLP</abbr> endpoint to use `ApiKey` authentication. This requires that incoming telemetry has a valid `x-otlp-api-key` header value.
