...
The Aspire test project templates rely on the [Aspire.Hosting.Testing](https://www.nuget.org/packages/Aspire.Hosting.Testing) NuGet package. This package exposes the `DistributedApplicationTestingBuilder` class, which is used to create a test host for your distributed application. The distributed application testing builder launches your AppHost project with instrumentation hooks so that you can access and manipulate the host at various stages of its lifecycle. In particular, `DistributedApplicationTestingBuilder` provides you access to `IDistributedApplicationBuilder` and `DistributedApplication` classes to create and start the [AppHost](/get-started/app-host/).
...
<LearnMore>
For more information about the .NET CLI, see [`dotnet new`](https://learn.microsoft.com/dotnet/core/tools/dotnet-new) command documentation.
</LearnMore>

## Explore the test project

The following example test project was created as part of the **Aspire Starter Application** template. If you're unfamiliar with it, see [Build your first app](/get-started/first-app/). The Aspire test project takes a project reference dependency on the target AppHost. Consider the template project:

<Pivot id="xunit">

```xml title="AspireApp.Tests.csproj"
...
- The `builder` is an instance of `IDistributedApplicationTestingBuilder` that represents the AppHost.
- The `builder` instance has its service collection configured with the standard HTTP resilience handler. For more information, see [Build resilient HTTP apps: Key development patterns](https://learn.microsoft.com/dotnet/core/resilience/http-resilience).
- The `builder` has its `BuildAsync` method invoked, which returns the `DistributedApplication` instance as the `app`.
