...
public sealed class LiveEndpointsTests
{
    ...
    public async Task YouTubeWebhook_ValidatesSignatureBeforeQueuing(bool valid, HttpStatusCode expected)
    {
        await using var server = await LiveHttpServer.StartAsync();
        const string body = "<feed><entry><id>video-123</id></entry></feed>";
        using var request = new HttpRequestMessage(HttpMethod.Post, "/api/live/youtube/webhook")
        {
            Content = new StringContent(body, Encoding.UTF8, "application/atom+xml"),
        };
        request.Headers.Add("X-Hub-Signature", valid
    ...
    private sealed class LiveHttpServer(
        ...
        TaskCompletionSource streamEnded) : IAsyncDisposable
    {
        ...
        public IYouTubeWebSubSubscriptionState Subscriptions =>
            app.Services.GetRequiredService<IYouTubeWebSubSubscriptionState>();

        public static async Task<LiveHttpServer> StartAsync(
            string environment = "Production", bool enableDev = true)
        {
            ...
            var options = new LiveStatusOptions
            {
                ...
                YouTube = new YouTubeOptions { WebhookSecret = WebhookSecret, ApiKey = "unused-test-key" },
            };
            ...
            return new LiveHttpServer(app, app.GetTestClient(), time, streamEnded);
        }
