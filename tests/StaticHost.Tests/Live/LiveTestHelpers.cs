...
internal static class LiveTestHelpers
{
    ...
    public static void AssertSafeLogs<T>(YouTubeRecordingLogger<T> logger, params string[] omittedValues)
    {
        ...
        foreach (var entry in logger.Entries)
        {
            ...
            AssertSafeText(entry.Message);
            foreach (var field in entry.Fields)
            {
                AssertSafeText(field.Key);
                AssertSafeText(field.Value?.ToString() ?? "");
            }
        }

        void AssertSafeText(string text)
        {
            Assert.DoesNotContain("payload-sentinel", text);
            Assert.DoesNotContain("forged-line", text);
            foreach (var separator in new[] { '\r', '\n', '\u0085', '\u2028', '\u2029' })
            {
                Assert.DoesNotContain(separator, text);
            }
            foreach (var value in omittedValues)
            {
